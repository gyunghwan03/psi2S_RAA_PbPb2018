...
void get_Eff_JPsi_pbpb_ctauCut_ATLAS(
    int state = 2,
...
)
{
  ...
  double centBin_mid[6] = {0, 10, 20, 80, 120, 160};
  ...
  TH1D *hcent_reco_2 = new TH1D("hcent_reco_2", "hcent_reco_2", 5, centBin_mid);
  ...
  TH1D *hcent_gen_2 = new TH1D("hcent_gen_2", "hcent_gen_2", 5, centBin_mid);
  ...
  // for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for (int igen = 0; igen < Gen_QQ_size; igen++)
    {
      ...
      if (fabs(JP_Gen->Rapidity()) <= 2 && JP_Gen->Pt() >= 9)
      ...
      if (Rapidity_g <= 2 && JP_Gen->Pt() >= 9) // && JP_Gen->Pt() > 6.5 && JP_Gen->Pt() < 40)
      {
...
//      if ( Reco_QQ_VtxProb[irqq]  < 0.005 ) continue;
  ...
  */
  ...
  hcent_eff_2->SetName(Form("mc_eff_vs_cent_TnP%d_PtW%d_pt_9_to_40_absy0_2  ", isTnP, 1));
  ...
  TString outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_JPsi_PtW%s_tnp%d_ctauCut_ATLAS.root", cLow, cHigh, ptSys.Data(), isTnP);
  if (state == 2)
    outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_JPsi_PtW%s_tnp%d_ctauCut_ATLAS.root", cLow, cHigh, ptSys.Data(), isTnP);
