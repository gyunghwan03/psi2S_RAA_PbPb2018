...
// v5 : prompt/nonprompt are handled by state (1: prompt, 2: nonprompt)
  ...
  if(state==1){
    fPtWMid = TFile::Open("/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_AA_Jpsi_DATA_ctauCut_y0_2p4_2260410.root","READ");
    if(!fPtWMid || fPtWMid->IsZombie()){
      cout << "[PtW] fallback to y0_2p4_260310_2exp.root for prompt." << endl;
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_AA_Jpsi_DATA_ctauCut_y0_2p4_260310_2exp.root","READ");
    }
    if(!fPtWMid || fPtWMid->IsZombie()){
    ...
    if(!fPtWFor || fPtWFor->IsZombie()){
      cout << "[PtW] fallback to y1p6_2p4_251118.root for prompt." << endl;
      fPtWFor = TFile::Open("../compareDataToMC/ratioDataMC_AA_Jpsi_DATA_ctauCut_y1p6_2p4_251118.root","READ");
    }
    if(!fPtWFor || fPtWFor->IsZombie()){
      cout << "[PtW] fallback to y1p6_2p4_251103.root for prompt." << endl;
    ...
    if(!fPtWMid || fPtWMid->IsZombie()){
      ...
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_AA_BtoJpsi_DATA_ctauCut_y0_2p4_260310_2exp.root","READ");
    }
    if(!fPtWMid || fPtWMid->IsZombie()){
    ...
    if(!fPtWFor || fPtWFor->IsZombie()){
      cout << "[PtW] fallback to y1p6_2p4_251118.root for nonprompt." << endl;
      fPtWFor = TFile::Open("../compareDataToMC/ratioDataMC_AA_BtoJpsi_DATA_ctauCut_y1p6_2p4_251118.root","READ");
    }
    if(!fPtWFor || fPtWFor->IsZombie()){
      cout << "[PtW] fallback to y1p6_2p4_251103.root for nonprompt." << endl;
...
//draw same
  ...
  TString outFileName;
  if(state==1 && isPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_psi2S_PtW%d_tnp%d_ctauCut_260416_2exp.root",cLow,cHigh,1,isTnP);
  else if(state==1 && !isPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_psi2S_PtW%d_tnp%d_ctauCut_260416_noPtW.root",cLow,cHigh,1,isTnP);
  else if(state==2 && isPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_psi2S_PtW%d_tnp%d_ctauCut_260416_2exp.root",cLow,cHigh,1,isTnP);
  else if(state==2 && !isPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_psi2S_PtW%d_tnp%d_ctauCut_260416_noPtW.root",cLow,cHigh,1,isTnP);
