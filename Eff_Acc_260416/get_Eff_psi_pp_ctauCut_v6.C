...
using namespace std;
// v3 : merge prompt and non-prompt psi(2S) pp ctau cut
...
void get_Eff_psi_pp_ctauCut_v6(
  int state=1, bool applyPtWeight = true,
  int nEvt = -1,
  ...
  else if(state==2) inputMC1 = "/data/Oniatree/miniAOD/OniatreeMC_BPsi2SMM_TuneCUETP8M1_5p02TeV_pythia8_ptHatMin2_ONIATREE.root"; //pp non prompt
  ...
  if(state==1){
    fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_psi2S_DATA_ctauCut_y0_2p4_260323_2exp.root","READ");
    if(!fPtWMid || fPtWMid->IsZombie()){
      cout << "[PtW] fallback to y0_2p4_260310_2exp.root for prompt." << endl;
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_psi2S_DATA_ctauCut_y0_2p4_260310_2exp.root","READ");
    }
    if(!fPtWMid || fPtWMid->IsZombie()){
      cout << "[PtW] fallback to y0_2p4_260310.root for prompt." << endl;
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_psi2S_DATA_ctauCut_y0_2p4_260310.root","READ");
    }

    fPtWFor = TFile::Open("../compareDataToMC/ratioDataMC_pp_psi2S_DATA_ctauCut_y1p6_2p4_260323_2exp.root","READ");
    if(!fPtWFor || fPtWFor->IsZombie()){
      cout << "[PtW] fallback to y1p6_2p4_260323.root for prompt." << endl;
      fPtWFor = TFile::Open("../compareDataToMC/ratioDataMC_pp_psi2S_DATA_ctauCut_y1p6_2p4_260323.root","READ");
    }
  }
  if(state==2){
    fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_Btopsi2S_DATA_ctauCut_y0_2p4_260323_2exp.root","READ");
    if(!fPtWMid || fPtWMid->IsZombie()){
      cout << "[PtW] fallback to y0_2p4_260323_2exp.root (BtoPsi2S name) for nonprompt." << endl;
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_BtoPsi2S_DATA_ctauCut_y0_2p4_260323_2exp.root","READ");
    }
    if(!fPtWMid || fPtWMid->IsZombie()){
      cout << "[PtW] fallback to y0_2p4_260310_2exp.root for nonprompt." << endl;
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_Btopsi2S_DATA_ctauCut_y0_2p4_260310_2exp.root","READ");
    }
    if(!fPtWMid || fPtWMid->IsZombie()){
      cout << "[PtW] fallback to y0_2p4_260310.root for nonprompt." << endl;
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_Btopsi2S_DATA_ctauCut_y0_2p4_260310.root","READ");
    }

    fPtWFor = TFile::Open("../compareDataToMC/ratioDataMC_pp_Btopsi2S_DATA_ctauCut_y1p6_2p4_260323_2exp.root","READ");
    if(!fPtWFor || fPtWFor->IsZombie()){
      cout << "[PtW] fallback to y1p6_2p4_260323_2exp.root (BtoPsi2S name) for nonprompt." << endl;
      fPtWFor = TFile::Open("../compareDataToMC/ratioDataMC_pp_BtoPsi2S_DATA_ctauCut_y1p6_2p4_260323_2exp.root","READ");
    }
    if(!fPtWFor || fPtWFor->IsZombie()){
      cout << "[PtW] fallback to y1p6_2p4_260323.root (BtoPsi2S name) for nonprompt." << endl;
      fPtWFor = TFile::Open("../compareDataToMC/ratioDataMC_pp_BtoPsi2S_DATA_ctauCut_y1p6_2p4_260323.root","READ");
    }
  }
  ...
  for(int i=0; i<5; ++i){
    TString fpath_tmp = Form("roots_2S_pp/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data());
  ...
  {
    TFile *f_ctau_tmp = TFile::Open(Form("roots_2S_pp/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root",fname.Data()), "READ");
    if(f_ctau_tmp && !f_ctau_tmp->IsZombie()){
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
  ...
  {
    if(iev%100000==0) cout << ">>>>> EVENT " << iev << " / " << nevt <<  " ("<<(int)(100.*iev/nevt) << "%)" << endl;

    mytree->GetEntry(iev);
      weight_gen = Gen_weight;
      weight_reco = Gen_weight;
    ...
    for(int igen = 0; igen<Gen_QQ_size; igen++){
      ...
      if(state==1){
        ...
        if(JP_Gen->Pt() > 6.5) hy_gen->Fill(Rapidity_g, weight_gen*pt_weight);
        ...
        { 
          hpt_gen_2->Fill(JP_Gen->Pt(),weight_gen*pt_weight); 
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      pt_weight = EvalPtWeight(applyPtWeight, JP_Reco->Rapidity(), JP_Reco->Pt(), fptwMid, fptwFor);
      cout.precision(4);
      //cout << "pT : " << JP_Reco->Pt() << "\t" << "pT weight =\t" << pt_weight << "\tweight =\t"<< weight_reco << endl;
      ...
      if(HLTPass==true && HLTFilterPass==true){
        if(Rapidity > 1.6 && Rapidity < 2.4) {
          ...
          if(pt > 3.5 && pt < 40) {
            ...
            if(state==1) { if(Reco_QQ_ctau3D[irqq] < l_cut) {
              hInt_reco_1->Fill(1, weight_reco*tnp_weight*pt_weight); 
        ...
        if(Rapidity < 1.6 && JP_Reco->Pt() > 6.5 && JP_Reco->Pt() < 40) {
          ...
          if(pt > 6.5 && pt < 50) {
            ...
            if(state==1) { 
              if(Reco_QQ_ctau3D[irqq] < l_cut) { 
                hInt_reco_2->Fill(1, weight_reco*tnp_weight*pt_weight); 
  ...
  hInt_eff_1->SetTitle("Integrated Eff: Rapidity 1.6-2.4");
  hInt_eff_2->SetTitle("Integrated Eff: Rapidity 0.0-1.6");
  hy_eff->SetTitle("Eff vs |y|"); 
  ...
  hpt_eff_1_noW ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy1p6_2p4_noW",isTnP, isPtWeight));
  hpt_eff_2_noW ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy0_1p6_noW",isTnP, isPtWeight));
  hInt_eff_1_noW ->SetName(Form("mc_eff_Integrated_TnP%d_PtW%d_absy1p6_2p4_noW",isTnP, isPtWeight));
  hInt_eff_2_noW ->SetName(Form("mc_eff_Integrated_TnP%d_PtW%d_absy0_1p6_noW",isTnP, isPtWeight));


  TString outFileName;
  if(state == 1 && applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_rap_prompt_pp_psi2S_PtW%d_tnp%d_ctauCut_260416_2exp.root",isPtWeight,isTnP);
  else if(state == 1 && !applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_rap_prompt_pp_psi2S_PtW%d_tnp%d_ctauCut_260416_noPtW.root",isPtWeight,isTnP);
  else if(state==2 && applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_rap_nprompt_pp_psi2S_PtW%d_tnp%d_ctauCut_260416_2exp.root",isPtWeight,
  isTnP);
  else if(state == 2 && !applyPtWeight)  outFileName = Form("./roots/mc_eff_vs_pt_rap_nprompt_pp_psi2S_PtW%d_tnp%d_ctauCut_260416_noPtW.root",isPtWeight,isTnP);
  ...
  cout << "Output file saved: " << outFileName.Data() << endl;
