	...
	if (type == 1 || type == 3) {
		...
		cout << endl << "Integrated bins:" << endl;
		{
			...
			if (totalPR < 10) {
				...
				// Draw and save integrated forward plot
				TCanvas* c_decayL_int_for = new TCanvas("c_decayL_int_for","",600,600);
				h_deff_pr_int_for->Draw("l");
				h_deff_np_int_for->Draw("same");
				h_deff_np_int_for->SetLineColor(kRed+2);
				jumSun(xmin,1,xmax,1,1,1);
				TLine *lcutv_int_for = new TLine(ctau3D_cut_int_for, 0, ctau3D_cut_int_for, 1);
				lcutv_int_for->Draw("same");
				drawText("3.5 < p_{T}^{#mu#mu} < 40 GeV/c", pos_x_mass, pos_y, text_color, text_size);
				drawText("1.6 < |y^{#mu#mu}| < 2.4", pos_x_mass, pos_y - pos_y_diff, text_color, text_size);
				drawText("|#eta^{#mu}| < 2.4", pos_x_mass, pos_y - pos_y_diff * 1.5, text_color, text_size);
				if (state == 1) {
					drawText(Form("L_{J/psi} cut: %.4f", ctau3D_cut_int_for), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
					drawText(Form("PR J/psi eff: %.3f", pr_eff_int_for), pos_x_mass, pos_y - pos_y_diff * 3.5, text_color, text_size);
					drawText(Form("NP J/psi res: %.3f", np_res_int_for), pos_x_mass, pos_y - pos_y_diff * 4, kRed+2, text_size);
					c_decayL_int_for->Update();
					c_decayL_int_for->SaveAs(Form("figs_1S_pp/decayL/PRMC/decay_state%d_pt3.5-40.0_y1.6-2.4_m%.1f-%.1f.pdf", state, massLow, massHigh));
				} else if (state == 2) {
					drawText(Form("L_{J/psi} cut: %.4f", ctau3D_cut_int_for), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
					drawText(Form("NP J/psi eff: %.3f", np_res_int_for), pos_x_mass, pos_y - pos_y_diff * 3.5, kRed+2, text_size);
					drawText(Form("PR J/psi res: %.3f", pr_eff_int_for), pos_x_mass, pos_y - pos_y_diff * 4, text_color, text_size);
					c_decayL_int_for->Update();
					c_decayL_int_for->SaveAs(Form("figs_1S_pp/decayL/NPMC/decay_state%d_pt3.5-40.0_y1.6-2.4_m%.1f-%.1f.pdf", state, massLow, massHigh));
				}
				delete c_decayL_int_for;
			}
		}

		{
			...
			if (totalPR < 10) {
				...
				// Draw and save integrated mid plot
				TCanvas* c_decayL_int_mid = new TCanvas("c_decayL_int_mid","",600,600);
				h_deff_pr_int_mid->Draw("l");
				h_deff_np_int_mid->Draw("same");
				h_deff_np_int_mid->SetLineColor(kRed+2);
				jumSun(xmin,1,xmax,1,1,1);
				TLine *lcutv_int_mid = new TLine(ctau3D_cut_int_mid, 0, ctau3D_cut_int_mid, 1);
				lcutv_int_mid->Draw("same");
				drawText("6.5 < p_{T}^{#mu#mu} < 40 GeV/c", pos_x_mass, pos_y, text_color, text_size);
				drawText("|y^{#mu#mu}| < 1.6", pos_x_mass, pos_y - pos_y_diff, text_color, text_size);
				drawText("|#eta^{#mu}| < 2.4", pos_x_mass, pos_y - pos_y_diff * 1.5, text_color, text_size);
				if (state == 1) {
					drawText(Form("L_{J/psi} cut: %.4f", ctau3D_cut_int_mid), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
					drawText(Form("PR J/psi eff: %.3f", pr_eff_int_mid), pos_x_mass, pos_y - pos_y_diff * 3.5, text_color, text_size);
					drawText(Form("NP J/psi res: %.3f", np_res_int_mid), pos_x_mass, pos_y - pos_y_diff * 4, kRed+2, text_size);
					c_decayL_int_mid->Update();
					c_decayL_int_mid->SaveAs(Form("figs_1S_pp/decayL/PRMC/decay_state%d_pt6.5-40.0_y0.0-1.6_m%.1f-%.1f.pdf", state, massLow, massHigh));
				} else if (state == 2) {
					drawText(Form("L_{J/psi} cut: %.4f", ctau3D_cut_int_mid), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
					drawText(Form("NP J/psi eff: %.3f", np_res_int_mid), pos_x_mass, pos_y - pos_y_diff * 3.5, kRed+2, text_size);
					drawText(Form("PR J/psi res: %.3f", pr_eff_int_mid), pos_x_mass, pos_y - pos_y_diff * 4, text_color, text_size);
					c_decayL_int_mid->Update();
					c_decayL_int_mid->SaveAs(Form("figs_1S_pp/decayL/NPMC/decay_state%d_pt6.5-40.0_y0.0-1.6_m%.1f-%.1f.pdf", state, massLow, massHigh));
				}
				delete c_decayL_int_mid;
			}
		}
