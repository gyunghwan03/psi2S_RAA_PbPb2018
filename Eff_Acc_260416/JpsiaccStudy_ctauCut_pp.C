...
void JpsiaccStudy_ctauCut_pp(int state = 1, int wtopt = 1, int isPtWgtUp = 0, TString rmk = "20260420")
{
  if (state != 1 && state != 2)
  {
  ...
  if (wtopt == 1)
  {
    ...
    if (state == 1)
    {
      fPtWMid = OpenFirstValid(ppPRMid, 1, "pp prompt mid");
      fPtWFor = OpenFirstValid(ppPRFor, 1, "pp prompt forward");
    }
    else
    {
      fPtWMid = OpenFirstValid(ppNPMid, 1, "pp nonprompt mid");
      fPtWFor = OpenFirstValid(ppNPFor, 1, "pp nonprompt forward");
    }

    if (!fPtWMid || !fPtWFor)
    {
      cout << "[ERROR] cannot open pt-weight input files for pp." << endl;
      return;
    }

    fptwMid = LoadClonedWeight(fPtWMid, "dataMC_Ratio1", "fptwMid_pp");
    fptwFor = LoadClonedWeight(fPtWFor, "dataMC_Ratio1", "fptwFor_pp");
    if (!fptwMid || !fptwFor)
    {
  ...
  TFile *wf = new TFile(Form("./roots/acceptance_%s_GenOnly_wgt%d_pp_SysUp%d_%s.root",
  ...
  TCanvas *cAll = new TCanvas("cAll_pp", "", 700, 600);
  hAccPt_2021_ally->GetXaxis()->SetRangeUser(6.5, 50.0);
  hAccPt_2021_ally->GetYaxis()->SetRangeUser(0.0, 1.3);
  hAccPt_2021_ally->Draw("E");
  cAll->SaveAs(Form("./figs/AccPt_AllY_pp_state%d_wgt%d_%s.png", state, wtopt, rmk.Data()));

  TCanvas *cMid = new TCanvas("cMid_pp", "", 700, 600);
  hAccPt_2021_midy->GetXaxis()->SetRangeUser(6.5, 50.0);
  hAccPt_2021_midy->GetYaxis()->SetRangeUser(0.0, 1.3);
  hAccPt_2021_midy->Draw("E");
  cMid->SaveAs(Form("./figs/AccPt_MidY_pp_state%d_wgt%d_%s.png", state, wtopt, rmk.Data()));

  TCanvas *cFor = new TCanvas("cFor_pp", "", 700, 600);
  hAccPt_2021_Fory->GetXaxis()->SetRangeUser(3.0, 50.0);
  hAccPt_2021_Fory->GetYaxis()->SetRangeUser(0.0, 1.3);
  hAccPt_2021_Fory->Draw("E");
  cFor->SaveAs(Form("./figs/AccPt_ForY_pp_state%d_wgt%d_%s.png", state, wtopt, rmk.Data()));
