...
void get_Eff_psi_pp_ctauCut_v5_noPtW(
  int state=1,
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
        ...
        if(Rapidity > 1.6 && Rapidity < 2.4) {
          ...
          for(int i=1; i < 5 ; i++) {
            ...
            if(pt > ptLow && pt < ptHigh) {
              ...
              if(state==1) { if(Reco_QQ_ctau3D[irqq] < l_cut) hpt_reco_1->Fill(pt, weight* tnp_weight* pt_weight); }
        ...
        if(Rapidity < 1.6 && JP_Reco->Pt() > 6.5 && JP_Reco->Pt() < 40) {
          ...
          for(int i=1; i < 7 ; i++) {
            ...
            if(pt > ptLow && pt < ptHigh) {
              ...
              if(state==1) { if(Reco_QQ_ctau3D[irqq] < l_cut) hpt_reco_2->Fill(pt, weight* tnp_weight* pt_weight); }
  ...
  TString outFileName = Form("./roots/mc_eff_vs_pt_rap_prompt_pp_psi2S_PtW%d_tnp%d_ctauCut_260323_2exp.root",isPtWeight,isTnP);
  if(state==2) outFileName = Form("./roots/mc_eff_vs_pt_rap_nprompt_pp_psi2S_PtW%d_tnp%d_ctauCut_260323_2exp.root",isPtWeight,isTnP);
