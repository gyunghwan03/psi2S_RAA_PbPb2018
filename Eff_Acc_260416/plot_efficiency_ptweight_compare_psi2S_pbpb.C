...
const TString kOutDir = "plot_outputs_eff_ptw_compare_psi2S";
...
void plot_efficiency_ptweight_compare_psi2S_pbpb(int state = 1) {
  gStyle->SetOptStat(0);
  EnsureOutDir();
  ...
  const TString filePtW = isPrompt
                              ? "./roots/mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_psi2S_PtW1_tnp1_ctauCut_260416_2exp.root"
                              : "./roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_psi2S_PtW1_tnp1_ctauCut_260416_2exp.root";

  const TString fileNoPtW = isPrompt
                                ? "./roots/mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_psi2S_PtW1_tnp1_ctauCut_260416_noPtW.root"
                                : "./roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_psi2S_PtW1_tnp1_ctauCut_260416_noPtW.root";
...
void run_ptweight_compare_psi2S_pbpb_all() {
  plot_efficiency_ptweight_compare_psi2S_pbpb(1);
  plot_efficiency_ptweight_compare_psi2S_pbpb(2);
}

void plot_efficiency_ptweight_compare_psi2S_pbpb(const char* mode) {
  TString opt = mode ? mode : "";
  opt.ToLower();
  opt.ReplaceAll(" ", "");

  if (opt.IsNull() || opt == "all" || opt.Contains("run_ptweight_compare_psi2s_pbpb_all")) {
    run_ptweight_compare_psi2S_pbpb_all();
    return;
  }
