...
bool ReadCtauCutValues(const TString &filePath, const char *histName, std::vector<double> &values)
{
  values.clear();
  TFile *fIn = TFile::Open(filePath, "READ");
  if (!fIn || fIn->IsZombie())
  {
    cout << "[ERROR] failed to open ctau cut file: " << filePath << "\n";
    if (fIn)
    {
      fIn->Close();
      delete fIn;
  ...
  TH1D *hist = dynamic_cast<TH1D *>(fIn->Get(histName));
  if (!hist)
  {
    cout << "[ERROR] missing histogram in ctau cut file: "
         << filePath << "::" << histName << "\n";
    fIn->Close();
    delete fIn;
    return false;
  ...
  for (int i = 1; i <= nBins; ++i)
    values.push_back(hist->GetBinContent(i));
...
bool BuildPtBinnedCtauCuts(const TString &filePath,
                           std::vector<double> &cutsForward,
                           std::vector<double> &cutsMid)
{
  std::vector<double> rawForward;
  std::vector<double> rawMid;
  if (!ReadCtauCutValues(filePath, "hpt_ctau_1", rawForward))
    return false;
  if (!ReadCtauCutValues(filePath, "hpt_ctau_2", rawMid))
    return false;

  constexpr size_t kExpectedForwardBins = 4; // 3.5-6.5, 6.5-9, 9-12, 12-40
  constexpr size_t kExpectedMidBins = 6;     // 6.5-9, 9-12, 12-15, 15-20, 20-25, 25-40

  if (rawForward.size() == kExpectedForwardBins)
    cutsForward = rawForward;
  else if (rawForward.size() == kExpectedForwardBins + 1)
    cutsForward.assign(rawForward.begin() + 1, rawForward.end());
  else
  {
    cout << "[ERROR] unexpected forward ctau bin count: " << rawForward.size()
         << " (expected 4 or 5)\n";
    return false;
  }

  if (rawMid.size() == kExpectedMidBins)
    cutsMid = rawMid;
  else if (rawMid.size() == kExpectedMidBins + 1)
    cutsMid.assign(rawMid.begin() + 1, rawMid.end());
  else
  {
    cout << "[ERROR] unexpected mid ctau bin count: " << rawMid.size()
         << " (expected 6 or 7)\n";
    return false;
  }

  cout << "[INFO] loaded pt-binned ctau cuts from " << filePath << "\n";
  return true;
}

bool FindPtBinnedCtauCut(double pt,
                         const std::vector<double> &ptEdges,
                         const std::vector<double> &cutValues,
                         double &ctauCut)
{
  if (ptEdges.size() < 2 || cutValues.size() + 1 != ptEdges.size())
    return false;

  for (size_t i = 0; i < cutValues.size(); ++i)
  {
    const bool isLast = (i + 1 == cutValues.size());
    const bool inBin = isLast
                           ? (pt >= ptEdges[i] && pt <= ptEdges[i + 1])
                           : (pt >= ptEdges[i] && pt < ptEdges[i + 1]);
    if (inBin)
    {
      ctauCut = cutValues[i];
...
void efficiency_1d_pp(long nEvt = 1000, bool isPr = true,
                      ...
                      bool applyCtauCut = true) // always true like PbPb
{
  ...
  if (isPtW)
  {
    const std::string ptWeightMidPath =
        isPr
            ? "/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_pp_Jpsi_DATA_ctauCut_y0_2p4_260410.root"
            : "/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_pp_BtoJpsi_DATA_ctauCut_y0_2p4_260410.root";
    const std::string ptWeightFwdPath =
        isPr
            ? "/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_pp_Jpsi_DATA_ctauCut_y1p6_2p4_260410.root"
            : "/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_pp_BtoJpsi_DATA_ctauCut_y1p6_2p4_260410.root";
    ...
    cout << "[INFO] pT reweight inputs (split-rapidity ratioDataMC mode): "
         << ptWeightMidPath << "::" << ptWeightMidName << ", "
         << ptWeightFwdPath << "::" << ptWeightFwdName << "\n";
  }
  ...
  const std::string weightLabel = Form("_ncollW1_genW%d_ptW%d_tnpW%d_ctauW%d",
                                       isGenW, isPtW, isTnPW, applyCtauCut);
  ...
  const std::vector<double> ctauPtEdgesMid = {6.5, 9, 12, 15, 20, 25, 40};
  const std::vector<double> ctauPtEdgesFwd = {3.5, 6.5, 9, 12, 40};
  std::vector<double> ctauCutsFwd;
  std::vector<double> ctauCutsMid;
  if (applyCtauCut)
  {
    const TString ctauLabel = isPr ? "PR" : "NP";
    const TString ctauFilePath = Form("%s/roots_1S_pp/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root",
                                      resolveBaseDir().Data(), ctauLabel.Data());
    if (!BuildPtBinnedCtauCuts(ctauFilePath, ctauCutsFwd, ctauCutsMid))
      return;
  }
  ...
  for (long iev = 0; iev < nEvt; ++iev)
  {
    ...
    for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
    {
      ...
      if (applyCtauCut)
      {
        const float ctau3DValue = Reco_QQ_ctau3D[irqq];
        double ctauCutValue = -1e9;
        bool hasCtauCut = false;
        if (absRecoY < 1.6)
          hasCtauCut = FindPtBinnedCtauCut(pt_, ctauPtEdgesMid, ctauCutsMid, ctauCutValue);
        else if (absRecoY < 2.4)
          hasCtauCut = FindPtBinnedCtauCut(pt_, ctauPtEdgesFwd, ctauCutsFwd, ctauCutValue);
