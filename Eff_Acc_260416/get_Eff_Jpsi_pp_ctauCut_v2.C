...
void get_Eff_Jpsi_pp_ctauCut_v2(
  int state=1,
  ...
  if(state==1){
    ...
    fPtW2 = new TFile("../compareDataToMC/ratioDataMC_pp_Jpsi_DATA_ctauCut_y1p6_2p4_251125.root", "read");
  }
  else if(state==2){
    fPtW1 = new TFile("../compareDataToMC/ratioDataMC_pp_BtoJpsi_DATA_ctauCut_y0_1p6_251103.root", "read");
    fPtW2 = new TFile("../compareDataToMC/ratioDataMC_pp_BtoJpsi_DATA_ctauCut_y1p6_2p4_251125.root", "read");
  }
  ...
  for(int i=0; i<5; ++i){
    double ptLow_tmp = ptBin_for[i];
    double ptHigh_tmp = ptBin_for[i+1];
    TString fpath_tmp = Form("roots_1S_pp/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data());
  ...
  for(int i=0; i<7; ++i){
    double ptLow_tmp = ptBin_mid[i];
    double ptHigh_tmp = ptBin_mid[i+1];
    TString fpath_tmp = Form("roots_1S_pp/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data());
    TFile *f_ctau_tmp = TFile::Open(fpath_tmp, "READ");
    if(f_ctau_tmp && !f_ctau_tmp->IsZombie()){
...
//draw same
  ...
  //if (state == 2)
  TString  outFileName;
  if(state==1) outFileName = Form("./roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtW%s_tnp%d_ctauCut_260219.root", ptSys.Data(), isTnP);
  else if(state==2) outFileName = Form("./roots/mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtW%s_tnp%d_ctauCut_260219.root", ptSys.Data(), isTnP);
