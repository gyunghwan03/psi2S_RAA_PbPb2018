...
const TString kOutDir = "plot_outputs_eff_ptw_compare_jpsi_pp";
...
void plot_efficiency_ptweight_compare_jpsi_pp(int state = 1) {
  gStyle->SetOptStat(0);
  EnsureOutDir();

  const bool isPrompt = (state == 1);
  const TString sample = isPrompt ? "Prompt J/#psi pp" : "Nonprompt J/#psi pp";

  const TString filePtW = isPrompt
                              ? "./roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtWnomi_tnp1_ctauCut_260416_2exp.root"
                              : "./roots/mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtWnomi_tnp1_ctauCut_260416_2exp.root";

  const TString fileNoPtW = isPrompt
                                ? "./roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtWnomi_tnp1_ctauCut_260416_2exp_noPtW.root"
                                : "./roots/mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtWnomi_tnp1_ctauCut_260416_2exp_noPtW.root";
...
void run_ptweight_compare_jpsi_pp_all() {
  plot_efficiency_ptweight_compare_jpsi_pp(1);
  plot_efficiency_ptweight_compare_jpsi_pp(2);
}

void plot_efficiency_ptweight_compare_jpsi_pp(const char* mode) {
  TString opt = mode ? mode : "";
  opt.ToLower();
  opt.ReplaceAll(" ", "");

  if (opt.IsNull() || opt == "all" || opt.Contains("run_ptweight_compare_jpsi_pp_all")) {
    run_ptweight_compare_jpsi_pp_all();
    return;
  }
