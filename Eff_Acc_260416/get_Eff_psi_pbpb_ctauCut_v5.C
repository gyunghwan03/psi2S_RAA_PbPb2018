...
static double EvalPtWeight(bool isPtWeight, double rap, double pt, TF1* fptwMid, TF1* fptwFor)
{
  ...
  if(absRap >= 1.6 && absRap < 2.4 && pt < 6.5 && fptwFor) return fptwFor->Eval(pt);
...
void get_Eff_psi_pbpb_ctauCut_v5(
  int state=1, bool isPtWeight = true,
...
// v5 : prompt/nonprompt are handled by state (1: prompt, 2: nonprompt)
  ...
  if(!fptwMid) cout << "[PtW] WARNING: missing dataMC_Ratio1 for |y|<2.4 file." << endl;
  if(!fptwFor) cout << "[PtW] WARNING: missing dataMC_Ratio1 for 1.6<|y|<2.4 file." << endl;
  ...
  TH1D* hpt_reco_1_noW = new TH1D("hpt_reco_1_noW","hpt_reco_1_noW",nPtBinsFor,ptBin_for);
  TH1D* hpt_reco_2_noW = new TH1D("hpt_reco_2_noW","hpt_reco_2_noW",nPtBinsMid,ptBin_mid);

  TH1D* hpt_gen_1_noW = new TH1D("hpt_gen_1_noW","hpt_gen_1_noW",nPtBinsFor,ptBin_for);
  TH1D* hpt_gen_2_noW = new TH1D("hpt_gen_2_noW","hpt_gen_2_noW",nPtBinsMid,ptBin_mid);

  TH1D* hInt_gen_1_noW = new TH1D("hInt_gen_1_noW", "",1,0,40);
  TH1D* hInt_gen_2_noW = new TH1D("hInt_gen_2_noW", "",1,0,40);

  TH1D* hInt_reco_1_noW = new TH1D("hInt_reco_1_noW", "",1,0,40);
  TH1D* hInt_reco_2_noW = new TH1D("hInt_reco_2_noW", "",1,0,40);
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
      ...
      if(HLTPass==true && HLTFilterPass==true){
          ...
          if(Rapidity > 1.6 && Rapidity <2.4 && JP_Reco->Pt() > 3.5 && JP_Reco->Pt() < 40){
            ...
            if(Centrality > 0 && Centrality < 180){
              ...
              if(PassCtauCut(state, Reco_QQ_ctau3D[irqq], l_cut)) { hInt_reco_1->Fill(1, weight*tnp_weight*pt_weight); hInt_reco_1_noW->Fill(1, 1.0); }
            }
          }
          if(Rapidity < 1.6 && JP_Reco->Pt() > 6.5 && JP_Reco->Pt() < 40){
            ...
            if(Centrality > 0 && Centrality < 180){
              ...
              if(PassCtauCut(state, Reco_QQ_ctau3D[irqq], l_cut)) { hInt_reco_2->Fill(1, weight*tnp_weight*pt_weight); hInt_reco_2_noW->Fill(1, 1.0); }
            }
          }
  ...
  hpt_eff_1_noW->Divide(hpt_eff_1_noW, hpt_gen_1_noW, 1, 1, "B");
  hpt_eff_2_noW->Divide(hpt_eff_2_noW, hpt_gen_2_noW, 1, 1, "B");
  ...
  hInt_eff_1_noW->Divide(hInt_eff_1_noW, hInt_gen_1_noW, 1, 1, "B");
  hInt_eff_2_noW->Divide(hInt_eff_2_noW, hInt_gen_2_noW, 1, 1, "B");
