...
void get_Eff_Jpsi_pp_ctauCut_v5(
  ...
  if(state==1){
    fPtWMid = TFile::Open("/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_pp_Jpsi_DATA_ctauCut_y0_2p4_260410.root","READ");
    if(!fPtWMid || fPtWMid->IsZombie()){
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_Jpsi_DATA_ctauCut_y0_2p4_260310.root","READ");
    }
    fPtWFor = TFile::Open("/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_pp_Jpsi_DATA_ctauCut_y1p6_2p4_260410.root","READ");
    if(!fPtWFor || fPtWFor->IsZombie()){
      fPtWFor = TFile::Open("../compareDataToMC/ratioDataMC_pp_Jpsi_DATA_ctauCut_y1p6_2p4_251103.root","READ");
    }
  }
  else if(state==2){
    fPtWMid = TFile::Open("/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_pp_BtoJpsi_DATA_ctauCut_y0_2p4_260410.root","READ");
    if(!fPtWMid || fPtWMid->IsZombie()){
      fPtWMid = TFile::Open("../compareDataToMC/ratioDataMC_pp_BtoJpsi_DATA_ctauCut_y0_2p4_260310.root","READ");
    }
    fPtWFor = TFile::Open("/data/users/hanseul/jpsi_2603/compareDataToMC/roots/ratioDataMC_pp_BtoJpsi_DATA_ctauCut_y1p6_2p4_260410.root","READ");
    if(!fPtWFor || fPtWFor->IsZombie()){
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
  ...
  //for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
    {
	  ...
	  counts[0]++;
      if ((Reco_QQ_trig[irqq] & kTrigSelMask) == kTrigSelMask)
        HLTFilterPass = true;
...
//draw same
  ...
  hpt_eff_2 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy0_1p6",isTnP, 1));
  hInt_eff_1 ->SetName(Form("mc_eff_Integrated_TnP%d_PtW%d_absy1p6_2p4",isTnP, 1));
  hInt_eff_2 ->SetName(Form("mc_eff_Integrated_TnP%d_PtW%d_absy0_1p6",isTnP, 1));
  ...
  //if (state == 2)
  TString  outFileName;
  if(state==1 && applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtW%s_tnp%d_ctauCut_260420_2exp.root", ptSys.Data(), isTnP);
  else if(state==1 && !applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtW%s_tnp%d_ctauCut_260420_2exp_noPtW.root", ptSys.Data(), isTnP);
  else if(state==2 && applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtW%s_tnp%d_ctauCut_260420_2exp.root", ptSys.Data(), isTnP);
  else if(state==2 && !applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtW%s_tnp%d_ctauCut_260420_2exp_noPtW.root", ptSys.Data(), isTnP);
