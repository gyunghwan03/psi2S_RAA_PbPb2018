...
void syst_2S_pb(int cLow = 120, int cHigh = 180,
    ...
    float SiMuPtCut = 0, float massLow = 3.3, float massHigh =4.1, int PRMC=0, bool dimusign=true, bool fAccW = false, bool fEffW = false)
{
  ...
  gSystem->mkdir("./syst/roots_2S_Pb/");
  gSystem->mkdir("./syst/roots_2S_Pb/decayL");
  gSystem->mkdir("./syst/roots_2S_Pb/decayL/PRMC");
  gSystem->mkdir("./syst/roots_2S_Pb/decayL/NPMC");

  gSystem->mkdir("./syst/figs_2S_Pb/");
  gSystem->mkdir("./syst/figs_2S_Pb/decayL");
  gSystem->mkdir("./syst/figs_2S_Pb/decayL/PRMC");
  gSystem->mkdir("./syst/figs_2S_Pb/decayL/NPMC");
  ...
  TFile *fPRnomi = new TFile(Form("./roots_2S_Pb/decayL/PRMC/decay_hist_%s.root",kineLabel.Data()));
  ...
  // PRMC==0일 때 가장 가까운 bin에서 라인 생성
  if(PRMC==0 && best_bin >= 0) {
    cout<<best_bin<<"th bin"<<", ljpsi: "<<h_decayPRMC->GetBinCenter(best_bin)<<", current res: "<<(h_decayNPMC->Integral(0,best_bin)/h_decayNPMC->GetEntries())*100<<"%, target: "<<target_res_value*100<<"%, diff: "<<best_diff*100<<"%"<<endl;
  ...
  // 라인이 생성되지 않았을 경우 기본값 설정
  if(lcutv == nullptr) {
    cout << "Warning: No suitable bin found, using default values" << endl;
    lcutv = new TLine(0.0, 0, 0.0, 1);
    lcuth = new TLine(xmin, 0.0, 0.0, 0.0);
    lresi = new TLine(xmin, 0.0, 0.0, 0.0);
  }
  ...
  c_mass->Update();
  if(PRMC==0){
  c_mass->SaveAs(Form("./syst/figs_2S_Pb/decayL/PRMC/mass_%s.pdf", kineLabel.Data()));}
  else if(PRMC==1){
  c_mass->SaveAs(Form("./syst/figs_2S_Pb/decayL/NPMC/mass_%s.pdf", kineLabel.Data()));}
  ...
  if(PRMC==0){
    ...
    c_decayL->Update();
    c_decayL->SaveAs(Form("./syst/figs_2S_Pb/decayL/PRMC/decay_%s.pdf",kineLabel.Data()));
    TFile *wf = new TFile(Form("./syst/roots_2S_Pb/decayL/PRMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
    wf->cd();}
    else if(PRMC==1){
      drawText(Form("L_{J/psi} cut: %.4f", lcutv->GetX1()),pos_x_mass,pos_y-pos_y_diff*3,text_color,text_size);
      ...
      c_decayL->Update();
      c_decayL->SaveAs(Form("./syst/figs_2S_Pb/decayL/NPMC/decay_%s.pdf",kineLabel.Data()));
      TFile *wf = new TFile(Form("./syst/roots_2S_Pb/decayL/NPMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
      wf->cd();}
