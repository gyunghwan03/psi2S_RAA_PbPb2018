...
void syst_2S_pp(float ptLow =  3, float ptHigh = 30, 
    float yLow = 1.6, float yHigh = 2.4,
    float SiMuPtCut = 0, float massLow = 3.3, float massHigh =4.1, int PRMC=0, bool dimusign=true, bool fAccW = false, bool fEffW = false)
{
  ...
  gSystem->mkdir("./syst/roots_2S_pp/");
  gSystem->mkdir("./syst/roots_2S_pp/decayL");
  gSystem->mkdir("./syst/roots_2S_pp/decayL/PRMC");
  gSystem->mkdir("./syst/roots_2S_pp/decayL/NPMC");

  gSystem->mkdir("./syst/figs_2S_pp/");
  gSystem->mkdir("./syst/figs_2S_pp/decayL");
  gSystem->mkdir("./syst/figs_2S_pp/decayL/PRMC");
  gSystem->mkdir("./syst/figs_2S_pp/decayL/NPMC");
  ...
  TFile *fPRnomi = new TFile(Form("./roots_2S_pp/decayL/PRMC/decay_hist_%s.root",kineLabel.Data()));
  if (!fPRnomi || fPRnomi->IsZombie()) {
    cout << "Error: Cannot open nominal file for " << kineLabel.Data() << endl;
    return;
  }
  TH1D *h_res_nomi = (TH1D*)fPRnomi->Get("h_res");
  if (!h_res_nomi) {
    cout << "Error: Cannot find h_res histogram in nominal file" << endl;
    return;
  }
  double target_res_value = 2.0 * h_res_nomi->GetBinContent(1);  // 2배 값을 target으로 설정
  cout << "Nominal res value: " << h_res_nomi->GetBinContent(1) << ", Target (2x): " << target_res_value << endl;
...
for(int bins=0; bins<h_decayPRMC->GetNbinsX(); bins++){
    if(PRMC==0){
      ...
      double current_res_value = h_decayNPMC->Integral(0,bins)/h_decayNPMC->GetEntries();
      
      // target 값에 가장 가까운 지점을 찾기
      if(fabs(current_res_value - target_res_value) < 0.001) {  // 1% 오차 범위
        cout<<bins<<"th bin"<<", ljpsi: "<<h_decayPRMC->GetBinCenter(bins)<<", current res: "<<current_res_value*100<<"%, target: "<<target_res_value*100<<"%"<<endl;
      ...
      if(h_decayPRMC->Integral(0,bins)<(0.95*totalPRMC)||h_decayPRMC->Integral(0,bins)>(totalPRMC*0.96)) continue;
  ...
  // target을 찾지 못한 경우 처리
  if (!found_target && PRMC==0) {
    cout << "Warning: Could not find exact target value. Using closest approximation." << endl;
    // 가장 가까운 값을 찾기
    double min_diff = 1e9;
    int best_bin = -1;
    for(int bins=0; bins<h_decayPRMC->GetNbinsX(); bins++){
      double current_res_value = h_decayNPMC->Integral(0,bins)/h_decayNPMC->GetEntries();
      double diff = fabs(current_res_value - target_res_value);
      if(diff < min_diff) {
        min_diff = diff;
        best_bin = bins;
      }
    ...
    if(best_bin >= 0) {
      double current_res_value = h_decayNPMC->Integral(0,best_bin)/h_decayNPMC->GetEntries();
      cout<<best_bin<<"th bin (closest)"<<", ljpsi: "<<h_decayPRMC->GetBinCenter(best_bin)<<", current res: "<<current_res_value*100<<"%, target: "<<target_res_value*100<<"%"<<endl;
      lcutv = new TLine(h_decayPRMC->GetBinCenter(best_bin), 0, h_decayPRMC->GetBinCenter(best_bin), 1);
      lcuth = new TLine(xmin, h_decayPRMC->Integral(0,best_bin)/h_decayPRMC->GetEntries(), h_decayPRMC->GetBinCenter(best_bin), h_decayPRMC->Integral(0,best_bin)/h_decayPRMC->GetEntries());
      lresi = new TLine(xmin, h_decayNPMC->Integral(0,best_bin)/h_decayNPMC->GetEntries(), h_decayNPMC->GetBinCenter(best_bin), h_decayNPMC->Integral(0,best_bin)/h_decayNPMC->GetEntries()); 
    }
  }

  if (!lcutv) {
    cout << "Error: Could not determine cut line. Exiting." << endl;
    return;
  }
  ...
  c_mass->Update();
  if(PRMC==0){
  c_mass->SaveAs(Form("syst/figs_2S_pp/decayL/PRMC/mass_%s.pdf", kineLabel.Data()));}
  else if(PRMC==1){
  c_mass->SaveAs(Form("syst/figs_2S_pp/decayL/NPMC/mass_%s.pdf", kineLabel.Data()));}
  ...
  if(PRMC==0){
    ...
    c_decayL->Update();
    c_decayL->SaveAs(Form("syst/figs_2S_pp/decayL/PRMC/decay_%s.pdf",kineLabel.Data()));
    TFile *wf = new TFile(Form("syst/roots_2S_pp/decayL/PRMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
    wf->cd();}
    else if(PRMC==1){
      drawText(Form("L_{J/psi} cut: %.4f", lcutv->GetX1()),pos_x_mass,pos_y-pos_y_diff*3,text_color,text_size);
      ...
      c_decayL->Update();
      c_decayL->SaveAs(Form("syst/figs_2S_pp/decayL/NPMC/decay_%s.pdf",kineLabel.Data()));
      TFile *wf = new TFile(Form("syst/roots_2S_pp/decayL/NPMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
      wf->cd();}
