...
void syst_1S_pp(float ptLow =  3, float ptHigh = 30, 
    ...
    float SiMuPtCut = 0, float massLow = 3.3, float massHigh =4.1, int PRMC=0, bool dimusign=true, bool fAccW = false, bool fEffW = false)
{
  ...
  gSystem->mkdir("./syst/roots_1S_pp/");
  gSystem->mkdir("./syst/roots_1S_pp/decayL");
  gSystem->mkdir("./syst/roots_1S_pp/decayL/PRMC");
  gSystem->mkdir("./syst/roots_1S_pp/decayL/NPMC");

  gSystem->mkdir("./syst/figs_1S_pp/");
  gSystem->mkdir("./syst/figs_1S_pp/decayL");
  gSystem->mkdir("./syst/figs_1S_pp/decayL/PRMC");
  gSystem->mkdir("./syst/figs_1S_pp/decayL/NPMC");
  ...
  //fPRMC  = new TFile("../skimmedFiles/OniaFlowSkim_JpsiTrig_Prompt_pp_Psi1S_isMC1_230717.root");
  ...
  TFile *fPRnomi = new TFile(Form("./roots_1S_pp/decayL/PRMC/decay_hist_%s.root",kineLabel.Data()));
...
for(int bins=0; bins<h_decayPRMC->GetNbinsX(); bins++){
  ...
  c_mass->Update();
  if(PRMC==0){
  c_mass->SaveAs(Form("syst/figs_1S_pp/decayL/PRMC/mass_%s.pdf", kineLabel.Data()));}
  else if(PRMC==1){
  c_mass->SaveAs(Form("syst/figs_1S_pp/decayL/NPMC/mass_%s.pdf", kineLabel.Data()));}
  ...
  if(PRMC==0){
    ...
    c_decayL->Update();
    c_decayL->SaveAs(Form("syst/figs_1S_pp/decayL/PRMC/decay_%s.pdf",kineLabel.Data()));
    TFile *wf = new TFile(Form("syst/roots_1S_pp/decayL/PRMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
    wf->cd();}
    else if(PRMC==1){
      drawText(Form("L_{J/psi} cut: %.4f", lcutv->GetX1()),pos_x_mass,pos_y-pos_y_diff*3,text_color,text_size);
      ...
      c_decayL->Update();
      c_decayL->SaveAs(Form("syst/figs_1S_pp/decayL/NPMC/decay_%s.pdf",kineLabel.Data()));
      TFile *wf = new TFile(Form("syst/roots_1S_pp/decayL/NPMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
      wf->cd();}
