...
void dndpt_y0_1p6_Jpsi_pp_noCtau(int PR=0, int WRITE=1) {
  ...
  if(WRITE==1&&PR==0){
    TFile *fJpsipb = new TFile("./ratioDataMC_pp_Jpsi_DATA_noCtau_y0_1p6_260514.root","RECREATE");
    fJpsipb->cd();
    ...
    TFile *fJpsipb = new TFile("./ratioDataMC_pp_BtoJpsi_DATA_noCtau_y0_1p6_260514.root","RECREATE");
    fJpsipb->cd();
    hptData1->SetName("WeightFactor");
    hptData1->Write();
    fitRatio1->SetName("dataMC_Ratio1");
  ...
  if(WRITE==1){
    c_A->SaveAs(Form("./dNdpt_plot_%s_Jpsi_pp_y0_1p6_noCtau_260514.pdf",fname.Data()));
    c_A->SaveAs(Form("./dNdpt_plot_%s_Jpsi_pp_y0_1p6_noCtau_260514.png",fname.Data()));
  }
}
