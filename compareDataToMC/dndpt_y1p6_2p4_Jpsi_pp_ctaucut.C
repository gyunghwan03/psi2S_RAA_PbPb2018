...
void dndpt_y1p6_2p4_Jpsi_pp_ctaucut(int PR=0, int WRITE=1) {
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  fitRatio1 = new TF1("fitRatio1","( [0] + [1]*x + [2]*x*x +[4]*x*x*x ) / (  (x-[3])*(x-[3])*(x-[3])  )",1,40);
  ...
  if(WRITE==1&&PR==0){
	  TFile *fJpsipb = new TFile("./ratioDataMC_pp_Jpsi_DATA_ctauCut_y1p6_2p4_251103.root","RECREATE");
	  fJpsipb->cd();
	  ...
	  TFile *fJpsipb = new TFile("./ratioDataMC_pp_BtoJpsi_DATA_ctauCut_y1p6_2p4_251103.root","RECREATE");
	  fJpsipb->cd();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
	  fitRatio1->SetName("dataMC_Ratio1");
  ...
  if(WRITE==1){
	  c_A->SaveAs(Form("./dNdpt_plot_%s_Jpsi_pp_y1p6_2p4_ctauCut_251103.pdf",fname.Data()));
	  c_A->SaveAs(Form("./dNdpt_plot_%s_Jpsi_pp_y1p6_2p4_ctauCut_251103.png",fname.Data()));
  }
}
...
valErr getYield(int isPR, float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh) {
  ...
  TFile* inf = new TFile(Form("../Macros/Jpsi_L_cut/roots_1S_pp/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel.Data()));
