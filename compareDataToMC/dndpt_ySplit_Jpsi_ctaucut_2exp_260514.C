...
void dndpt_ySplit_Jpsi_ctaucut_2exp_260514(int PR=0, int rapMode=0, int WRITE=1) {

  TString fname;
  if(PR==0) fname = "Prompt";
  else if(PR==1) fname = "NonPrompt";
  const bool isFwd = (rapMode != 0);
  const double yLowTarget = isFwd ? 1.6 : 0.0;
  const double yHighTarget = isFwd ? 2.4 : 1.6;
  const TString rapTag = isFwd ? "y1p6_2p4" : "y0_1p6";
  const TString rapLabel = isFwd ? "1.6 < |y| < 2.4" : "|y| < 1.6";
  gStyle->SetOptStat(0);
  //gStyle->SetOptFit(1);
  ...
  const int nPtBins = isFwd ? 8 : 10;
  const double ptBinFwd[9]={3.0,4.0,5.0,6.5,8.5,12.0,15.0,20.0,40.0};
  const double ptBinMid[11]={6.5,7.5,8.5,9.5,11,13,15,17.5,20,25,40};
  double ptBin[13];
  for (int i=0; i<=nPtBins; ++i) ptBin[i] = isFwd ? ptBinFwd[i] : ptBinMid[i];
  ...
  double l_cut[12] = {-10};
  double ctauEff_PR[13];
  double ctauEff_NP[13];
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  for(int ipt=1;ipt<=nPtBins;ipt++)
  {
    ...
    valErr ctauEff;
    if(PR==0){
    yieldAA = getYield(0,ptBin[ipt-1],ptBin[ipt],yLowTarget,yHighTarget,0,180);
    ctauEff = getCtauEff(0,ptBin[ipt-1],ptBin[ipt],yLowTarget,yHighTarget,0,2.6,3.5);
    ...
    cout << Form("yield, pt  ") << ptBin[ipt-1] << " - " << ptBin[ipt] << " : " << yieldAA.val <<" +/- "<< yieldAA.err 
      << ", error : " << 100*yieldAA.err/yieldAA.val <<"%"<<endl;
    
    hptData->SetBinContent(ipt,yieldAA.val*(1/ctauEff_PR[ipt]));
    hptData->SetBinError(ipt,yieldAA.err);
    hptData1->SetBinContent(ipt,yieldAA.val*(1/ctauEff_PR[ipt]));
    hptData1->SetBinError(ipt,yieldAA.err);
    }
    if(PR==1){
    yieldAA = getYield(1,ptBin[ipt-1],ptBin[ipt],yLowTarget,yHighTarget,0,180);
    ctauEff = getCtauEff(1,ptBin[ipt-1],ptBin[ipt],yLowTarget,yHighTarget,0,2.6,3.5);
    ctauEff_NP[ipt]=ctauEff.val;
    cout << "ctauEff_NP for pt "<< ptBin[ipt-1]<<"-"<<ptBin[ipt]<<": "<< ctauEff.val << endl;
    cout << Form("yield, pt  ") << ptBin[ipt-1] << " - " << ptBin[ipt] << " : " << yieldAA.val <<" +/- "<< yieldAA.err 
      << ", error : " << 100*yieldAA.err/yieldAA.val <<"%"<<endl;
    hptData->SetBinContent(ipt,yieldAA.val*(1/ctauEff_NP[ipt]));
    hptData->SetBinError(ipt,yieldAA.err);
    hptData1->SetBinContent(ipt,yieldAA.val*(1/ctauEff_NP[ipt]));
    hptData1->SetBinError(ipt,yieldAA.err);
    }
  ...
  int textSize = 18;
  drawText(Form("%s J/#psi in PbPb, %s", fname.Data(), rapLabel.Data()), posX, posY, kBlack, textSize);
  ...
  const double fitLow = isFwd ? 3.0 : 6.5;
  hptData1->Fit(fitRatio1,"IE","",fitLow,12);
  TFitResultPtr r = hptData1->Fit(fitRatio1,"S","",fitLow,40);
  ...
  if(WRITE==1&&PR==0){
    fJpsipb = new TFile(Form("./ratioDataMC_AA_Jpsi_DATA_ctauCut_%s_260514_2exp.root", rapTag.Data()),"RECREATE");
	  fJpsipb->cd();
    ...
    fJpsipb = new TFile(Form("./ratioDataMC_AA_BtoJpsi_DATA_ctauCut_%s_260514_2exp.root", rapTag.Data()),"RECREATE");
	  fJpsipb->cd();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
	  fitRatio1->SetName("dataMC_Ratio1");
  ...
  if(WRITE==1){
    c_A->SaveAs(Form("./figs/dNdpt_plot_AA_%s_Jpsi_%s_260514_2exp.pdf", fname.Data(), rapTag.Data()));
    c_A->SaveAs(Form("./figs/dNdpt_plot_AA_%s_Jpsi_%s_260514_2exp.png", fname.Data(), rapTag.Data()));
  }
  if(fJpsipb) cout << "File saved : " << fJpsipb->GetName() << endl;
}
...
valErr getYield(int isPR, float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh) {
  ...
  TFile* inf = new TFile(Form("../Macros/Jpsi_L_cut_pTreweight/roots_1S_Pb/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel.Data()));
...
valErr getCtauEff(int isPR, float ptLow, float ptHigh, float yLow, float yHigh, float SiMuPtCut, float massLow, float massHigh) {
  ...
  TFile* inf = new TFile(Form("../Eff_Acc_pTreweight/roots_1S_PbPb_pTreweight/ctau3D_cut_%s_pt%.1f-%.1f_y%.1f-%.1f.root", PR.Data(), ptLow, ptHigh, yLow, yHigh));
  valErr ret; ret.val = 0; ret.err = 0;
  TH1D* hEff = nullptr;
  if (inf && !inf->IsZombie()) hEff = (TH1D*)inf->Get(Form("%s", hPR.Data()));
  if (!hEff) {
      if (inf) inf->Close();
      inf = new TFile(Form("../ctau_eff/roots_1S_Pb/decayL/%s/decay_hist_%s_m2.6-3.5.root", PR.Data(), kineLabel.Data()));
      if (!inf || inf->IsZombie()) {
          cout << "파일을 열 수 없습니다: legacy ctau " << kineLabel << endl;
          return ret;
      }
      hEff = (TH1D*)inf->Get("h_eff");
      if (!hEff) {
