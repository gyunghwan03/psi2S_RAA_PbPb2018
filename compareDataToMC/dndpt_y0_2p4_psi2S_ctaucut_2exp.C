...
void dndpt_y0_2p4_psi2S_ctaucut_2exp(int PR=0, int WRITE=1) {
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  hptData1->Fit(fitRatio1,"IE","",6.5,40);
  TFitResultPtr r = hptData1->Fit(fitRatio1,"S","",6.5,40);
  ...
  if(WRITE==1&&PR==0){
	  fJpsipb = new TFile("./ratioDataMC_AA_psi2S_DATA_ctauCut_y0_2p4_260323_2exp.root","RECREATE");
	  fJpsipb->cd();
    ...
    c_A->Write();
	  ...
	  fJpsipb = new TFile("./ratioDataMC_AA_BtoPsi2S_DATA_ctauCut_y0_2p4_260323_2exp.root","RECREATE");
	  fJpsipb->cd();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
    if(fitRatioOnHist) fitRatioOnHist->Write("dataMC_Ratio1");
  ...
  if(WRITE==1){
    if(fJpsipb){
	  ...
	  c_A->SaveAs(Form("./figs/dNdpt_plot_AA_psi2S_%s_y0_2p4_260323_2exp.pdf",fname.Data()));
	  c_A->SaveAs(Form("./figs/dNdpt_plot_AA_psi2S_%s_y0_2p4_260323_2exp.png",fname.Data()));
