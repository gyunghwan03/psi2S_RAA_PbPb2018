...
void dndpt_y1p6_2p4_psi2S_pp_ctaucut_260323_2exp(int PR=0, int WRITE=1) {
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  for(int ipt=1;ipt<=nPtBins;ipt++)
  {
    ...
    valErr ctauEff;
    if(PR==0){
    yieldAA = getYield(0,ptBin[ipt-1],ptBin[ipt],1.6,2.4);
    ctauEff = getCtauEff(0,ptBin[ipt-1],ptBin[ipt],1.6,2.4,0,massLow,massHigh);
    ...
    if(PR==1){
    yieldAA = getYield(1,ptBin[ipt-1],ptBin[ipt],1.6,2.4);
    ctauEff = getCtauEff(1,ptBin[ipt-1],ptBin[ipt],1.6,2.4,0,massLow,massHigh);
  ...
  //fitRatio1 = new TF1("fitRatio1","( [0] + [1]*x + [2]*x*x +[4]*x*x*x ) / (  (x-[3])*(x-[3])*(x-[3])  )",0,12);
  fitRatio1 = new TF1("fitRatio1","[0]*TMath::Exp(-[1]*x) + [2]*TMath::Exp(-[3]*x) + [4]",3,12);
  ...
  TFile *fJpsipb;
  if(WRITE==1&&PR==0){
	  fJpsipb = new TFile("./ratioDataMC_pp_psi2S_DATA_ctauCut_y1p6_2p4_260323_2exp.root","RECREATE");
	  fJpsipb->cd();
    ...
    c_A->Write();
	  ...
	  fJpsipb = new TFile("./ratioDataMC_pp_Btopsi2S_DATA_ctauCut_y1p6_2p4_260323_2exp.root","RECREATE");
	  fJpsipb->cd();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
	  fitRatio1->SetName("dataMC_Ratio1");
  ...
  if(WRITE==1){
	  c_A->SaveAs(Form("./dNdpt_plot_pp_%s_psi2S_y1p6_2p4_260323_2exp.pdf",fname.Data()));
	  c_A->SaveAs(Form("./dNdpt_plot_pp_%s_psi2S_y1p6_2p4_260323_2exp.png",fname.Data()));
