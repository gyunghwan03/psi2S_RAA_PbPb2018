...
void dndpt_y1p6_2p4_psi2S_pp_ctaucut(int PR=0, int WRITE=1) {
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  if(WRITE==1&&PR==0){
	  TFile *fJpsipb = new TFile("./ratioDataMC_pp_psi2S_DATA_ctauCut_y1p6_2p4_251103.root","RECREATE");
	  fJpsipb->cd();
	  ...
	  TFile *fJpsipb = new TFile("./ratioDataMC_pp_BtoPsi2S_DATA_ctauCut_y1p6_2p4_251103.root","RECREATE");
	  fJpsipb->cd();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
	  fitRatio1->SetName("dataMC_Ratio1");
  ...
  if(WRITE==1){
	  c_A->SaveAs(Form("./dNdpt_plot_pp_%s_psi2S_y1p6_2p4.pdf",fname.Data()));
	  c_A->SaveAs(Form("./dNdpt_plot_pp_%s_psi2S_y1p6_2p4.png",fname.Data()));
  }
}
...
valErr getYield(int isPR, float ptLow, float ptHigh, float yLow, float yHigh) {
  ...
  TFile* inf = new TFile(Form("../Macros/psi2S_L_cut_250427/roots_2S_pp/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel.Data()));
...
valErr getCtauEff(int isPR, float ptLow, float ptHigh, float yLow, float yHigh, float SiMuPtCut, float massLow, float massHigh) {
  TString kineLabel = getKineLabelpp(ptLow, ptHigh, yLow, yHigh, 0.0);
   TString PR;
    ...
    TFile* inf = new TFile(Form("../ctau_eff/roots_2S_pp/decayL/%s/decay_hist_%s_m3.3-4.1.root", PR.Data(), kineLabel.Data()));
    valErr ret; ret.val = 0; ret.err = 0;
    if (!inf || inf->IsZombie()) {
        cout << "파일을 열 수 없습니다: " << inf->GetName() << endl;
        return ret;
