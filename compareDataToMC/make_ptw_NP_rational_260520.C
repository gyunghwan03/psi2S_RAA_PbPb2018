// =============================================================
//  make_ptw_NP_rational_260520.C
//
//  Re-fit the nonprompt (B->J/psi) PtW with the same aggregated
//  high-pT binning as option 2 BUT using the rational fit form
//  (A_nominal_rational style). The 2-exp NP fit was too flat at
//  high pT, leaving the integrated AA NP eff weighted too heavily
//  toward low pT (lower-eff) bins and pushing NP RAA below
//  HIN-16-025 by ~13%. The rational form has more curvature at
//  high pT.
//
//  Output: compareDataToMC/ptw_candidates_260520_v3/H_NP_rational/
//          ratioDataMC_{pp,AA}_BtoJpsi_DATA_*_260515.root  (4 files)
//
//  The four PR (Jpsi) files for this candidate are *symlinked* from
//  the option-2 G_aggregateHighPt_2exp dir (PR fits remain unchanged).
// =============================================================
...
namespace PtWNPRational260520
{
constexpr const char *kRepo = "/data/hwan/psi2S_RAA_PbPb2018";
constexpr const char *kOutBase = "ptw_candidates_260520_v3";
constexpr const char *kCandTag = "H_NP_rational";
...
void ProcessNP(bool isPbPb, bool fwd, McSample &mc, const TString &outDir)
{
  LoadMc(mc);
  ...
  // Aggregated edges: merge last 2 native bins
  std::vector<double> agg(nat.begin(), nat.end() - 2);
  agg.push_back(nat.back());
  const int nAgg = (int)agg.size() - 1;

  std::vector<double> aggY(nAgg, 0.0), aggE(nAgg, 0.0);
  for (int i = 0; i < (int)nat.size() - 1; ++i) {
    YieldErr ye; double w = 1.0;
    if (isPbPb) {
      ye = GetPbPbYield(nat[i], nat[i+1], yLow, yHigh);
      double frac = GetPbPbFrac(nat[i], nat[i+1], yLow, yHigh);
      w = frac;  // NP weight
    } else {
      ye = GetPpYield(false, nat[i], nat[i+1], yLow, yHigh);
    }
    if (!ye.ok) continue;
    int b = std::min(i, nAgg - 1);
    aggY[b] += ye.val * w;
    aggE[b] = std::sqrt(aggE[b]*aggE[b] + std::pow(ye.err * std::max(w, 1e-3), 2));
  }

  std::unique_ptr<TH1D> hData(new TH1D(Form("hData_%s_BtoJpsi_%s", sysLab.Data(), rapLab.Data()),
                                       "; p_{T} (GeV/c); ", nAgg, agg.data()));
  std::unique_ptr<TH1D> hDataR(new TH1D(Form("hDataR_%s_BtoJpsi_%s", sysLab.Data(), rapLab.Data()),
                                        "; p_{T} (GeV/c); ", nAgg, agg.data()));
  hData->SetDirectory(nullptr); hDataR->SetDirectory(nullptr);
  hData->Sumw2(); hDataR->Sumw2();
  for (int b = 1; b <= nAgg; ++b) {
    hData->SetBinContent(b, aggY[b-1]); hData->SetBinError(b, aggE[b-1]);
    hDataR->SetBinContent(b, aggY[b-1]); hDataR->SetBinError(b, aggE[b-1]);
  }
  if (hData->Integral() <= 0) return;

  std::unique_ptr<TH1D> hMc(new TH1D(Form("hMC_%s_BtoJpsi_%s", sysLab.Data(), rapLab.Data()),
                                     "; p_{T} (GeV/c); ", nAgg, agg.data()));
  std::unique_ptr<TH1D> hMcR(new TH1D(Form("hMCR_%s_BtoJpsi_%s", sysLab.Data(), rapLab.Data()),
                                      "; p_{T} (GeV/c); ", nAgg, agg.data()));
  hMc->SetDirectory(nullptr); hMcR->SetDirectory(nullptr);
  hMc->Sumw2(); hMcR->Sumw2();
  for (int ix = 1; ix <= mc.hFine->GetNbinsX(); ++ix) {
    ...
    hMc->Fill(x, w); hMcR->Fill(x, w);
  }
  if (hMc->Integral() <= 0) return;
  ...
  // Rational fit form with strict parameter constraints to keep PtW positive
  // for all pT >= 0:
  //   - d (pole) constrained strictly negative -> (x-d)^3 > 0 for any x>=0
  //   - a, b, c, e >= 0 -> numerator >= 0 for x>=0
  // This guarantees positivity and gives a smooth monotone decay typical of
  // the data/MC ratio (more aggressive than 2-exp).
  std::unique_ptr<TF1> fit(new TF1("fitRatio1",
                                   "( [0] + [1]*x + [2]*x*x + [4]*x*x*x ) / "
                                   "( (x-[3])*(x-[3])*(x-[3]) )",
                                   3.0, 40.0));
  fit->SetNpx(1000);
  fit->SetParameters(10.0, 1.0, 0.05, -1.0, 0.0);
  fit->SetParLimits(0, 0.0, 1e6);
  fit->SetParLimits(1, 0.0, 1e4);
  fit->SetParLimits(2, 0.0, 1e2);
  fit->SetParLimits(3, -50.0, -0.1);   // pole strictly negative
  fit->SetParLimits(4, 0.0, 10.0);
  hDataR->Fit(fit.get(), "IEQ", "", 3.0, 40.0);
  TFitResultPtr res = hDataR->Fit(fit.get(), "SQ", "", 3.0, 40.0);
  if (res.Get())
    std::cout << "[fit] H_NP_rational " << sysLab << " BtoJpsi " << rapLab
              << " chi2/ndf = " << res->Chi2() << " / " << res->Ndf() << "\n";

  TString outName;
  if (isPbPb)
    outName = Form("%s/ratioDataMC_AA_BtoJpsi_DATA_%s_260515.root", outDir.Data(), rapLab.Data());
  else
    outName = Form("%s/ratioDataMC_pp_BtoJpsi_DATA_noCtau_%s_260515.root", outDir.Data(), rapLab.Data());
  std::unique_ptr<TFile> out(TFile::Open(outName, "RECREATE"));
  if (!out || out->IsZombie()) return;
  out->cd();
  ...
  TNamed note("candidate_label", "H_NP_rational: NP (BtoJpsi) rational fit on aggregated bins");
  note.Write();
...
void make_ptw_NP_rational_260520()
{
  ...
  std::cout << "\n##### H_NP_rational: NP rational fit on aggregated bins #####\n";
  ProcessNP(false, false, mcPpNp, outDir);  // pp mid
  ProcessNP(false, true, mcPpNp, outDir);   // pp fwd
  ProcessNP(true, false, mcAaNp, outDir);   // PbPb mid
  ProcessNP(true, true, mcAaNp, outDir);    // PbPb fwd
}
