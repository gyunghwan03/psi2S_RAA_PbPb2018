...
void dndpt_y0_1p6_psi2S_pp_ctaucut(int PR=0, int WRITE=1) {
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  for(int ipt=1;ipt<=nPtBins;ipt++)
  {
    ...
    valErr ctauEff;
    if(PR==0){
    yieldAA = getYield(0,ptBin[ipt-1],ptBin[ipt],0,1.6);
    ctauEff = getCtauEff(0,ptBin[ipt-1],ptBin[ipt],0,1.6,0,massLow,massHigh);
    ...
    if(PR==1){
    yieldAA = getYield(1,ptBin[ipt-1],ptBin[ipt],0,1.6);
    ctauEff = getCtauEff(1,ptBin[ipt-1],ptBin[ipt],0,1.6,0,massLow,massHigh);
  ...
  if(WRITE==1&&PR==0){
	  TFile *fJpsipb = new TFile("./ratioDataMC_pp_psi2S_DATA_ctauCut_y0_1p6_251103.root","RECREATE");
	  fJpsipb->cd();
	  ...
	  TFile *fJpsipb = new TFile("./ratioDataMC_pp_Btopsi2S_DATA_ctauCut_y0_1p6_251103.root","RECREATE");
	  fJpsipb->cd();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
	  fitRatio1->SetName("dataMC_Ratio1");
  ...
  if(WRITE==1){
	  c_A->SaveAs(Form("./dNdpt_plot_pp_%s_psi2S_y0_1p6.pdf",fname.Data()));
	  c_A->SaveAs(Form("./dNdpt_plot_pp_%s_psi2S_y0_1p6.png",fname.Data()));
  }
}
