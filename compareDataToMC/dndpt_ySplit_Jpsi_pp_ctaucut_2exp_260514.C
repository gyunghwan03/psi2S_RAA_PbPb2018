...
void dndpt_ySplit_Jpsi_pp_ctaucut_2exp_260514(int PR=0, int rapMode=0, int WRITE=1) {
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  int textSize = 18;
  drawText(Form("%s J/#psi in pp, %s", fname.Data(), rapLabel.Data()), posX, posY, kBlack, textSize);
  ...
  hptData1->Fit(fitRatio1,"IE","",fitLow,40);
  ...
  if(WRITE==1&&PR==0){
    fJpsipb = new TFile(Form("./ratioDataMC_pp_Jpsi_DATA_ctauCut_%s_260514_2exp.root", rapTag.Data()),"RECREATE");
	  fJpsipb->cd();
    ...
    fJpsipb = new TFile(Form("./ratioDataMC_pp_BtoJpsi_DATA_ctauCut_%s_260514_2exp.root", rapTag.Data()),"RECREATE");
	  fJpsipb->cd();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
	  fitRatio1->SetName("dataMC_Ratio1");
  ...
  if(WRITE==1){
    c_A->SaveAs(Form("./figs/dNdpt_plot_pp_%s_Jpsi_%s_260514_2exp.pdf", fname.Data(), rapTag.Data()));
    c_A->SaveAs(Form("./figs/dNdpt_plot_pp_%s_Jpsi_%s_260514_2exp.png", fname.Data(), rapTag.Data()));
  }
...
valErr getYield(int isPR, float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh) {
  ...
  TFile* inf = new TFile(Form("../Macros/Jpsi_L_cut_pTreweight/roots_1S_pp/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel.Data()));
...
valErr getCtauEff(int isPR, float ptLow, float ptHigh, float yLow, float yHigh, float SiMuPtCut, float massLow, float massHigh) {
  ...
  TFile* inf = new TFile(Form("../Eff_Acc_pTreweight/roots_1S_pp_pTreweight/ctau3D_cut_%s_pt%.1f-%.1f_y%.1f-%.1f.root", PR.Data(), ptLow, ptHigh, yLow, yHigh));
  valErr ret; ret.val = 0; ret.err = 0;
  TH1D* hEff = nullptr;
  if (inf && !inf->IsZombie()) hEff = (TH1D*)inf->Get(Form("%s", hPR.Data()));
  if (!hEff) {
      if (inf) inf->Close();
      inf = new TFile(Form("../ctau_eff/roots_1S_pp/decayL/%s/decay_hist_%s_m2.6-3.5.root", PR.Data(), kineLabel.Data()));
      if (!inf || inf->IsZombie()) {
