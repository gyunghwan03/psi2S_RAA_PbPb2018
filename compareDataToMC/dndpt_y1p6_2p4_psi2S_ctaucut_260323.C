...
void dndpt_y1p6_2p4_psi2S_ctaucut_260323(int PR=0, int WRITE=1) {
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  TFile*fJpsipb;
  if(WRITE==1&&PR==0){
	  fJpsipb = new TFile("./ratioDataMC_AA_psi2S_DATA_ctauCut_y1p6_2p4_260323.root","RECREATE");
	  fJpsipb->cd();
    ...
    c_A->Write();
	  ...
	  fJpsipb = new TFile("./ratioDataMC_AA_Btopsi2S_DATA_ctauCut_y1p6_2p4_260323.root","RECREATE");
	  fJpsipb->cd();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
	  fitRatio1->SetName("dataMC_Ratio1");
  ...
  if(WRITE==1){
	  c_A->SaveAs(Form("./dNdpt_plot_psi2S_%s_y1p6_2p4_260323.pdf",fname.Data()));
	  c_A->SaveAs(Form("./dNdpt_plot_psi2S_%s_y1p6_2p4_260323.png",fname.Data()));
