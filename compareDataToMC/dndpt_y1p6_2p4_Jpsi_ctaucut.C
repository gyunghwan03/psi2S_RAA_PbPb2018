...
valErr getYield(int isPR, float ptLow, float ptHigh, float yLow, float yHigh, int cLow, int cHigh) {
  ...
  TString PR = (isPR==0) ? "PRMC" : "NPRMC";
  TFile* inf = new TFile(Form("../Macros/Jpsi_L_cut/roots_1S_Pb/%s/Mass/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel.Data()));
