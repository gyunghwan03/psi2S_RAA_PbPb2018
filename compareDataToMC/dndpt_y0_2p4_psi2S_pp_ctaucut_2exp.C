...
void dndpt_y0_2p4_psi2S_pp_ctaucut_2exp(int PR=0, int WRITE=1) {
...
//  else if(PR==1) f1 ="../../skimmedFiles/OniaFlowSkim_Jpsi_MC_NonPrompt_210107.root";
  ...
  if(WRITE==1&&PR==0){
	  fJpsipb = new TFile("./ratioDataMC_pp_psi2S_DATA_ctauCut_y0_2p4_260323_2exp.root","RECREATE");
	  fJpsipb->cd();
    c_A->Write();
    ...
    if(fitRatioOnHist) fitRatioOnHist->Write("dataMC_Ratio1");
	  ...
	  fJpsipb = new TFile("./ratioDataMC_pp_Btopsi2S_DATA_ctauCut_y0_2p4_260323_2exp.root","RECREATE");
	  fJpsipb->cd();
    c_A->Write();
	  hptData1->SetName("WeightFactor");
	  hptData1->Write();
  ...
  if(WRITE==1){
	  c_A->SaveAs(Form("./figs/dNdpt_plot_pp_%s_psi2S_y0_2p4_260323_2exp.pdf",fname.Data()));
	  c_A->SaveAs(Form("./figs/dNdpt_plot_pp_%s_psi2S_y0_2p4_260323_2exp.png",fname.Data()));
