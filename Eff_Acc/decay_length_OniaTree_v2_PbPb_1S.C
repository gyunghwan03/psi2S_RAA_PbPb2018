...
void decay_length_OniaTree_v2_PbPb_1S(
		int state = 1,
		...
		)
{
	...
	// Create directories
	gSystem->mkdir("./roots_1S_PbPb/");
	gSystem->mkdir("./roots_1S_PbPb/decayL");
	gSystem->mkdir("./roots_1S_PbPb/decayL/PRMC");
	gSystem->mkdir("./roots_1S_PbPb/decayL/NPMC");

	gSystem->mkdir("./figs_1S_PbPb/");
	gSystem->mkdir("./figs_1S_PbPb/decayL");
	gSystem->mkdir("./figs_1S_PbPb/decayL/PRMC");
	gSystem->mkdir("./figs_1S_PbPb/decayL/NPMC");
	...
	TFile *fPtW2;
	if(state==1){
		fPtW1 = new TFile("../compareDataToMC/ratioDataMC_AA_Jpsi_DATA_ctauCut_y0_1p6_251103.root", "read");
		fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_Jpsi_DATA_ctauCut_y1p6_2p4_251103.root", "read");
	}
	else if(state==2){
		fPtW1 = new TFile("../compareDataToMC/ratioDataMC_AA_BtoJpsi_DATA_ctauCut_y0_1p6_251103.root", "read");
		fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_BtoJpsi_DATA_ctauCut_y1p6_2p4_251103.root", "read");
	}
	...
	if(state==1){
		...
		c_decayL->Update();
		c_decayL->SaveAs(Form("figs_1S_PbPb/decayL/PRMC/decay_%s.pdf",kineLabel.Data()));
	}
	else if(state==2){
		drawText(Form("L_{#J/#psi} cut: %.4f", lcutv->GetX1()),pos_x_mass,pos_y-pos_y_diff*3,text_color,text_size);
		...
		c_decayL->Update();
		c_decayL->SaveAs(Form("figs_1S_PbPb/decayL/NPMC/decay_%s.pdf",kineLabel.Data()));
	}

	// Save histograms
	TFile *wf;
	if(state==1){
		wf = new TFile(Form("roots_1S_PbPb/decayL/PRMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
	}
	else if(state==2){
		wf = new TFile(Form("roots_1S_PbPb/decayL/NPMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
	}
	wf->cd();
	...
	if (type == 1 || type == 3) {
		...
		for (int ipt = 0; ipt < nPtBins_for; ipt++) {
			...
			cout<<bins<<"th bin"<<", lJpsi: "<<h_decay_pr_pt_for[ipt]->GetBinCenter(bins)<<", PR eff: "<<h_decay_pr_pt_for[ipt]->Integral(0,bins)/h_decay_pr_pt_for[ipt]->GetEntries()*100<<"%"<<endl;
			...
			if (state == 1) {
				...
				c_decayL_for->Update();
				c_decayL_for->SaveAs(Form("figs_1S_PbPb/decayL/PRMC/decay_state%d_pt%.1f-%.1f_y1.6-2.4_m%.1f-%.1f.pdf", state, ptBin_for[ipt], ptBin_for[ipt+1], massLow, massHigh));
			} else if (state == 2) {
				drawText(Form("L_{#J/#psi} cut: %.4f", ctau3D_cut_ptBin_for[ipt]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				...
				c_decayL_for->Update();
				c_decayL_for->SaveAs(Form("figs_1S_PbPb/decayL/NPMC/decay_state%d_pt%.1f-%.1f_y1.6-2.4_m%.1f-%.1f.pdf", state, ptBin_for[ipt], ptBin_for[ipt+1], massLow, massHigh));
			}
			delete c_decayL_for;
		}
		...
		for (int ipt = 0; ipt < nPtBins_mid; ipt++) {
			...
			cout<<bins<<"th bin"<<", lJpsi: "<<h_decay_pr_pt_mid[ipt]->GetBinCenter(bins)<<", PR eff: "<<h_decay_pr_pt_mid[ipt]->Integral(0,bins)/h_decay_pr_pt_mid[ipt]->GetEntries()*100<<"%"<<endl;
			...
			if (state == 1) {
				...
				c_decayL_mid->Update();
				c_decayL_mid->SaveAs(Form("figs_1S_PbPb/decayL/PRMC/decay_state%d_pt%.1f-%.1f_y0.0-1.6_m%.1f-%.1f.pdf", state, ptBin_mid[ipt], ptBin_mid[ipt+1], massLow, massHigh));
			} else if (state == 2) {
				drawText(Form("L_{#J/#psi} cut: %.4f", ctau3D_cut_ptBin_mid[ipt]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				...
				c_decayL_mid->Update();
				c_decayL_mid->SaveAs(Form("figs_1S_PbPb/decayL/NPMC/decay_state%d_pt%.1f-%.1f_y0.0-1.6_m%.1f-%.1f.pdf", state, ptBin_mid[ipt], ptBin_mid[ipt+1], massLow, massHigh));
			}
			delete c_decayL_mid;
		}
		...
		if (state == 1) {
			rootFileNamePt = Form("./roots_1S_PbPb/ctau3D_cut_ptBin_PRMC_y%.1f-%.1f.root", yLow, yHigh);
		} else {
			rootFileNamePt = Form("./roots_1S_PbPb/ctau3D_cut_ptBin_NPMC_y%.1f-%.1f.root", yLow, yHigh);
		}
	...
	if (type == 2 || type == 3) {
		...
		if (state == 1) {
			rootFileNameCent = Form("./roots_1S_PbPb/ctau3D_cut_centBin_PRMC_y%.1f-%.1f.root", yLow, yHigh);
		} else {
			rootFileNameCent = Form("./roots_1S_PbPb/ctau3D_cut_centBin_NPMC_y%.1f-%.1f.root", yLow, yHigh);
		}
