		...
		)
{
	...
	if(isTnP){
		...
		tnp_weight_pr = tnp_weight_pr * tnp_weight_muid_pbpb(pt1_pr, eta1_pr, 0) * tnp_weight_muid_pbpb(pt2_pr, eta2_pr, 0); //mu id
		tnp_weight_pr = tnp_weight_pr * tnp_weight_trk_pbpb(eta1_pr, 0) * tnp_weight_trk_pbpb(eta2_pr, 0); //inner tracker

		if(!((Reco_mu_trig_pr[Reco_QQ_mupl_idx_pr[irqq]]&((ULong64_t)pow(2, kL2filter))) == ((ULong64_t)pow(2, kL2filter)) && (Reco_mu_trig_pr[Reco_QQ_mumi_idx_pr[irqq]]&((ULong64_t)pow(2, kL2filter))) == ((ULong64_t)pow(2, kL2filter)) ) ) continue;

		bool mupl_L2Filter = ( (Reco_mu_trig_pr[Reco_QQ_mupl_idx_pr[irqq]]&((ULong64_t)pow(2, kL2filter))) == ((ULong64_t)pow(2, kL2filter)) ) ? true : false ;
		bool mupl_L3Filter = ( (Reco_mu_trig_pr[Reco_QQ_mupl_idx_pr[irqq]]&((ULong64_t)pow(2, kL3filter))) == ((ULong64_t)pow(2, kL3filter)) ) ? true : false ;
		bool mumi_L2Filter = ( (Reco_mu_trig_pr[Reco_QQ_mumi_idx_pr[irqq]]&((ULong64_t)pow(2, kL2filter))) == ((ULong64_t)pow(2, kL2filter)) ) ? true : false ;
		bool mumi_L3Filter = ( (Reco_mu_trig_pr[Reco_QQ_mumi_idx_pr[irqq]]&((ULong64_t)pow(2, kL3filter))) == ((ULong64_t)pow(2, kL3filter)) ) ? true : false ;
		...
		if( mupl_isL2 && mumi_isL3){
		 tnp_trig_weight_mupl = tnp_weight_trg_pbpb(mupl_pr->Pt(), mupl_pr->Eta(), 2, 0); 
		 tnp_trig_weight_mumi = tnp_weight_trg_pbpb(mumi_pr->Pt(), mumi_pr->Eta(), 3, 0); 
		 SelDone = true;
		}   
		else if( mupl_isL3 && mumi_isL2){
		 tnp_trig_weight_mupl = tnp_weight_trg_pbpb(mupl_pr->Pt(), mupl_pr->Eta(), 3, 0); 
		 tnp_trig_weight_mumi = tnp_weight_trg_pbpb(mumi_pr->Pt(), mumi_pr->Eta(), 2, 0); 
		 SelDone = true;
		}   
		...
		tnp_weight_pr = tnp_weight_pr * tnp_trig_weight_mupl * tnp_trig_weight_mumi;
	  }
	...
	if(isTnP){
		...
		tnp_weight_np = tnp_weight_np * tnp_weight_muid_pbpb(pt1_np, eta1_np, 0) * tnp_weight_muid_pbpb(pt2_np, eta2_np, 0); //mu id
		tnp_weight_np = tnp_weight_np * tnp_weight_trk_pbpb(eta1_np, 0) * tnp_weight_trk_pbpb(eta2_np, 0); //inner tracker

		if(!((Reco_mu_trig_np[Reco_QQ_mupl_idx_np[irqq]]&((ULong64_t)pow(2, kL2filter))) == ((ULong64_t)pow(2, kL2filter)) && (Reco_mu_trig_np[Reco_QQ_mumi_idx_np[irqq]]&((ULong64_t)pow(2, kL2filter))) == ((ULong64_t)pow(2, kL2filter)) ) ) continue;

		bool mupl_L2Filter = ( (Reco_mu_trig_np[Reco_QQ_mupl_idx_np[irqq]]&((ULong64_t)pow(2, kL2filter))) == ((ULong64_t)pow(2, kL2filter)) ) ? true : false ;
		bool mupl_L3Filter = ( (Reco_mu_trig_np[Reco_QQ_mupl_idx_np[irqq]]&((ULong64_t)pow(2, kL3filter))) == ((ULong64_t)pow(2, kL3filter)) ) ? true : false ;
		bool mumi_L2Filter = ( (Reco_mu_trig_np[Reco_QQ_mumi_idx_np[irqq]]&((ULong64_t)pow(2, kL2filter))) == ((ULong64_t)pow(2, kL2filter)) ) ? true : false ;
		bool mumi_L3Filter = ( (Reco_mu_trig_np[Reco_QQ_mumi_idx_np[irqq]]&((ULong64_t)pow(2, kL3filter))) == ((ULong64_t)pow(2, kL3filter)) ) ? true : false ;
		if(mupl_L2Filter == false || mumi_L2Filter == false){ cout << "TnP ERROR !!!! ::: No matched L2 filter2 " << endl; cout << endl;}
		...
		if( mupl_isL2 && mumi_isL3){
		 tnp_trig_weight_mupl = tnp_weight_trg_pbpb(mupl_np->Pt(), mupl_np->Eta(), 2, 0); 
		 tnp_trig_weight_mumi = tnp_weight_trg_pbpb(mumi_np->Pt(), mumi_np->Eta(), 3, 0); 
		 SelDone = true;
		}   
		else if( mupl_isL3 && mumi_isL2){
		 tnp_trig_weight_mupl = tnp_weight_trg_pbpb(mupl_np->Pt(), mupl_np->Eta(), 3, 0); 
		 tnp_trig_weight_mumi = tnp_weight_trg_pbpb(mumi_np->Pt(), mumi_np->Eta(), 2, 0); 
		 SelDone = true;
		}   
		else if( mupl_isL3 && mumi_isL3){
		 int t[2] = {-1,1}; // mupl, mumi
		...
		if(SelDone == false || (tnp_trig_weight_mupl == -1 || tnp_trig_weight_mumi == -1)){cout << "ERROR :: No trigger muon tnp scale factors selected !!!!" << endl; continue;}
		tnp_weight_np = tnp_weight_np * tnp_trig_weight_mupl * tnp_trig_weight_mumi;
	  }
