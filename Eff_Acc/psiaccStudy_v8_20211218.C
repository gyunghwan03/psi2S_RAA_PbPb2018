...
void psiaccStudy_v8_20211218(int wtopt=1, int isPtWgtUp = 0, TString rmk="20211218")
{
  //Basic Setting
  gStyle->SetOptStat(0);
  setTDRStyle();
  ...
  for(int i=0; i<nEvt; i++){
    ...
    for(int j = 0; j < Gen_QQ_size; j++){
      ...
      if (wtopt==1) {
        wt1 = fptw1->Eval(pt);
        wt2 = fptw2->Eval(pt);
        if(1.6<=fabs(y) && fabs(y)<2.4 && pt > 12.0) wt2 = fptw2->Eval(12);
      }
      //else if (wtopt==2) wt = fptw2->Eval(pt);
  ...
  TFile *wf = new TFile(Form("acceptance_Prompt_psi2s_GenOnly_wgt%d_%s_SysUp%d_20211218.root",wtopt,rmk.Data(),isPtWgtUp),"RECREATE");
  wf->cd();
