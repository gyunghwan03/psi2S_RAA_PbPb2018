...
)
{
  ...
  for (int iev = 0; iev < treePRMC->GetEntries(); ++iev) {
    if (iev % 100000 == 0) cout << "PRMC: " << iev << " / " << treePRMC->GetEntries() << endl;
  ...
  for (int iev = 0; iev < treeNPMC->GetEntries(); ++iev) {
    if (iev % 100000 == 0) cout << "NPMC: " << iev << " / " << treeNPMC->GetEntries() << endl;
  ...
  //============================================================================
  gSystem->mkdir("./figs_ctau3D/", 1);
  gSystem->mkdir("./roots_ctau3D/", 1);
  ...
  c_decayL->Update();
  if (state == 1) {
    c_decayL->SaveAs(Form("./figs_ctau3D/ctau3D_cut_PRMC_%s.pdf", kineLabel.Data()));
    c_decayL->SaveAs(Form("./figs_ctau3D/ctau3D_cut_PRMC_%s.png", kineLabel.Data()));
  } else {
    c_decayL->SaveAs(Form("./figs_ctau3D/ctau3D_cut_NPMC_%s.pdf", kineLabel.Data()));
    c_decayL->SaveAs(Form("./figs_ctau3D/ctau3D_cut_NPMC_%s.png", kineLabel.Data()));
  }
  ...
  c_ctau_dist->Update();
  if (state == 1) {
    c_ctau_dist->SaveAs(Form("./figs_ctau3D/ctau3D_dist_PRMC_%s.pdf", kineLabel.Data()));
    c_ctau_dist->SaveAs(Form("./figs_ctau3D/ctau3D_dist_PRMC_%s.png", kineLabel.Data()));
  } else {
    c_ctau_dist->SaveAs(Form("./figs_ctau3D/ctau3D_dist_NPMC_%s.pdf", kineLabel.Data()));
    c_ctau_dist->SaveAs(Form("./figs_ctau3D/ctau3D_dist_NPMC_%s.png", kineLabel.Data()));
  }
  
  // Save histograms and cut value to ROOT file
  TString rootFileName;
  if (state == 1) {
    rootFileName = Form("./roots_ctau3D/ctau3D_cut_PRMC_%s.root", kineLabel.Data());
  } else {
    rootFileName = Form("./roots_ctau3D/ctau3D_cut_NPMC_%s.root", kineLabel.Data());
  }
  ...
  for (int ipt = 0; ipt < nPtBins_for; ipt++) {
    int totalPR_pt = h_decay_pr_pt_for[ipt]->GetEntries();
    int totalNP_pt = h_decay_np_pt_for[ipt]->GetEntries();
    ...
    if (totalPR_pt > 0 && totalNP_pt > 0) {
      ...
      cout << Form("pt %.1f-%.1f: ctau3D cut = %.4f, eff = %.3f, res = %.3f, entries PR=%d NP=%d", 
      ...
      c_eff_pt_for->SaveAs(Form("./figs_ctau3D/eff_ptBin_for_%s_pt%.1f-%.1f.pdf", stateStr.Data(), ptBin_for[ipt], ptBin_for[ipt+1]));
      c_eff_pt_for->SaveAs(Form("./figs_ctau3D/eff_ptBin_for_%s_pt%.1f-%.1f.png", stateStr.Data(), ptBin_for[ipt], ptBin_for[ipt+1]));
      ...
      cout << Form("pt %.1f-%.1f: Not enough entries (PR=%d, NP=%d)", ptBin_for[ipt], ptBin_for[ipt+1], totalPR_pt, totalNP_pt) << endl;
    }
  ...
  for (int ipt = 0; ipt < nPtBins_mid; ipt++) {
    int totalPR_pt = h_decay_pr_pt_mid[ipt]->GetEntries();
    int totalNP_pt = h_decay_np_pt_mid[ipt]->GetEntries();
    ...
    if (totalPR_pt > 0 && totalNP_pt > 0) {
      ...
      TString stateStr = (state == 1) ? "PRMC" : "NPMC";
      c_eff_pt_mid->SaveAs(Form("./figs_ctau3D/eff_ptBin_mid_%s_pt%.1f-%.1f.pdf", stateStr.Data(), ptBin_mid[ipt], ptBin_mid[ipt+1]));
      c_eff_pt_mid->SaveAs(Form("./figs_ctau3D/eff_ptBin_mid_%s_pt%.1f-%.1f.png", stateStr.Data(), ptBin_mid[ipt], ptBin_mid[ipt+1]));
      ...
      cout << Form("pt %.1f-%.1f: Not enough entries (PR=%d, NP=%d)", ptBin_mid[ipt], ptBin_mid[ipt+1], totalPR_pt, totalNP_pt) << endl;
    }
  ...
  for (int iev = 0; iev < treeNPMC->GetEntries(); ++iev) {
    ...
    for (Int_t irqq = 0; irqq < Reco_QQ_size_np; irqq++) {
      ...
      if (rapidity < 1.6 && JP_np->Pt() >= 6.5 && JP_np->Pt() < 40) {
        // Fill integrated histogram (cent 0-180)
        if (Centrality_np >= 0 && Centrality_np < 180) {
          h_decay_np_int_mid->Fill(ctau3D_val);
        }
        // Fill cent bins
        for (int icent = 0; icent < nCentBins_mid; icent++) {
  ...
  int totalPR_int_for = h_decay_pr_int_for->GetEntries();
  int totalNP_int_for = h_decay_np_int_for->GetEntries();
  ...
  if (totalPR_int_for > 0 && totalNP_int_for > 0) {
    ...
    cout << Form("Integrated: ctau3D cut = %.4f, PR eff = %.3f, NP res = %.3f, entries PR=%d NP=%d", 
    ...
    TString stateStr = (state == 1) ? "PRMC" : "NPMC";
    c_eff_int_for->SaveAs(Form("./figs_ctau3D/eff_integrated_for_%s_pt3.5-40_cent0-90.pdf", stateStr.Data()));
    c_eff_int_for->SaveAs(Form("./figs_ctau3D/eff_integrated_for_%s_pt3.5-40_cent0-90.png", stateStr.Data()));
  ...
  int totalPR_int_mid = h_decay_pr_int_mid->GetEntries();
  int totalNP_int_mid = h_decay_np_int_mid->GetEntries();
  ...
  if (totalPR_int_mid > 0 && totalNP_int_mid > 0) {
    ...
    TString stateStr = (state == 1) ? "PRMC" : "NPMC";
    c_eff_int_mid->SaveAs(Form("./figs_ctau3D/eff_integrated_mid_%s_pt6.5-40_cent0-90.pdf", stateStr.Data()));
    c_eff_int_mid->SaveAs(Form("./figs_ctau3D/eff_integrated_mid_%s_pt6.5-40_cent0-90.png", stateStr.Data()));
  ...
  if (state == 1) {
    rootFileName_int = "./roots_ctau3D/ctau3D_cut_integrated_PRMC.root";
  } else {
    rootFileName_int = "./roots_ctau3D/ctau3D_cut_integrated_NPMC.root";
  }
  ...
  for (int icent = 0; icent < nCentBins_for; icent++) {
    ...
    int totalPR = h_decay_pr_cent_for[icent]->GetEntries();
    int totalNP = h_decay_np_cent_for[icent]->GetEntries();
    ...
    if (totalPR > 0 && totalNP > 0) {
      ...
      cout << Form("cent %.0f-%.0f: ctau3D cut = %.4f, PR eff = %.3f, NP res = %.3f", 
      ...
      TString stateStr = (state == 1) ? "PRMC" : "NPMC";
      c_eff_cent_for->SaveAs(Form("./figs_ctau3D/eff_centBin_for_%s_cent%.0f-%.0f.pdf", stateStr.Data(), centLow_bin/2, centHigh_bin/2));
      c_eff_cent_for->SaveAs(Form("./figs_ctau3D/eff_centBin_for_%s_cent%.0f-%.0f.png", stateStr.Data(), centLow_bin/2, centHigh_bin/2));
  ...
  for (int icent = 0; icent < nCentBins_mid; icent++) {
    ...
    int totalPR = h_decay_pr_cent_mid[icent]->GetEntries();
    int totalNP = h_decay_np_cent_mid[icent]->GetEntries();
    ...
    if (totalPR > 0 && totalNP > 0) {
      ...
      TString stateStr = (state == 1) ? "PRMC" : "NPMC";
      c_eff_cent_mid->SaveAs(Form("./figs_ctau3D/eff_centBin_mid_%s_cent%.0f-%.0f.pdf", stateStr.Data(), centLow_bin/2, centHigh_bin/2));
      c_eff_cent_mid->SaveAs(Form("./figs_ctau3D/eff_centBin_mid_%s_cent%.0f-%.0f.png", stateStr.Data(), centLow_bin/2, centHigh_bin/2));
  ...
  c_cent_ctau->Update();
  if (state == 1) {
    c_cent_ctau->SaveAs("./figs_ctau3D/cent_vs_ctau3D_cut_PRMC.pdf");
    c_cent_ctau->SaveAs("./figs_ctau3D/cent_vs_ctau3D_cut_PRMC.png");
  } else {
    c_cent_ctau->SaveAs("./figs_ctau3D/cent_vs_ctau3D_cut_NPMC.pdf");
    c_cent_ctau->SaveAs("./figs_ctau3D/cent_vs_ctau3D_cut_NPMC.png");
  }
  ...
  if (state == 1) {
    rootFileName_centBin_for = "./roots_ctau3D/ctau3D_cut_centBin_for_PRMC_pt3.5-40.0.root";
    rootFileName_centBin_mid = "./roots_ctau3D/ctau3D_cut_centBin_mid_PRMC_pt6.5-40.0.root";
  } else {
    rootFileName_centBin_for = "./roots_ctau3D/ctau3D_cut_centBin_for_NPMC_pt3.5-40.0.root";
    rootFileName_centBin_mid = "./roots_ctau3D/ctau3D_cut_centBin_mid_NPMC_pt6.5-40.0.root";
  }
  ...
  c_pt_ctau->Update();
  if (state == 1) {
    c_pt_ctau->SaveAs("./figs_ctau3D/pt_vs_ctau3D_cut_PRMC.pdf");
    c_pt_ctau->SaveAs("./figs_ctau3D/pt_vs_ctau3D_cut_PRMC.png");
  } else {
    c_pt_ctau->SaveAs("./figs_ctau3D/pt_vs_ctau3D_cut_NPMC.pdf");
    c_pt_ctau->SaveAs("./figs_ctau3D/pt_vs_ctau3D_cut_NPMC.png");
  }
  
  // Save pt bin ctau3D cut values to ROOT file
  TString rootFileName_ptBin;
  if (state == 1) {
    rootFileName_ptBin = Form("./roots_ctau3D/ctau3D_cut_ptBin_PRMC_cent%.0f-%.0f.root", cLow, cHigh);
  } else {
    rootFileName_ptBin = Form("./roots_ctau3D/ctau3D_cut_ptBin_NPMC_cent%.0f-%.0f.root", cLow, cHigh);
  }
