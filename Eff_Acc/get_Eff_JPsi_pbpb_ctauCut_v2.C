...
void get_Eff_JPsi_pbpb_ctauCut_v2(
    int state = 2,
...
//      if ( Reco_QQ_VtxProb[irqq]  < 0.005 ) continue;
      ...
      if (HLTPass == true && HLTFilterPass == true)
      {
        ...
        if ((Centrality > cLow && Centrality < cHigh))
        {
          ...
          if (Rapidity > 1.6 && Rapidity < 2.4)
          {
            ...
            TString kineLabel_tmp = getKineLabel(ptLow_tmp, ptHigh_tmp, 1.6, 2.4, 0, 0, 180);
            TString fpath_tmp = Form("../ctau_eff/roots_1S_Pb/decayL/%sMC/decay_hist_%s_m2.6-3.5.root", fname.Data(), kineLabel_tmp.Data());
            TFile *f_ctau_tmp = TFile::Open(fpath_tmp, "READ");
            TH1D *h_Lcut_tmp = (TH1D *)f_ctau_tmp->Get("h_Lcut");
            l_cut_for[i] = h_Lcut_tmp->GetBinContent(1);
            double ptLow = ptBin_for[i+1];
            double ptHigh = ptBin_for[i+2];
            double l_cut = -10.0;
            if(JP_Reco->Pt() > ptLow && JP_Reco->Pt() < ptHigh) {
              l_cut = l_cut_for[i];
              if(state==1) { if(Reco_QQ_ctau3D[irqq] < l_cut) hpt_reco_1->Fill(JP_Reco->Pt(), weight * tnp_weight * pt_weight); }
              else if(state==2) { if(Reco_QQ_ctau3D[irqq] > l_cut) hpt_reco_1->Fill(JP_Reco->Pt(), weight * tnp_weight * pt_weight); }
            }
            f_ctau_tmp->Close();
            delete f_ctau_tmp;
            }
          ...
          if (Rapidity < 1.6)
          {
            ...
            TString kineLabel_tmp = getKineLabel(ptLow_tmp, ptHigh_tmp, 0, 1.6, 0, 0, 180);
            TString fpath_tmp = Form("../ctau_eff/roots_1S_Pb/decayL/%sMC/decay_hist_%s_m2.6-3.5.root", fname.Data(), kineLabel_tmp.Data());
            TFile *f_ctau_tmp = TFile::Open(fpath_tmp, "READ");
            TH1D *h_Lcut_tmp = (TH1D *)f_ctau_tmp->Get("h_Lcut");
            l_cut_mid[i] = h_Lcut_tmp->GetBinContent(1);
            double ptLow = ptBin_mid[i+1];
            double ptHigh = ptBin_mid[i+2];
            double l_cut = -10.0;
            if(JP_Reco->Pt() > ptLow && JP_Reco->Pt() < ptHigh) {
              l_cut = l_cut_mid[i];
              if(state==1) { if(Reco_QQ_ctau3D[irqq] < l_cut) hpt_reco_2->Fill(JP_Reco->Pt(), weight * tnp_weight * pt_weight); }
              else if(state==2) { if(Reco_QQ_ctau3D[irqq] > l_cut) hpt_reco_2->Fill(JP_Reco->Pt(), weight * tnp_weight * pt_weight); }
            }
            f_ctau_tmp->Close();
            delete f_ctau_tmp;
            }
        ...
        if (Rapidity > 1.6 && Rapidity < 2.4)
        {
          ...
          if (JP_Reco->Pt() > 3.5 && JP_Reco->Pt() < 40)
          {
            ...
            TString kineLabel_tmp = getKineLabel(3.5, 40, 1.6, 2.4, 0, cLow_tmp, cHigh_tmp);
            TFile *f_ctau_tmp = TFile::Open(Form("../ctau_eff/roots_1S_Pb/decayL/%sMC/decay_hist_%s_m2.6-3.5.root", fname.Data(), kineLabel_tmp.Data()), "READ");
              TH1D *h_Lcut_tmp = (TH1D *)f_ctau_tmp->Get("h_Lcut");
              l_cut_for[i] = h_Lcut_tmp->GetBinContent(1);
            ...
            if(Centrality > 0 && Centrality < 180)
            {
              ...
              TH1D *h_Lcut_tmp = (TH1D *)f_ctau_tmp->Get("h_Lcut");
              double l_cut = h_Lcut_tmp->GetBinContent(1);
        ...
        if (Rapidity < 1.6 && JP_Reco->Pt() > 6.5 && JP_Reco->Pt() < 40)
        {
          for(int i=0; i < 6 ; i++) {
            ...
            TString kineLabel_tmp = getKineLabel(6.5, 40, 0, 1.6, 0, cLow_tmp, cHigh_tmp);
            TFile *f_ctau_tmp = TFile::Open(Form("../ctau_eff/roots_1S_Pb/decayL/%sMC/decay_hist_%s_m2.6-3.5.root", fname.Data(), kineLabel_tmp.Data()), "READ");
            TH1D *h_Lcut_tmp = (TH1D *)f_ctau_tmp->Get("h_Lcut");
            l_cut_for[i] = h_Lcut_tmp->GetBinContent(1);
  ...
  */
  ...
  TString outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_JPsi_PtW%s_tnp%d_ctauCut_260103.root", cLow, cHigh, ptSys.Data(), isTnP);
  if (state == 2)
    outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_JPsi_PtW%s_tnp%d_ctauCut_260103.root", cLow, cHigh, ptSys.Data(), isTnP);
