...
void get_Eff_JPsi_pbpb_ctauCut_v3(
    int state = 2,
...
)
{
  ...
  for(int i=0; i<4; ++i){
    ...
    if(f_ctau_tmp && !f_ctau_tmp->IsZombie()){ 
      ...
      if(h_Lcut_tmp) l_cut_for_pt[i] = h_Lcut_tmp->GetBinContent(1);
  ...
  for(int i=0; i<6; ++i){
    ...
    if(f_ctau_tmp && !f_ctau_tmp->IsZombie()){
      TH1D *h_Lcut_tmp = (TH1D *)f_ctau_tmp->Get("h_Lcut");
      if(h_Lcut_tmp) l_cut_mid_pt[i] = h_Lcut_tmp->GetBinContent(1);
  ...
  for(int i=0; i<4; ++i){
    ...
    if(f_ctau_tmp && !f_ctau_tmp->IsZombie()){
      TH1D *h_Lcut_tmp = (TH1D *)f_ctau_tmp->Get("h_Lcut");
      if(h_Lcut_tmp) l_cut_for_cent[i] = h_Lcut_tmp->GetBinContent(1);
  ...
  for(int i=0; i<6; ++i){
    ...
    if(f_ctau_tmp && !f_ctau_tmp->IsZombie()){
      TH1D *h_Lcut_tmp = (TH1D *)f_ctau_tmp->Get("h_Lcut");
      if(h_Lcut_tmp) l_cut_mid_cent[i] = h_Lcut_tmp->GetBinContent(1);
