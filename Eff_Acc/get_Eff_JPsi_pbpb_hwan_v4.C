...
void get_Eff_JPsi_pbpb_hwan_v4(
    int state = 2,
...
)
{
  ...
  TFile *fPtW1 = new TFile("../compareDataToMC/ratioDataMC_pp_JPsi_DATA_y0_1p6_250114.root", "read");
  TFile *fPtW2 = new TFile("../compareDataToMC/ratioDataMC_pp_JPsi_DATA_y1p6_2p4_250114.root", "read");
  // if (state == 2) {
  ...
  // for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for (int igen = 0; igen < Gen_QQ_size; igen++)
    {
      ...
      if (fabs(JP_Gen->Rapidity()) < 2.4 && JP_Gen->Pt() > 6.5)
...
//      if ( Reco_QQ_VtxProb[irqq]  < 0.005 ) continue;
      ...
      if (fabs(JP_Reco->Rapidity()) < 2.4 && JP_Reco->Pt() > 6.5)
  ...
  */
  ...
  TString outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_JPsi_PtW%s_tnp%d_ppWfunc_v4.root", cLow, cHigh, ptSys.Data(), isTnP);
  if (state == 2)
    outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_JPsi_PtW%s_tnp%d_ppWfunc_v4.root", cLow, cHigh, ptSys.Data(), isTnP);
