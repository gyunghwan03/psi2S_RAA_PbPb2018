...
void decay_length_OniaTree_v2_pp_2S(
		int state = 1,
		...
		)
{
	...
	TString fPRMC_path = "/data/Oniatree/psi2S/OniatreeMC_Psi2SMM_TuneCUETP8M1_5p02TeV_pythia8_RunIIpp5Spring18DR-94X_mc2017_realistic_forppRef5TeV-v2.root"; // pp prompt
	TString fNPMC_path = "/data/Oniatree/miniAOD/OniatreeMC_BPsi2SMM_TuneCUETP8M1_5p02TeV_pythia8_ptHatMin2_ONIATREE.root"; // pp non-prompt
	...
	if(state==1){
		...
		c_decayL->Update();
		c_decayL->SaveAs(Form("figs_2S_pp/decayL/PRMC/decay_%s.pdf",kineLabel.Data()));
	}
	else if(state==2){
		drawText(Form("L_{#psi(2S)} cut: %.4f", lcutv->GetX1()),pos_x_mass,pos_y-pos_y_diff*3,text_color,text_size);
		...
		c_decayL->Update();
		c_decayL->SaveAs(Form("figs_2S_pp/decayL/NPMC/decay_%s.pdf",kineLabel.Data()));
	}

	// Save histograms
	TFile *wf;
	if(state==1){
		wf = new TFile(Form("roots_2S_pp/decayL/PRMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
	}
	else if(state==2){
		wf = new TFile(Form("roots_2S_pp/decayL/NPMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
	}
	wf->cd();
	...
	if (type == 1 || type == 3) {
		...
		for (int ipt = 0; ipt < nPtBins_for; ipt++) {
			...
			if (state == 1) {
				...
				c_decayL_for->Update();
				c_decayL_for->SaveAs(Form("figs_2S_pp/decayL/PRMC/decay_state%d_pt%.1f-%.1f_y1.6-2.4_m%.1f-%.1f.pdf", state, ptBin_for[ipt], ptBin_for[ipt+1], massLow, massHigh));
			} else if (state == 2) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_ptBin_for[ipt]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				...
				c_decayL_for->Update();
				c_decayL_for->SaveAs(Form("figs_2S_pp/decayL/NPMC/decay_state%d_pt%.1f-%.1f_y1.6-2.4_m%.1f-%.1f.pdf", state, ptBin_for[ipt], ptBin_for[ipt+1], massLow, massHigh));
			}
			delete c_decayL_for;
		}
		...
		for (int ipt = 0; ipt < nPtBins_mid; ipt++) {
			...
			if (state == 1) {
				...
				c_decayL_mid->Update();
				c_decayL_mid->SaveAs(Form("figs_2S_pp/decayL/PRMC/decay_state%d_pt%.1f-%.1f_y0.0-1.6_m%.1f-%.1f.pdf", state, ptBin_mid[ipt], ptBin_mid[ipt+1], massLow, massHigh));
			} else if (state == 2) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_ptBin_mid[ipt]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				...
				c_decayL_mid->Update();
				c_decayL_mid->SaveAs(Form("figs_2S_pp/decayL/NPMC/decay_state%d_pt%.1f-%.1f_y0.0-1.6_m%.1f-%.1f.pdf", state, ptBin_mid[ipt], ptBin_mid[ipt+1], massLow, massHigh));
			}
			delete c_decayL_mid;
		}
		...
		if (state == 1) {
			rootFileNamePt = Form("./roots_2S_pp/ctau3D_cut_ptBin_PRMC_y%.1f-%.1f.root", yLow, yHigh);
		} else {
			rootFileNamePt = Form("./roots_2S_pp/ctau3D_cut_ptBin_NPMC_y%.1f-%.1f.root", yLow, yHigh);
		}
