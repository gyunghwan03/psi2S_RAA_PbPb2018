...
void mc_mass_wide(float ptLow = 6.5, float ptHigh = 50, float yLow = 0, float yHigh = 1.2, bool isSkipFit = false)
{
	cout << "=== start mc_mass_wide() ===\n";
  TStopwatch time;
  time.Start();
  ...
  TString rootDir = Form("mc_roots%s/pT%.1f_%.1f_y%.1f_%.1f", userLabel.Data(), ptLow, ptHigh, yLow, yHigh);
  ...
  RooRealVar massMean("massMean", "", 3.096, 3.08, 3.102);
  RooRealVar massSigma1("massSigma1", "CB sigma", 0.08, 0.00001, 0.1);
  // RooRealVar massSigma2("massSigma2", "CB sigma", 0.12, 0.01, 2);
  RooRealVar massSigma12("massSigma12", "ratio of sigma 1 vs 2", 0.06, 0.01, 0.1);
  RooFormulaVar massSigma2("massSigma2", "@0*@1", {massSigma1, massSigma12});

  RooRealVar alphaL("alphaL", "", 1.5, 0.01, 10.0);
  RooRealVar nL("nL", "nL", 3.0, 1, 20.0);

	// RooRealVar alphaR("alphaR", "", -1, -100, 100.0);
  // RooRealVar nR("nR", "", 2.0, 1, 20.0);
  
  RooRealVar alphaLR("alphaLR", "", 1.0, -10, 10.0);
  RooFormulaVar alphaR("alphaR", "@0*@1", {alphaL, alphaLR});
  RooRealVar nLR("nLR", "", 20.0, 1.0, 200.0);
  ...
  RooRealVar fCB1("fCB1", "frac(Gauss in total)", 0.95, 0.01, 1.0);

  RooRealVar fCB2("fCB2", "frac(Gauss in total)", 0.20, 0.00, 1.00);
	
  
	RooCBShape CB1("CB1", "left-tail CB", *ws->var("mass"), massMean, massSigma1, alphaL, nL);

  RooCBShape CB2("CB2", "left-tail CB", *ws->var("mass"), massMean, massSigma12, alphaR, nR);

  RooAddPdf DCB("DCB", "", RooArgList(CB1, CB2), RooArgList(fCB2));

  // Gauss
  RooRealVar massSigma1G("massSigma1G", "ratio of sigma1 vs sigmaG", 1.4, 1, 5);
  //RooRealVar massSigma1G("massSigma1G", "ratio of sigma1 vs sigmaG", 0.001, 0.00001, 1);
  // RooFormulaVar massSigmaG("massSigmaG", "@0*@1", {massSigma1, massSigma1G});
  
  RooRealVar massSigmaG("massSigmaG", "CB sigma", 0.2, 0.1, 0.3);
  ...
  // --- model ---
  RooRealVar nSigMass("nSigMass", "mass signal yield", 1e6, 1e3, 1e8);
  ...
  if (isSkipFit && !gSystem->AccessPathName(Form("%s/mc_mass_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
	...
	else fitMcMass = massFitModel.fitTo(*redData, Offset(true), Save(), Strategy(1), PrintLevel(-1), PrintEvalErrors(-1),  NumCPU(28) ); //, 
  ...
  { return fr ? fr->findObject(n) : nullptr; };
  {
    ...
    // --- CMS/info latex ---
    {
      ...
      tx.DrawLatex(x, y0 + dy * k++, "MC 2023, J/#psi #rightarrow #mu^{+}#mu^{-}");
      if (yLow == 0)
