		...
		)
{
	...
	TFile* f1 = new TFile("../../../../skimmedFiles/OniaRooDataSet_isMC1_PR_JPsi_211012_NcollWeight.root", "read");
    ...
    double paramsupper[8] = {0.4,       3,      10,     10,   1.0,    25.0};
    ...
    double N_Jpsi_high = 5e+8; // 2500000
	double N_Bkg_high = 1e+7;
    double fit_limit = 3.32;
	double fit_lower = massLow;

	// if(ptLow==6.5&&ptHigh==9) N_Jpsi_high = 5000000;
	// if(ptLow==3.5&&ptHigh==50) N_Jpsi_high = 500000; f_init=0.4; sigma_1_init = 0.05; n_1_init = 2; x_init = 2; alpha_1_init = 2;
    ...
    else if(ptLow==3.5&&ptHigh==40&&cLow==100) {
        x_init = 2.5;
        fit_limit = 3.28;
        N_Jpsi_high = 1e+7;
    }
    ...
    if(ptLow==3.5 && ptHigh==50 && cHigh == 180) { 
     ...
     if(ptLow==3.5 && ptHigh==50 && cHigh == 80) { 
            N_Jpsi_high = 500000;
            f_init=0.6; sigma_1_init = 0.03; n_1_init = 2;
                       x_init = 2; alpha_1_init = 2;
     }
        if(ptLow==3.5 && ptHigh==50 && cHigh == 100) { 
            N_Jpsi_high = 100000;
            f_init=0.4; sigma_1_init = 0.025; n_1_init = 0.5;
                       x_init = 1; alpha_1_init = 1.5;
     }
             if(ptLow==3.5 && ptHigh==50 && cLow == 80 && cHigh == 180) { 
            N_Jpsi_high = 100000;
            f_init=0.4; sigma_1_init = 0.025; n_1_init = 0.5;
                       x_init = 1; alpha_1_init = 1.5;
     }
                if(ptLow==3.5 && ptHigh==50 && cLow == 100 && cHigh == 180) { 
            N_Jpsi_high = 100000;
            f_init=0.8; sigma_1_init = 0.025; n_1_init = 0.5;
                       x_init = 2; alpha_1_init = 2;
     }
                    if(ptLow==3.5 && ptHigh==50 && cLow == 100 && cHigh == 180) { 
            N_Jpsi_high = 100000;
            f_init=0.8; sigma_1_init = 0.015; n_1_init = 0.5;
                       x_init = 2; alpha_1_init = 2;
     }
                         if(ptLow==6.5 && ptHigh==50 && cLow == 80 && cHigh == 100) { 
            N_Jpsi_high = 50000;
            f_init=0.2; sigma_1_init = 0.01; n_1_init = 0.5;
    ...
    RooFitResult* fitMass = ws->pdf("pdfMASS_Tot")->fitTo(*dsAB,Save(), Hesse(kTRUE), Timer(kTRUE), Extended(kTRUE), SumW2Error(isWeighted), Range(fit_lower, fit_limit), NumCPU(nCPU));
    ...
    */
    ...
    ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A,Name("pdfMASS_tot"), LineColor(kBlack), Range(fit_lower, fit_limit));
    ...
    ws->pdf("cball_1_A")->plotOn(myPlot2_A,Name("cball_1_A"), LineColor(kBlue+2), Normalization(fitFraction.getVal()), Range(fit_lower, fit_limit));
    ws->pdf("gauss")->plotOn(myPlot2_A,Name("gauss"), LineColor(kGreen+2), Normalization(1-fitFraction.getVal()), Range(fit_lower, fit_limit));
    ...
    TLine *line = new TLine(fit_limit,0,fit_limit,Ydown*TMath::Power((Yup/Ydown),0.4));
    line->SetLineStyle(2);
    line->SetLineColor(kBlack);
    line->SetLineWidth(3);
    myPlot2_A->addObject(line);
