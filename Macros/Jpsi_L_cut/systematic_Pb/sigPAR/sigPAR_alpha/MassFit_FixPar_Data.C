...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  kineCut = Form("pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>2.5 && mass<3.5 && cBin>=%d && cBin<%d",ptLow, ptHigh, yLow, yHigh, cLow, cHigh);
  ...
  double low=0.; double high=10.;
  double slow = sigma_lower; double shigh = sigma_higher;
  ...
  //else if(ptLow==3.5&ptHigh==50&&cLow==20&&cHigh==40) { slow=sigma_lower; shigh=sigma_higher;}
  //else {low=0; high=10; slow=0; shigh=0.12;}

  double paramslower[6] = {alpha_lower, 0., slow, 0., 0., 0.};
  double paramsupper[6] = {alpha_higher, 10 , shigh, 1, 1.0, 25.0};
  ...
  if(ptLow==3.5&&ptHigh==6.5) { NJpsi_limit = 1e+6; NBkg_limit = 2e+7; }
...
//	  NJpsi_limit = 1700; }
  ...
  //else if(ptLow==3.5&&ptHigh==50&&cLow==0&&cHigh==20) { pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(*pdfMASS_Jpsi, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg)); }
  ...
  //drawText(Form("#sigma1_{J/#Psi} = %.2f #pm %.2f MeV/c^{2}, (#sigma2/#sigma1)_{J/#Psi} = %.3f #pm %.3f",(ws->var("sigma_1_A")->getVal())*1000,(ws->var("sigma_1_A")->getError())*1000,ws->var("x_A")->getVal(),ws->var("x_A")->getError()),text_x,text_y-y_diff*6,text_color,text_size);
