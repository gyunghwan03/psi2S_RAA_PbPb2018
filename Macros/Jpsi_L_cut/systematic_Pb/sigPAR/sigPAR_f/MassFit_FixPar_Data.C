...
//  f1 = new TFile(Form("../../skimmedFiles/v2Cut_Nom/OniaRooDataSet_isMC0_Psi2S_%s_m3.3-4.1_OS_Effw%d_Accw%d_PtW%d_TnP%d_221013_root618.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  f2 = new TFile(Form("../../../../../ctau_eff/roots_1S_Pb/decayL/%sMC/decay_hist_%s_m2.6-3.5.root", fname.Data(), kineLabel.Data()));
...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  TFile * f_fit = new TFile(Form("../../../../Jpsi_250423/roots_MC/Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  low=0; high=10; slow=0; shigh=0.15;

  double paramslower[6] = {low, 0., 0, 0., f_lower, 0.};
  double paramsupper[6] = {high, 10 , 0.13, 1, f_higher, 25.0};

  //if(ptLow==3.5&cLow==0&&cHigh==20) { paramslower[4] = 0.26; paramsupper[4] = 0.28;}
  //else if (ptLow==3.5&&cLow==20&&cHigh==40) { paramslower[2]=sigma_lower; paramsupper[2]=6.231e-2; cout << "HERE" << endl;}
  //else if (ptLow==3.5&&ptHigh==6.5) { slow = sigma_lower; shigh= sigma_higher; }
  if (ptLow==15&&ptHigh==20) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher;}
  else if (ptLow==25&&ptHigh==40) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher;}
  ...
  //SIGNAL FIX
  RooRealVar    mean("m_{J/#Psi}","mean of the signal gaussian mass PDF",pdgMass.Psi2S, pdgMass.JPsi -0.1, pdgMass.JPsi + 0.1 ) ;
  ...
  if(ptLow==20&&ptHigh==25) { NJpsi_lower = 1000.; }
  else if(ptLow==12&&ptHigh==15) { NJpsi_limit = 5e+4; NJpsi_lower = 2000; NBkg_limit=5000;}
  else if(ptLow==15&&ptHigh==20) { NJpsi_limit = 1e+4; NJpsi_lower = 2000; NBkg_limit=5000;}
  else if(ptLow==25&&ptHigh==40) { NJpsi_lower = 500; NBkg_limit=1000;}
  ...
  if(ptLow==15&&ptHigh==20) { RooProdPdf model("model","model with constraint",RooArgSet(*pdfMASS_Jpsi,RooArgSet(f_constraint,sigma1_constraint))); }
  else if(ptLow==25&&ptHigh==40) { RooProdPdf model("model","model with constraint",RooArgSet(*pdfMASS_Jpsi,RooArgSet(f_constraint,sigma1_constraint))); }
  else RooProdPdf model("model","model with constraint",RooArgSet(*pdfMASS_Jpsi,RooArgSet(f_constraint)));
  //RooProdPdf model("model","model with constraint",RooArgSet(*pdfMASS_Jpsi,RooArgSet(f_constraint,sigma1_constraint)));
  RooAddPdf* pdfMASS_Tot;
  //if(ptLow==3.5&&ptHigh==5) { pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(*pdfMASS_Jpsi, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg)); }
  if(ptLow==3.5&&ptHigh==5&&cLow==0&&cHigh==20) { pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(*pdfMASS_Jpsi, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg)); }
  ...
  //drawText(Form("f_{#Psi} = %.4f (fixed)",ws->var("f")->getVal()),text_x,text_y-y_diff*4,text_color,text_size);
  drawText(Form("f_{J/#Psi} = %.4f #pm %.4f",ws->var("f")->getVal(),ws->var("f")->getError()),text_x,text_y-y_diff*4,text_color,text_size);
  ...
  //Free
  //drawText(Form("m_{#Psi} = %.4f #pm %.4f",ws->var("m_{J/#Psi}")->getVal(),ws->var("m_{J/#Psi}")->getError()),text_x,text_y-y_diff*2,text_color,text_size);
  //drawText(Form("#alpha_{#Psi} = %.4f #pm %.4f",ws->var("alpha_1_A")->getVal(),ws->var("alpha_1_A")->getError()),text_x,text_y-y_diff*3,text_color,text_size);
  //drawText(Form("f_{#Psi} = %.4f #pm %.4f",ws->var("f")->getVal(),ws->var("f")->getError()),text_x,text_y-y_diff*4,text_color,text_size);
  //drawText(Form("n_{#Psi} = %.4f #pm %.4f",ws->var("n_1_A")->getVal(),ws->var("n_1_A")->getError()),text_x,text_y-y_diff*5,text_color,text_size);
  //drawText(Form("#sigma1_{#Psi} = %.2f #pm %.2f MeV/c^{2}, (#sigma2/#sigma1)_{#Psi} = %.3f #pm %.3f",(ws->var("sigma_1_A")->getVal())*1000,(ws->var("sigma_1_A")->getError())*1000,ws->var("x_A")->getVal(),ws->var("x_A")->getError()),text_x,text_y-y_diff*6,text_color,text_size);
  //drawText(Form("(#sigma2/#sigma1)_{#Psi} = %.3f (fixed)",ws->var("x_A")->getVal()),text_x,text_y-y_diff*7,text_color,text_size);
