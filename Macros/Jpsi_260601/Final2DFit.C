...
//  ctauErrMax = 0.06;}//0.124872
  ...
  RooDataSet *dsTot = (RooDataSet*)datasetW->reduce(*argSet, Form("ctau3DErr>=%.6f&&ctau3DErr<=%.6f&&%s", ctauErrMin, ctauErrMax, kineCut.Data()) );
  ...
  if(ptLow==6.5&&cLow==100&&cHigh==180) ws->factory("b_Jpsi[0.37, 0.27, 0.4]");
  else if(ptLow==6.5&&ptHigh==9&&yLow==0) ws->factory("b_Jpsi[0.27, 0.25, 0.3]");
  else if(ptLow==3&&ptHigh==4) ws->factory("b_Jpsi[0.3, 1e-8, 1.0]");//260608: freed from artificial [0.13,0.15] range; free min=0.150+/-0.003, chi2/ndof 213/57 (was 0.137 constrained)
  else ws->factory("b_Jpsi[0.52, 1e-8, 1.0]");//NP fraction for Sig
  ...
  // NOTE: A previous version forced numerical integration on all RooDecay
  // components here (forceNumInt(kTRUE)) to guard against an exp() overflow in
  // the analytical erfc convolution formula. In ROOT 6.18 this made each
  // per-event conditional convolution so slow that MIGRAD never returned from
  // its first NLL evaluation (>500 s hang, fit effectively dead).
  // For this analysis' resolution/lambda regime the analytical convolution does
  // NOT overflow, so the forced numerical integration is removed: the fit then
  // converges cleanly (MINIMIZE=0) in well under a few minutes.
