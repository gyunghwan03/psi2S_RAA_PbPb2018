    ...
    )
{
  ...
  else if(ptLow==3.&&ptHigh==40) fCTrue = new TFile(Form("../../roots/2DFit_%s/CtauTrue/CtauTrueResult_Inclusive_pt3.0-40.0_y1.6-2.4_muPt0.0_centrality0-180.root",DATE.Data()));
  ...
  else if(ptLow==6.5&&ptHigh==30) fCTrue = new TFile(Form("../../roots/2DFit_%s/CtauTrue/CtauTrueResult_Inclusive_pt6.5-30.0_y0.0-1.6_muPt0.0_centrality0-180.root",DATE.Data()));
...
//  ctauErrMax = 0.06;}//0.124872
  ...
  if(ptLow==6.5 && ptHigh==40 && cLow==100) ws->factory("b_Jpsi[0.371, 0.36, 0.38]");//NP fraction for Sig
  //if(ptLow==6.5 && ptHigh==40 && cLow==0 && cHigh==20) ws->factory("b_Jpsi[0.38, 0.36, 0.43]");//NP fraction for Sig
  if(ptLow==6.5 && ptHigh==40 && cLow==0 && cHigh==180) ws->factory("b_Jpsi[0.39, 0.36, 0.4]");//NP fraction for Sig
  if(ptLow==6.5&&ptHigh==9&&yLow==0) ws->factory("b_Jpsi[0.29, 0.28, 0.32]");
  if(ptLow==6.5&&ptHigh==9&&yLow==1.6) ws->factory("b_Jpsi[0.25, 0.23, 0.28]");
  if(ptLow==3&&cLow==100&&cHigh==180) ws->factory("b_Jpsi[0.212, 0.21, 0.23]");
  if(ptLow==3.5&&ptHigh==6.5) ws->factory("b_Jpsi[0.20, 0.17, 0.2]");
