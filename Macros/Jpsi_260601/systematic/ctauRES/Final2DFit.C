    ...
    )
{
  ...
  if(ptLow==3.5&&ptHigh==40) fCTrue = new TFile(Form("../../roots/2DFit_%s/CtauTrue/CtauTrueResult_Inclusive_pt3.5-50.0_y1.6-2.4_muPt0.0_centrality0-180.root",DATE.Data()));
  else if(ptLow==6.5&&ptHigh==40) fCTrue = new TFile(Form("../../roots/2DFit_%s/CtauTrue/CtauTrueResult_Inclusive_pt6.5-50.0_y0.0-1.6_muPt0.0_centrality0-180.root",DATE.Data()));
...
//  ctauErrMax = 0.06;}//0.124872
  ...
  ws->factory(Form("lambdaDSS_test3[%.4f, %.4f, %.4f]", lambda2, 1e-8, 1.));
  ...
  else if(ptLow==6.5 && ptHigh==40 && cLow==100) ws->factory("b_Jpsi[0.371, 0.36, 0.38]");//NP fraction for Sig
  else if(ptLow==3.5&&ptHigh==6.5) ws->factory("b_Jpsi[0.11, 0.15, 0.2]"); 
  else if(ptLow==3.5&&cLow==0&&cHigh==20) ws->factory("b_Jpsi[0.22, 0.1, 0.27]"); 
  else if(ptLow==6.5&&ptHigh==9&&yLow==0) ws->factory("b_Jpsi[0.291, 0.28, 0.32]");
  else if(ptLow==6.5 && ptHigh==40 && cLow==0 && cHigh==180) ws->factory("b_Jpsi[0.382, 0.36, 0.4]");//NP fraction for Sig
