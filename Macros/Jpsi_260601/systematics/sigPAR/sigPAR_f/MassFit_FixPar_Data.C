...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  double paramslower[6] = {low, 0., 0, 0., 0., 0.};
  double paramsupper[6] = {high, 10 , 0.15, 1, 1.0, 25.0};

  if(ptLow==6.5&&ptHigh==9&&yLow==1.6) {paramslower[4] = f_lower; paramsupper[4] = f_higher;}
  else if(ptLow==3.5&cLow==20&&cHigh==60) { paramslower[4] = f_lower; paramsupper[4] = f_higher;}
  ...
  //Build the model
  Double_t NBkg_limit = 1.0e+06;
  Double_t NJpsi_limit = 1.0e+06;
  Double_t NJpsi_lower = 0.;

  if(ptLow==3.5&&cLow==100&&cHigh==180) { NJpsi_limit = 1.0e+5; NBkg_limit = 1.0e+6; }
  else if(ptLow==6.5&&ptHigh==9&&yLow==1.6) { NJpsi_limit = 1.0e+5; } 
  else if(ptLow==6.5&&cLow==0&&cHigh==20) { NJpsi_limit = 5.0e+4; } 
  else if(ptLow==6.5&&cLow==20&&cHigh==40) { NJpsi_limit = 5.0e+4; } 
  else if(ptLow==6.5&&cLow==40&&cHigh==60) { NJpsi_limit = 5.0e+4; NBkg_limit=1e+5; } 
  else if(ptLow==6.5&&cLow==80&&cHigh==100) { NJpsi_limit = 5.0e+4; NBkg_limit=1e+5; } 
  else if(ptLow==6.5&&cLow==100&&cHigh==180) { NJpsi_limit = 5.0e+4; NBkg_limit=1e+5; } 
  else if(ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==180) { NJpsi_limit = 2e+5;}
  else if(ptLow==6.5&&ptHigh==40&&cLow==0&&cHigh==180) { NJpsi_limit = 2e+5;}
  else if (ptLow==3.5&&ptHigh==6.5&&cHigh==180) { NBkg_limit = 1e+6; NJpsi_limit = 9e+4; }
  ...
  else if(ptLow==6.5&&ptHigh==9&&yLow==1.6) { pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(*pdfMASS_Jpsi, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg)); }
  //else if(ptLow==3.5&&ptHigh==6.5&&yLow==1.6) { pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(*pdfMASS_Jpsi, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg)); }
