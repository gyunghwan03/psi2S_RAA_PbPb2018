...
void draw_Bfraction_pp_y0_2p4()
{
	...
	double var_HIN[11] = {0.205,0.228,0.262,0.295,0.343,0.391,0.442,0.484,0.515,0.574,0.576};
	double err_HIN[11] = {0.020,0.017,0.020,0.017,0.018,0.021,0.022,0.027,0.026,0.039,0.056};
	...
	leg->SetTextFont(43);
	leg->SetBorderSize(0);
	leg->AddEntry(gfracPP,"pp (2018 DATA), |y|<2.4","PE");
	leg->AddEntry(gfracHIN,"pp 5.02 TeV (HIN-16-025), |y| < 2.4","P");
	leg->Draw("SAME");

	//CMS_lumi_v2mass(c1,iPeriod,iPos);

	c1->SaveAs("figs/Bfraction_pp_y0_2p4.pdf");
...
valErr getFrac_psi2S_pp(int i) {
	...
	TFile* inf = new TFile(Form("roots/2DFit_No_Weight/Final/final_b/2DFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", kineLabel[i].Data()));
