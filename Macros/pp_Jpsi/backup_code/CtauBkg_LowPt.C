    ...
    )
{
  ...
  ws->factory("lambdaDF_Bkg1[0.7, 1e-6, 1]");
  ws->factory("lambdaDF_Bkg2[0.16, 1e-6, 1]");
  ws->factory("lambdaDSS_Bkg1[0.45, 1e-6, 1.]");
  ws->factory("lambdaDSS_Bkg2[0.11, 1e-6, 1.]");
  ws->factory("fDSS12[0.3, 0., 1.]");
  ws->factory("fDF12[0.3, 0., 1.]");}
  ...
  ws->factory("fDFSS[0.87, 0., 1.]");
  ws->factory("fDLIV[0.52, 0., 1]");
  ws->factory("lambdaDDS_Bkg[0.95, 1e-6, 1.]");
  ws->factory("lambdaDF_Bkg1[0.1, 1e-6, 1]");
  ws->factory("lambdaDF_Bkg2[0.5, 1e-6, 1]");
  ws->factory("lambdaDSS_Bkg1[0.45, 1e-6, 1.]");
...
//  if(ptLow==3&&ptHigh==6.5) {ctauMin=-2.; ctauMax=3.65;}
  ...
  else if(ptLow==12&&ptHigh==40) { ctauMin = -1.2; ctauMax = 4; }
  else if(ptLow==25&&ptHigh==40) ctauMin = -2.;
