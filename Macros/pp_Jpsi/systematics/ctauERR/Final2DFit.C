    ...
    )
{
  ...
  RooProdPdf pdfJpsiPR("pdfCTAU_JpsiPR","",*ws->pdf("pdfCTAUERR_Tot"),
      Conditional(*ws->pdf("pdfCTAUCOND_JpsiPR"),RooArgList(*ws->var("ctau3D"))));
  ws->import(pdfJpsiPR);
  RooProdPdf pdfJpsiNoPR("pdfCTAU_JpsiNoPR","",*ws->pdf("pdfCTAUERR_Tot"),
  ...
  //double normBkg = ws->data("dsToFit")->sumEntries()*normDSTot/ws->data("dataw_Tot")->sumEntries();
