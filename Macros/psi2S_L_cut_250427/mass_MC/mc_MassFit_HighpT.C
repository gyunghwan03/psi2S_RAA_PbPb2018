		...
		)
{
    ...
    gSystem->mkdir(Form("roots_Pb"));
    gSystem->mkdir(Form("roots_Pb/PRMC"),kTRUE);
    gSystem->mkdir(Form("roots_Pb/NPMC"),kTRUE);
    gSystem->mkdir(Form("figs_Pb"));
    gSystem->mkdir(Form("figs_Pb/PRMC"),kTRUE);
    gSystem->mkdir(Form("figs_Pb/NPMC"),kTRUE);
    ...
    // MC
    TFile* f1;
    if (PRw==1) f1 = new TFile("../../../skimmedFiles/OniaRooDataSet_miniAOD_isMC1_Psi2S_Prompt_cent0_200_Effw0_Accw0_PtW0_TnP0_230822.root", "read");
    else if (PRw==2)  f1 = new TFile("../../../skimmedFiles/OniaRooDataSet_miniAOD_isMC1_Psi2S_NonPrompt_cent0_200_Effw0_Accw0_PtW0_TnP0_230601.root", "read");
	...
	double sigma_up=0.13; double sigma_lo=0; double x_up=5; double x_lo=1.; double alpha_up=5; double alpha_lo=0; double n_up=5; double n_lo=0; double f_up=1; double f_lo=1e-6;
    ...
    double fit_limit = 3.82;

    if(PRw==2) {
    if(ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==20) {n_up = 130; fit_limit = 3.82;}
    else if(ptLow==3.5&&ptHigh==40&&cLow==20&&cHigh==60) {n_up = 15; x_up = 7.; fit_limit=3.82;}
    else if(ptLow==3.5&&ptHigh==40&&cLow==60&&cHigh==100) {n_up = 10; x_up = 3.; fit_limit=3.82;}
    else if(ptLow==3.5&&ptHigh==40&&cLow==100&&cHigh==180) {n_up = 15; x_up = 5.; fit_limit=3.84;}
    else if(ptLow==3.5&&ptHigh==6.5) {n_up = 10; x_up = 3.; fit_limit=3.82;}
    else if(ptLow==6.5&&ptHigh==9&&yLow==0) {n_lo = 1; n_up = 10; f_init = 0.3; x_lo=0.1; x_up = 1.; fit_limit=3.88;}
    else if(ptLow==6.5&&ptHigh==9) {n_lo = 1; n_up = 10; f_init = 0.3; x_up = 5.; fit_limit=3.82;}
    else if(ptLow==6.5&&cLow==0&&cHigh==20) { x_init = 3.32; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==20&&cHigh==40) { x_init = 3.32; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==40&&cHigh==60) { n_up = 10; x_init = 1.3; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==60&&cHigh==80) { n_up = 10; x_init = 1.3; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==80&&cHigh==100) { n_up = 10; x_init = 1.3; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==100&&cHigh==180) { n_up = 10; x_init = 1.3; fit_limit = 3.88;}
    else if(ptLow==15&&ptHigh==20) { x_init = 1.9; fit_limit = 3.86;}
    else if(ptLow==20&&ptHigh==25) { x_init = 2.1; x_up = 7; fit_limit = 3.89;}
    else if(ptLow==25&&ptHigh==40) { x_init = 2.1; x_up = 7; fit_limit = 3.86;}
    else if(ptLow==12&&ptHigh==40) { x_init = 2.1; x_up = 7; fit_limit = 3.88;}
    else if(yLow==0) fit_limit = 3.88; }

    else if(PRw==1) {
    if(ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==20) {n_up = 130; fit_limit = 3.84;}
    else if(ptLow==3.5&&ptHigh==40&&cLow==20&&cHigh==60) {n_up = 15; x_up = 3.; fit_limit=3.82;}
    else if(ptLow==3.5&&ptHigh==40&&cLow==60&&cHigh==100) {n_up = 10; x_up = 3.; fit_limit=3.82;}
    else if(ptLow==3.5&&ptHigh==6.5) {n_up = 10; x_up = 5.; fit_limit=3.84;}
    else if(ptLow==6.5&&ptHigh==9) {n_lo = 1; n_up = 10; f_init = 0.3; x_up = 5.; fit_limit=3.82;}
    else if(ptLow==6.5&&cLow==0&&cHigh==20) { x_init = 1.1; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==20&&cHigh==40) { x_init = 3.32; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==40&&cHigh==60) { n_up = 10; x_init = 1.3; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==60&&cHigh==80) { n_up = 10; x_init = 1.3; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==80&&cHigh==100) { n_up = 10; x_init = 1.3; fit_limit = 3.88;}
    else if(ptLow==6.5&&cLow==100&&cHigh==180) { N_Jpsi_high = 1e+7; n_up = 10; x_init = 2.5; fit_limit = 3.88;}
    else if(ptLow==9&&ptHigh==12&&yLow==0) { n_up = 10; x_init = 2.8; x_lo = 1.5; x_up = 7; fit_limit = 3.88;}
    else if(ptLow==12&&ptHigh==15) { x_init = 2.1; x_lo = 1.5; x_up = 7; fit_limit = 3.89;}
    else if(ptLow==15&&ptHigh==20) { x_init = 1.9; x_lo = 1.5; x_up = 7; fit_limit = 3.86;}
    else if(ptLow==20&&ptHigh==25) { x_init = 2.1; x_lo = 1.5; x_up = 7; fit_limit = 3.89;}
    else if(ptLow==25&&ptHigh==40) { x_init = 2.1; x_lo = 1.5; x_up = 7; fit_limit = 3.86;}
    else if(yLow==0) fit_limit = 3.88; }
    /*
	if(ptLow==6.5&&ptHigh==9&&yLow==1.6&&PRw==2) { N_Jpsi_high = 5000000; n_up=10; f_init = 0.1; }
	else if(ptLow==6.5&&ptHigh==9&&yLow==1.6&&PRw==1) { N_Jpsi_high = 5000000; n_up=10; f_init = 0.2; }
	else if(ptLow==9&&ptHigh==12&&yLow==0&&PRw==2) { N_Jpsi_high = 1e+6; n_up=10; f_init = 0.1;}
	else if(ptLow==9&&ptHigh==12&&yLow==1.6&&PRw==1) { N_Jpsi_high = 1e+6; n_lo = 1.; n_up=10; f_init = 0.1;}
	else if(ptLow==9&&ptHigh==12&&yLow==1.6&&PRw==2) { N_Jpsi_high = 1e+6; n_lo = 1.; n_up=10; f_init = 0.4;}
	else if(ptLow==12&&ptHigh==50) N_Jpsi_high = 1e+7;
	else if(ptLow==3.5&&ptHigh==6.5) { N_Jpsi_high = 1e+7; n_up=150; n_lo=1.; f_init=0.33; x_init=0.1; sigma_up = 0.3; n_1_init = 3.8; alpha_up = 2; alpha_lo = .1;}
	...
	else if(ptLow==3.5&&cLow==0&&cHigh==20&&PRw==1) {N_Jpsi_high = 1e+8; n_up=10.; f_lo=1e-6; x_up=0.7; n_1_init = 6.; alpha_1_init = 3.; sigma_1_init = 0.07; sigma_up = 0.08;}

	else if(ptLow==3.5&&cLow==0&&cHigh==20&&PRw==2) {N_Jpsi_high = 2e+8; alpha_lo=1; alpha_1_init = 3; n_lo=.1; n_up=10.; f_lo=0.01; f_up=.9; f_init = 0.24; x_lo = 0.1; x_up=.8; x_init = 0.4; n_1_init = 1.5; alpha_1_init = 10.; sigma_1_init = 0.05; sigma_lo = 0.048; sigma_up = 0.052; x_init = 0.11; fit_limit = 3.8;}
	
    else if(ptLow==3.5&&cLow==0&&cHigh==60) {N_Jpsi_high = 1e+8; n_up=15; f_lo=0.01; x_up=0.7; n_1_init = 7.; sigma_up = 0.13;}
	else if(ptLow==3.5&&cLow==20&&cHigh==60&&PRw==1) {N_Jpsi_high = 1e+8; n_lo = 1; n_up=10.; f_lo=0.01; f_init = 0.04; x_up=0.9; n_1_init = 7.; sigma_up = 0.7;}
	else if(ptLow==3.5&&cLow==20&&cHigh==60&&PRw==2) {N_Jpsi_high = 1e+8; n_lo = 1; n_up=10.; f_lo=0.01; f_init = 0.24; x_up=0.7; n_1_init = 7.; sigma_up = 0.7;}
	else if(ptLow==6.5&&cLow==100&&cHigh==180) { N_Jpsi_high = 1e+6; f_lo=0.1; }
	else if(ptLow==6.5&&cLow==0&&cHigh==20&&PRw==1) { N_Jpsi_high = 5e+6; f_lo=0.1; f_init = 0.2; n_lo = 1.; n_up = 10; n_1_init = 3;}
	else if(ptLow==6.5&&cLow==20&&cHigh==40&&PRw==2) { N_Jpsi_high = 5e+6; f_lo=0.1; }
	else if(ptLow==6.5&&cLow==40&&cHigh==60&&PRw==1) { N_Jpsi_high = 1e+6; f_lo=0.1; n_up = 10; n_lo=1; }
	else if(ptLow==6.5&&cLow==40&&cHigh==60&&PRw==2) { N_Jpsi_high = 1e+6; f_lo=0.1; n_up = 10; n_lo=1; }
	else if(ptLow==6.5&&cLow==60&&cHigh==80&PRw==1) { N_Jpsi_high = 1e+6; f_lo=0.1; n_up = 10; n_lo=1; }
	else if(ptLow==6.5&&cLow==60&&cHigh==80&PRw==2) { N_Jpsi_high = 1e+6; f_lo=0.1; n_up = 10; n_lo=1; }
    else if(ptLow==6.5&&ptHigh==40) N_Jpsi_high = 1e+7;
    */
    ...
    if(ptLow==6.5&&ptHigh==40&&cLow==20&&cHigh==30) {sigma_up=0.3;}
    ...
    */
    ...
    TString kineLabel = getKineLabel (ptLow, ptHigh,yLow, yHigh, 0.0, cLow, cHigh);
        ...
        fitMass->Print("v");
    TFile* outFile;
    outFile = new TFile(Form("roots_Pb/%sMC/mc_MassFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", fname.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP),"recreate");
    c_A->SaveAs(Form("figs_Pb/%sMC/mc_Mass_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", fname.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
