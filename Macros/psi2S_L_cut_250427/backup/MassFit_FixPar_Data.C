    ...
    int PRw = 1, bool fEffW = false, bool fAccW = false, bool isPtW = false, bool isTnP = false)
{
  ...
  if(ptLow==3.5) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher;}
  else if(ptLow==20) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher;}
  else if(ptLow==25) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher;}
  else if(ptLow==6.5&&cLow==60&&cHigh==80) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher; }
  else if(ptLow==6.5&&cLow==80&&cHigh==100) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher; }
  else if(ptLow==6.5&&ptHigh==9&&yLow==0) { paramslower[2] = 0.05;}
  ...
  if (cLow==0&&cHigh==40)  {
	  NBkg_limit = 500000;
	  NJpsi_limit = 10000; }
  if (ptLow==6.5 && ptHigh==40 && cLow==80&&cHigh==100)  {
	  NBkg_limit = 50000;
	  NJpsi_limit = 200; }
  ...
  if (ptLow==6.5 && ptHigh==12 && cLow==0&&cHigh==180)  {
	  NBkg_limit = 50000;
	  NJpsi_limit = 1000; }
  if (ptLow==12 && ptHigh==40 && cLow==0&&cHigh==180)  {
	  NBkg_limit = 100000;
	  NJpsi_limit = 5000; }
  if(ptLow==3.5 && ptHigh==40 && cLow==100 && cHigh==180) {
	  NJpsi_limit = 500;
	  NJpsi_low = 100;
	  NBkg_limit = 5000; }
  ...
  //if(ptLow==3.5&&cLow==100) {pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(*pdfMASS_Jpsi, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg)); cout << "HERE" << endl;}
...
//  ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A,VisualizeError(*fitMass,1),FillColor(kOrange));
  ...
  if(ptLow==3.5) {
      Ydown = YMin/(TMath::Power((YMax/YMin), (0.1/(1.0-0.1-0.4))));
      Yup = YMax*TMath::Power((YMax/YMin), (0.8/(1.0-0.1-0.4)));
  }
