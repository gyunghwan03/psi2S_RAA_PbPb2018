    ...
    int PRw = 1, bool fEffW = false, bool fAccW = false, bool isPtW = false, bool isTnP = false)
{
  ...
  TFile *f_fit = new TFile(Form("../pp_psi2S_230512/roots_MC/Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabel.Data(), fEffW, fAccW, isPtW, isTnP));
  ...
  if(mc_type==1) drawText(Form("l_{#psi(2S)} < %.4f", l_cut),text_x,text_y-y_diff*7,text_color,text_size);
  else drawText(Form("l_{#psi(2S)} > %.4f", l_cut),text_x,text_y-y_diff*7,text_color,text_size);
