    ...
    bool fEffW = false, bool fAccW = false, bool isPtW = false, bool isTnP = false)
{
  ...
  gSystem->mkdir(Form("roots/%sMC",fname.Data()),kTRUE);
  gSystem->mkdir(Form("figs/%sMC",fname.Data()),kTRUE);
  ...
  f1 = new TFile(Form("../../../../skimmedFiles/OniaRooDataSet_miniAOD_isMC0_Psi2S_cent0_200_Effw0_Accw0_PtW0_TnP0_HFDown_230523.root"));
  ...
  TFile * f_fit = new TFile(Form("../../../psi2S_250423/roots_MC/Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  //TFile *f_fit;
  //if (PRw == 1 && PRw == 2)
  //{
  ...
  if (ptLow == 3.5 && cLow == 0 && cHigh == 20 && PRw == 2)
    ...
    sigma_index = 1;
  else if(ptLow==9&&ptHigh==12&&yLow==0&&PRw==1) sigma_index=1;
  else if (ptLow==20&&ptHigh==25&&PRw==2) sigma_index=1;
  else if (ptLow==6.5 && ptHigh==40 && cLow==20 && cHigh==40 ) sigma_index=4;
  else if (ptLow==6.5 && ptHigh==40 && cLow==40 && cHigh==60 ) sigma_index=1;
  else if (ptLow==6.5 && ptHigh==40 && cLow==80 && cHigh==100 ) sigma_index=1;
  else if (ptLow==6.5 && ptHigh==40 && cLow==40 && cHigh==60 && PRw==2 ) sigma_index=5;
  else if (ptLow==6.5 && ptHigh==40 && cLow==60 && cHigh==80 && PRw==2 ) sigma_index=5;
  ...
  else if (ptLow == 6.5 && cLow == 20 && cHigh == 4) 
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  ...
  else if (ptLow == 9 && ptHigh == 12 && yLow == 0 && PRw == 2)
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  ...
  //  paramsupper[2] = 0.14;
  //}
  //else if(ptLow==6.5&&ptHigh==40&&cLow==100&&cHigh==180) 
  //{
  //  cout << "HERE" << endl;
  //  paramslower[2] = sigma_lower;
  //  paramsupper[2] = sigma_higher;
  ...
  else if(ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==180) 
  {
    cout << "HERE" << endl;
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  ...
  else if(ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==180&&PRw==2)
  {
    NJpsi_limit = 2500;
    NJpsi_lower = 1000;
    NBkg_limit = 1e+6;
  ...
  else if (ptLow == 6.5 && ptHigh==40 && cLow==20 && cHigh==40 && PRw==2) 
  {
    pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot", "Jpsi + Bkg", RooArgList(model, *pdfMASS_bkg), RooArgList(*N_Jpsi, *N_Bkg));
  }
  else if (ptLow == 6.5 && ptHigh==40 && cLow==40 && cHigh==60 && PRw==2) 
  {
    pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot", "Jpsi + Bkg", RooArgList(model, *pdfMASS_bkg), RooArgList(*N_Jpsi, *N_Bkg));
  }
  else if (ptLow == 25)
  ...
  TFile *outFile;
  outFile = new TFile(Form("roots/%sMC/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP), "recreate");
  c_A->SaveAs(Form("figs/%sMC/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  // ws->Write();
