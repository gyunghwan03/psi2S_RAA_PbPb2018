...
void MassFit_FixPar_Data_Sigma(
    ...
    )
{
  ...
  gSystem->mkdir(Form("roots_2S_Pb_sigma/PRMC"),kTRUE);
  gSystem->mkdir(Form("roots_2S_Pb_sigma/NPMC"),kTRUE);
  gSystem->mkdir(Form("figs_2S_Pb_sigma/PRMC"),kTRUE);
  gSystem->mkdir(Form("figs_2S_Pb_sigma/NPMC"),kTRUE);
...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  if(PRw == 1)
    ...
    f_fit = new TFile(Form("mass_MC/roots_Pb/PRMC/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabel.Data(), fEffW, fAccW, isPtW, isTnP));
  TFile * f_Jpsi = new TFile(Form("../Jpsi/roots_MC_Root618_v2Final/2DFit_No_Weight/Mass/MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  RooDataSet *dataset_fit = (RooDataSet*)f_fit->Get("datasetMass");
  RooDataSet *dataset_Jpsi = (RooDataSet*)f_Jpsi->Get("datasetMass");
  RooWorkspace *ws_fit = new RooWorkspace("workspace_fit");
  RooWorkspace *ws_Jpsi = new RooWorkspace("workspace_Jpsi");
  ws_fit->import(*dataset_fit);
  ws_Jpsi->import(*dataset_Jpsi);
  ...
  Double_t sigma_MC_value = (ws_Jpsi->var("sigma_1_A")->getVal())*(pdgMass.Psi2S/pdgMass.JPsi);
  Double_t sigma_MC_value_err = (ws_Jpsi->var("sigma_1_A")->getError())*(pdgMass.Psi2S/pdgMass.JPsi);
  cout << "JPsi sigma : " << ws_Jpsi->var("sigma_1_A")->getVal() << " +/- " << ws_Jpsi->var("sigma_1_A")->getError() << endl;
  cout << "psi2S/Jpsi : " << pdgMass.Psi2S/pdgMass.JPsi << endl;

  double sigma_index = 6;
  if(ptLow==3.5&&cLow==0&&cHigh==20) sigma_index = 2;
  else if(ptLow==3.5&&cLow==20&&cHigh==60) sigma_index = 2;
  ...
  Double_t sigma_lower = sigma_MC_value-(sigma_MC_value_err);
  Double_t sigma_higher = sigma_MC_value+(sigma_MC_value_err);
  ...
  double paramslower[6] = {alpha_lower,   n_lower, sigma_lower,  xA_lower, 0.0,  0.0};
  double paramsupper[6] = {alpha_higher, n_higher, sigma_higher, xA_higher, 1.0, 25.0};
  ...
  //if (ptLow==3.5&&cLow==0&&cHigh==20) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher; }
  ////else if (ptLow==3.5&&ptHigh==5) { paramslower[2] = 0.08; paramsupper[2] = 0.16; }
  //else if (ptLow==3.5&&ptHigh==5) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher; }
  //else if (ptLow==6.5&&cLow==60&&cHigh==80) { paramslower[2] = 0; paramsupper[2] = 0.06; }
  //else if (ptLow==3.5&&cLow==20&&cHigh==60) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher; }
  //else if (ptLow==3.5&&cLow==60&&cHigh==100) { paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher;}
  //else if(ptLow==6.5&&ptHigh==9&&yLow==0) { paramslower[2] = 0.07; paramsupper[2] = 0.15; }
  ...
  if (ptLow==12&&ptHigh==15)  {
     ...
     NJpsi_lower = 10;
	   NJpsi_limit = 2e+3; }
  else if (ptLow==6.5&&ptHigh==9&&PRw==1)  {  
	   NBkg_limit = 5e+5;
	   NJpsi_limit = 5e+3; }
  ...
  if(ptLow==3.5) { pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(model, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg)); }
  else if(ptLow==25) { pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(model, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg)); }
  ...
  TFile* outFile;
  outFile = new TFile(Form("roots_2S_Pb_sigma/%sMC/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP),"recreate");
  c_A->SaveAs(Form("figs_2S_Pb_sigma/%sMC/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  //ws->Write();
