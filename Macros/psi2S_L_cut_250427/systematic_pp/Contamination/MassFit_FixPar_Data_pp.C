    ...
    int PRw = 1, bool fEffW = false, bool fAccW = false, bool isPtW = false, bool isTnP = false)
{
  ...
  if (mc_type == 1)
  {
    f2 = new TFile(Form("../../ctau_eff/syst/roots_2S_pp/decayL/PRMC/decay_hist_%s_m3.3-4.1.root", kineLabel.Data()));
    DATE = "PRMC";
  }
  else
  {
    f2 = new TFile(Form("../../ctau_eff/syst/roots_2S_pp/decayL/NPMC/decay_hist_%s_m3.3-4.1.root", kineLabel.Data()));
    DATE = "NPMC";
  }
  ...
  if(mc_type == 2) kineCut = Form("ctau3D>%.5f && pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>3.3 && mass<4.1", l_cut, ptLow, ptHigh, yLow, yHigh);
  else if(mc_type == 1) kineCut = Form("ctau3D<%.5f && pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>3.3 && mass<4.1", l_cut, ptLow, ptHigh, yLow, yHigh);
  ...
  TFile *f_fit = new TFile(Form("../pp_psi2S_250423/roots_MC/Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabel.Data(), fEffW, fAccW, isPtW, isTnP));
  ...
  TFile *outFile;
  outFile = new TFile(Form("roots_2S_pp/%s/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP), "recreate");
  c_A->SaveAs(Form("figs_2S_pp/%s/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", DATE.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  ws->Write();
