...
bool IsRooRealVarAtLimit(const RooRealVar &var, double absTol = 1e-8, double relTol = 1e-6)
{
  const double val = var.getVal();

  if (var.hasMin())
  {
    const double minVal = var.getMin();
    const double tol = std::max(absTol, relTol * std::max(1.0, std::fabs(minVal)));
    if (std::fabs(val - minVal) <= tol)
      return true;
  }

  if (var.hasMax())
  {
    const double maxVal = var.getMax();
    const double tol = std::max(absTol, relTol * std::max(1.0, std::fabs(maxVal)));
    if (std::fabs(val - maxVal) <= tol)
      return true;
  }
...
bool HasFloatingParameterAtLimit(const RooArgList &pars, std::vector<std::string> &paramsAtLimit)
{
  paramsAtLimit.clear();

  for (int i = 0; i < pars.getSize(); ++i)
  {
    RooRealVar *var = dynamic_cast<RooRealVar *>(pars.at(i));
    if (!var)
      continue;

    if (IsRooRealVarAtLimit(*var))
      paramsAtLimit.push_back(var->GetName());
  }

  return !paramsAtLimit.empty();
}

void QuietListCleanupErrors_psi2S_pbpb(Int_t level, Bool_t abort_bool, const char *location, const char *msg)
{
  if (level >= kError)
  {
    TString loc = location ? location : "";
    TString message = msg ? msg : "";
    if (loc.Contains("TList::Clear") || loc.Contains("THashList::Delete")) return;
    if (message.Contains("already deleted") && (message.Contains("list name = TList") || message.Contains("list name = THashList"))) return;
  }
...
void MassFit_FixPar_Data(
    ...
    int PRw = 1, // 0=PR, 1=NP, 2=Inc.
    bool fEffW = false, bool fAccW = false, bool isPtW = false, bool isTnP = false)
{
  SetErrorHandler(QuietListCleanupErrors_psi2S_pbpb);

  nCPU = 28;
  double l_cut = -10.;
  double l_low = -10.;
  double l_high = -10.;
  TString lcutFilePath = "";
  TString lcutHistName = "";
  Int_t lcutBin = -1;
  ...
  TString kineLabelpp = getKineLabelpp(ptLow, ptHigh, yLow, yHigh, 0.0);
  ...
  /*if (PRw == 0)
  {
    ...
    f3 = new TFile(Form("../../ctau_eff/roots_2S_Pb/decayL/NPMC/decay_hist_%s_m3.3-4.1.root", kineLabel.Data()));

    TH1D *h_Llow = (TH1D *)f2->Get("h_Lcut");
    TH1D *h_Lhigh = (TH1D *)f3->Get("h_Lcut");
    l_low = h_Llow->GetBinContent(1);
    l_high = h_Lhigh->GetBinContent(1);
    cout << "l_low: " << l_low << "l_high: " << l_high << endl;
  }

  else
  {
    f2 = new TFile(Form("../../ctau_eff/roots_2S_Pb/decayL/%sMC/decay_hist_%s_m3.3-4.1.root", fname.Data(), kineLabel.Data()));
    TH1D *h_Lcut = (TH1D *)f2->Get("h_Lcut");
    l_cut = h_Lcut->GetBinContent(1);
    cout << "l_cut: " << l_cut << endl;
  }
*/
f2 = new TFile(Form("../../Eff_Acc_pTreweight/roots_2S_PbPb_pTreweight/ctau3D_cut_%sMC_pt%.1f-%.1f_y%.1f-%.1f.root", fname.Data(), ptLow, ptHigh, yLow, yHigh));
f3 = new TFile(Form("../../Eff_Acc_pTreweight/roots_2S_PbPb/ctau3D_cut_centBin_%sMC_y0.0-2.4.root", fname.Data()));
TH1D *h_Lcut = nullptr;
if(yLow==1.6&&yHigh==2.4&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("h_ctau_bin"); lcutHistName = "hpt_ctau_1"; lcutFilePath = Form("../../Eff_Acc_pTreweight/roots_2S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_pt%.1f-%.1f-y%.1f-%.1f.root", fname.Data(), ptLow, ptHigh, yLow, yHigh); }
else if(yLow==0&&yHigh==2.4&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("h_ctau_bin"); lcutHistName = "hpt_ctau_2"; lcutFilePath = Form("../../Eff_Acc_pTreweight/roots_2S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_pt%.1f-%.1f-y%.1f-%.1f.root", fname.Data(), ptLow, ptHigh, yLow, yHigh); }
else if(yLow==1.6&&yHigh==2.4&&ptLow==3.5&&ptHigh==40) { h_Lcut = (TH1D *)f3->Get("hcent_ctau_1"); lcutHistName = "hcent_ctau_1"; lcutFilePath = Form("../../Eff_Acc_pTreweight/roots_2S_PbPb_pTreweight/ctau3D_cut_centBin_%sMC_pt%.1f-%.1f-y%.1f-%.1f.root", fname.Data(), ptLow, ptHigh, yLow, yHigh); }
else if(yLow==0&&yHigh==1.6&&ptLow==6.5&&ptHigh==40) { h_Lcut = (TH1D *)f3->Get("hcent_ctau_2"); lcutHistName = "hcent_ctau_2"; lcutFilePath = Form("../../Eff_Acc_pTreweight/roots_2S_PbPb_pTreweight/ctau3D_cut_centBin_%sMC_pt%.1f-%.1f-y%.1f-%.1f.root", fname.Data(), ptLow, ptHigh, yLow, yHigh); }
else if(yLow==1.6&&yHigh==2.4&&ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("h_ctau_int_for"); lcutHistName = "h_ctau_int_for"; lcutFilePath = Form("../../Eff_Acc_pTreweight/roots_2S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_pt%.1f-%.1f-y%.1f-%.1f.root", fname.Data(), ptLow, ptHigh, yLow, yHigh); }
else if(yLow==0&&yHigh==1.6&&ptLow==6.5&&ptHigh==40&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("h_ctau_int_mid"); lcutHistName = "h_ctau_int_mid"; lcutFilePath = Form("../../Eff_Acc_pTreweight/roots_2S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_pt%.1f-%.1f-y%.1f-%.1f.root", fname.Data(), ptLow, ptHigh, yLow, yHigh); }
if (!h_Lcut) {
  cout << "[ERROR] Failed to load ctau-cut histogram." << endl;
  return;
}
lcutBin = h_Lcut->GetXaxis()->FindBin(ptLow);
l_cut = h_Lcut->GetBinContent(1);
cout << "[L-cut source] file: " << lcutFilePath << endl;
cout << "[L-cut source] hist: " << lcutHistName
     << ", bin: " << lcutBin
     << ", range: [" << h_Lcut->GetXaxis()->GetBinLowEdge(lcutBin)
     << ", " << h_Lcut->GetXaxis()->GetBinUpEdge(lcutBin) << ")"
     << ", value: " << h_Lcut->GetBinContent(lcutBin) << endl;
cout << "l_cut: " << l_cut << endl; 
  ...
  TString ctauCut;
  if (PRw == 1)
    ctauCut = Form("ctau3D<%.5f &&", l_cut);
  else if (PRw == 2)
    ctauCut = Form("ctau3D>%.5f &&", l_cut);
  else if (PRw == 0)
    ctauCut = Form("ctau3D>%.5f && ctau3D<%.5f &&", l_low, l_high);

  // TString SglMuPt="pt1>0.5&&pt2>0.5"

  kineCut = OS + accCut + ctauCut + kineCut;
  ...
  TFile * f_fit = new TFile(Form("./roots_MC/Pb_Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  //TFile *f_fit;
  //if (PRw == 1 && PRw == 2)
  //{
  //  f_fit = new TFile(Form("mass_MC/roots_Pb/%sMC/mc_MassFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", fname.Data(), kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
  //}
  //if(PRw == 1)
  //  f_fit = new TFile(Form("mass_MC/roots_Pb/PRMC/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabel.Data(), fEffW, fAccW, isPtW, isTnP));
  //else if(PRw == 2)
  //  f_fit = new TFile(Form("mass_MC/roots_Pb/NPMC/mc_MassFitResult_%s_NPw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabel.Data(), fEffW, fAccW, isPtW, isTnP));
  //else if (PRw == 0)
  //  f_fit = new TFile(Form("mass_MC/roots_Pb/PRMC/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabel.Data(), fEffW, fAccW, isPtW, isTnP));
  ...
  if (ptLow == 3.5 && cLow == 0 && cHigh == 20 && PRw == 2)
    sigma_index = 1;
  else if (ptLow == 3.5 && cLow == 0 && cHigh == 20 && PRw == 1)
    sigma_index = 5;
  else if (ptLow == 3.5 && cLow == 20 && cHigh == 60)
    sigma_index = 1;
  else if (ptLow == 3.5 && cLow == 60 && cHigh == 100&& PRw == 1)
    sigma_index = 1;
  else if (ptLow == 3.5 && cLow == 60 && cHigh == 100&& PRw == 2)
    sigma_index = 2;
  else if (ptLow == 6.5 && ptHigh == 9 && yLow==0 && PRw == 1)
    sigma_index = 1;
  else if (ptLow == 6.5 && ptHigh == 9 && yLow==0)
    sigma_index = 1;
  else if(ptLow == 3.5 && ptHigh == 4.5 && PRw == 1)
    sigma_index = 1;
  else if(ptLow == 4.5 && ptHigh == 6.5 && PRw == 1)
    sigma_index = 1;
  else if(ptLow == 3.5 && ptHigh == 4.5 && PRw == 2)
    sigma_index = 1;
  else if (ptLow==20&&ptHigh==25&&PRw==2) sigma_index=1;
  else if (ptLow==6.5 && ptHigh==40 && cLow==80 && cHigh==100 ) sigma_index=1;
  ...
  double paramslower[6] = {alpha_lower, n_lower, 0, xA_lower, 0.0, 0.0};
  double paramsupper[6] = {alpha_higher, n_higher, 0.14, xA_higher, 1.0, 25.0};
  ...
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  else if (ptLow == 3.5 && ptHigh == 4.5)
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  ...
  // if (ptLow==3.5&&cLow==0&&cHigh==20) { paramslower[2] = 0; paramsupper[2] = 0.14; }
  // else if (ptLow==3.5&&ptHigh==5) { paramslower[2] = 0.08; paramsupper[2] = 0.16; }
  else if (ptLow == 6.5 && ptHigh == 9 && yLow == 0 && PRw==1)
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  else if (ptLow == 6.5 && ptHigh == 9 && yLow == 0 && PRw==2)
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  ...
  else if (ptLow == 6.5 && ptHigh == 40 && cLow == 0 && cHigh == 180)
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  else if (ptLow == 3.5 && cLow ==0 && cHigh ==20 && PRw==2)
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  ...
  else if (ptLow == 9 && ptHigh == 12 && yLow == 1.6 && PRw == 2)
  {
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher;
  }
  else if(ptLow==20&&ptHigh==25&&PRw==1)
  { 
    paramslower[2] = sigma_lower; paramsupper[2] = sigma_higher; 
  }
  else if(ptLow==20&&ptHigh==25&&PRw==2)
  { 
    paramslower[2] = sigma_lower;
    paramsupper[2] = sigma_higher; 
  }
  else if(ptLow==6.5&&ptHigh==40&&cLow==80&&cHigh==100) 
  {
    cout << "HERE" << endl;
    paramslower[2] = 0;
    paramsupper[2] = 0.14;
  ...
  cout << "sigma lower : " << paramslower[2] << "\tsigma higher : " << paramsupper[2] << endl;
  ...
  cout << "psi 2S MASS : " << pdgMass.Psi2S << endl;
  ...
  else if (ptLow == 3.5 && ptHigh == 6.5 && PRw == 0)
  {
    NJpsi_lower = 10;
    NBkg_limit = 1.65e+5;
    NJpsi_limit = 10000;
  }
  else if (ptLow == 3.5 && ptHigh == 6.5 && PRw == 1)
  {
    NJpsi_lower = 10;
    NBkg_limit = 1.65e+5;
    NJpsi_limit = 10000;
  ...
  else if (ptLow == 9.5 && ptHigh == 11 && PRw == 2)
  {
    NBkg_limit = 5e+5;
    NJpsi_lower = 0;
    NJpsi_limit = 3000;//300;
  }
  else if (ptLow == 8.5 && ptHigh == 12)
  {
    NBkg_limit = 5e+5;
    NJpsi_lower = 0;
    NJpsi_limit = 3000;//300;
  ...
  else if (ptLow == 6.5 && ptHigh == 9 && yLow == 1.6 && PRw == 0)
  {
    NJpsi_lower = 10;
    NBkg_limit = 5e+5;
    NJpsi_limit = 5e+3;
  ...
  else if (ptLow == 9 && ptHigh == 12 && yLow == 0 && PRw == 0)
  {
    NBkg_limit = 500000;
    NJpsi_limit = 10000;
  }
  else if (ptLow == 9 && ptHigh == 12 && yLow == 1.6 && PRw == 0)
  {
    NBkg_limit = 50000;
    NJpsi_limit = 1000;
  }
  else if (ptLow == 9 && ptHigh == 12 && yLow == 1.6 && PRw == 1)
  {
    NBkg_limit = 50000;
    NJpsi_limit = 1000;
  }
  else if (ptLow == 9.5 && ptHigh == 12 && yLow == 1.6)
  {
    NBkg_limit = 50000;
    NJpsi_limit = 1000;
  }
  else if (ptLow == 11 && ptHigh == 13 && yLow == 0 && PRw == 2)
  {
    NBkg_limit = 50000;
    NJpsi_limit = 1000;
  }
  else if (ptLow == 13 && ptHigh == 15 && yLow == 0 )
  {
    NBkg_limit = 50000;
    NJpsi_limit = 1000;
  }
  ...
  else if (ptLow == 12 && ptHigh == 40 && PRw == 0)
  {
    NBkg_limit = 500000;
    NJpsi_limit = 10000;
  }
  else if (ptLow == 12 && ptHigh == 40 && PRw == 1)
  {
    NBkg_limit = 500000;
    NJpsi_limit = 10000;
  }
  else if (ptLow == 12 && ptHigh == 40 && PRw ==2) 
  { 
    NBkg_limit = 500000;
    NJpsi_limit = 10000;
  }
  ...
  else if (ptLow == 3.5 && cLow == 0 && cHigh == 20 && PRw == 0)
  {
    NJpsi_lower = 10;
    NBkg_limit = 1e+6;
    NJpsi_limit = 1000;
  ...
  else if (ptLow == 3.5 && cLow == 20 && cHigh == 60 && PRw==2)
  {
    // NJpsi_lower = 600;
    NBkg_limit = 8.2e+4;
    NJpsi_limit = 1580;
  ...
  else if (ptLow == 3.5 && cLow == 60 && cHigh == 100 && PRw == 0)
  {
    NJpsi_lower = 10;
    NBkg_limit = 500000;
    NJpsi_limit = 1000;
  ...
  else if (ptLow == 3.5 && cLow == 100 && cHigh == 180 && PRw==1)
  {
    NBkg_limit = 7000;
    NJpsi_limit = 500;
  }
  ...
  else if (ptLow == 9 && ptHigh == 12 && PRw == 2)
  {
    NJpsi_limit = 1000;
    NBkg_limit = 1e+5;
  }
  ...
  RooGaussian sigma1_constraint("sigma1_constraint", "sigma1_constraint", sigma_1_A, RooConst(sigma_MC_value), RooConst(sigma_MC_value_err*sigma_index));
  ...
  RooProdPdf model("model", "model with constraint", RooArgSet(*pdfMASS_Jpsi, RooArgSet(sigma1_constraint)));
  ...
  {
    Ydown = YMin / (TMath::Power((YMax / YMin), (0.1 / (1.0 - 0.1 - 0.4))));
    Yup = YMax * TMath::Power((YMax / YMin), (0.7 / (1.0 - 0.1 - 0.4)));
  }
  ...
  drawText(Form("N_{#psi(2S)} = %.f #pm %.f,  N_{Bkg} = %.f #pm %.f", ws->var("N_Jpsi")->getVal(), ws->var("N_Jpsi")->getError(), ws->var("N_Bkg")->getVal(), ws->var("N_Bkg")->getError()), text_x, text_y - y_diff * 1, text_color, text_size);
  ...
  if (PRw == 1)
    ...
    drawText(Form("%.4f < l_{#psi(2S)} < %.4f", l_low, l_high), text_x, text_y - y_diff * 7, text_color, text_size);
  ...
  outh->GetXaxis()->SetBinLabel(2, "fitStatus");
  outh->GetXaxis()->SetBinLabel(3, "covQual");
  outh->GetXaxis()->SetBinLabel(4, "fitEDM");
  outh->GetXaxis()->SetBinLabel(5, "parAtLimit");
  outh->GetXaxis()->SetBinLabel(6, "fitFailed");
  ...
  fitMass->Print("V");
  cout << "sigma_lower = " << paramslower[2] << "\tsigma_higher = " << paramsupper[2] << endl;
  Double_t theNLL = fitMass->minNll();
  cout << " *** NLL : " << std::setprecision(15) << theNLL << endl;

  std::vector<std::string> paramsAtLimit;
  bool hasParAtLimit = HasFloatingParameterAtLimit(fitMass->floatParsFinal(), paramsAtLimit);
  int fitStatus = fitMass->status();
  int fitCovQual = fitMass->covQual();
  double fitEdm = fitMass->edm();
  bool fitFailed = (fitStatus != 0) || (fitCovQual < 3) || !std::isfinite(fitEdm) || hasParAtLimit;

  cout << "[FIT CHECK] status = " << fitStatus
       << ", covQual = " << fitCovQual
       << ", edm = " << fitEdm << endl;
  if (hasParAtLimit)
  {
    cout << "[FIT CHECK] Parameters at limit:";
    for (size_t i = 0; i < paramsAtLimit.size(); ++i)
      cout << " " << paramsAtLimit[i];
    cout << endl;
  }
  cout << "[FIT CHECK] Result = " << (fitFailed ? "FAIL" : "PASS") << endl;

  if (fitFailed)
    drawText("Fit status: FAIL", text_x, text_y - y_diff * 8, kRed + 1, text_size);
  else
    drawText("Fit status: PASS", text_x, text_y - y_diff * 8, kGreen + 2, text_size);

  outh->SetBinContent(2, fitStatus);
  outh->SetBinContent(3, fitCovQual);
  outh->SetBinContent(4, fitEdm);
  outh->SetBinContent(5, hasParAtLimit ? 1.0 : 0.0);
  outh->SetBinContent(6, fitFailed ? 1.0 : 0.0);
  ...
  TFile *outFile;
  outFile = new TFile(Form("roots_2S_Pb/%sMC/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP), "recreate");
  c_A->SaveAs(Form("figs_2S_Pb/%sMC/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  // ws->Write();
