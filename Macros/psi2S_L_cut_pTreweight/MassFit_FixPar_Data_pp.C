...
bool IsRooRealVarAtLimit_pp(const RooRealVar &var, double absTol = 1e-8, double relTol = 1e-6)
{
...
bool HasFloatingParameterAtLimit_pp(const RooArgList &pars, std::vector<std::string> &paramsAtLimit)
{
  paramsAtLimit.clear();
...
void QuietListCleanupErrors_psi2S_pp(Int_t level, Bool_t abort_bool, const char *location, const char *msg)
{
  if (level >= kError)
  {
...
{
  SetErrorHandler(QuietListCleanupErrors_psi2S_pp);

  double ctauCutValue = 1;
  ...
  TString kineLabelPb = getKineLabel(ptLow, ptHigh, yLow, yHigh, 0.0, 0, 180);
  ...
  auto f1 = new TFile(Form("../../skimmedFiles/OniaRooDataSet_isMC0_Psi2S_pp_y0.00_2.40_Effw0_Accw0_PtW0_TnP0_230515.root"));

  auto f2 = new TFile();
  auto f3 = new TFile();

  f2 = new TFile(Form("../../Eff_Acc_pTreweight/roots_2S_pp/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()));
  lcutFilePath = Form("../../Eff_Acc_pTreweight/roots_2S_pp/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data());

  gSystem->mkdir(Form("roots_2S_pp/%s/", DATE.Data()), kTRUE);
  gSystem->mkdir(Form("figs_2S_pp/%s/", DATE.Data()), kTRUE);

  TH1D *h_Lcut = nullptr;
  if (yLow == 1.6 && yHigh == 2.4 && ptLow == 3.5 && ptHigh == 40)
  {
    lcutHistName = "h_ctau_int_for";
    h_Lcut = (TH1D *)f2->Get("h_ctau_int_for");
    lcutBin = 1;
    ctauCutValue = h_Lcut->GetBinContent(1);
  }
  else if (yLow == 0 && yHigh == 1.6 && ptLow == 6.5 && ptHigh == 40)
  {
    lcutHistName = "h_ctau_int_mid";
    h_Lcut = (TH1D *)f2->Get("h_ctau_int_mid");
    lcutBin = 1;
    ctauCutValue = h_Lcut->GetBinContent(1);
  }
  else if (yLow == 1.6 && yHigh == 2.4)
  {
    lcutHistName = "hpt_ctau_1";
    h_Lcut = (TH1D *)f2->Get("hpt_ctau_1");
    lcutBin = h_Lcut->GetXaxis()->FindBin(ptLow);
    ctauCutValue = h_Lcut->GetBinContent(lcutBin);
  }
  else if (yLow == 0 && yHigh == 2.4)
  {
    lcutHistName = "hpt_ctau_2";
    h_Lcut = (TH1D *)f2->Get("hpt_ctau_2");
    lcutBin = h_Lcut->GetXaxis()->FindBin(ptLow);
    ctauCutValue = h_Lcut->GetBinContent(lcutBin);
  }
  if (!h_Lcut)
  {
    cout << "[ERROR] Failed to load ctau-cut histogram from input file." << endl;
    return;
  }
  if (lcutBin < 1) lcutBin = 1;
    const double lcutValueFromHist = h_Lcut->GetBinContent(lcutBin);
    ctauCutValue = lcutValueFromHist;
    ...
    cout << "[L-cut source] hist: " << lcutHistName
      ...
      << ", value: " << lcutValueFromHist << endl;
    printf("[L-cut debug] captured=%.10f, variable=%.10f\n", lcutValueFromHist, ctauCutValue);
    cout << "l_cut: " << lcutValueFromHist << endl;

  if(mc_type == 2) kineCut = Form("ctau3D>%.5f && pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>3.3 && mass<4.1", lcutValueFromHist, ptLow, ptHigh, yLow, yHigh);
  else if(mc_type == 1) kineCut = Form("ctau3D<%.5f && pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>3.3 && mass<4.1", lcutValueFromHist, ptLow, ptHigh, yLow, yHigh);
  ...
  TFile *f_fit = new TFile(Form("./roots_MC/Pb_Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabelPb.Data(), fEffW, fAccW, isPtW, isTnP));
  ...
  else if(ptLow==6.5&&ptHigh==40&&PRw==2)
  { sigma_index = 5; }
  ...
  double paramsupper[6] = {alpha_higher, n_higher, 0.15, xA_higher, 1.0, 25.0};
  ...
  // RooRealVar *sl1 = new RooRealVar("sl1","sl1", 0.28, -10., 10.); // setting 1 230510
  RooRealVar *sl1 = new RooRealVar("sl1", "sl1", 0.01, -1., 1.); // setting 2 230510
  ...
  else if (ptLow==20 && ptHigh==40 && yLow==1.6) {
    pdfMASS_bkg = new RooChebychev("pdfMASS_bkg","Background",*(ws->var("mass")),RooArgList(*sl1, *sl2));
  } 
  else {
  ...
  Double_t NBkg_limit = 1e6;
  Double_t NJpsi_limit = 1e5;
  Double_t NBkg_low = 0;
  Double_t NJpsi_low = 0;
  ...
  RooRealVar *N_Jpsi = new RooRealVar("N_Jpsi", "inclusive Jpsi signals", NJpsi_low, NJpsi_limit);
  RooRealVar *N_Bkg = new RooRealVar("N_Bkg", "fraction of component 1 in bkg", NBkg_low, NBkg_limit);
  ...
  else pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot", "Jpsi + Bkg", RooArgList(model, *pdfMASS_bkg), RooArgList(*N_Jpsi, *N_Bkg));  
  //RooAddPdf *pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot", "Jpsi + Bkg", RooArgList(*pdfMASS_Jpsi, *pdfMASS_bkg), RooArgList(*N_Jpsi, *N_Bkg));
  ...
  if (yLow == 0)
    drawText(Form("%.1f < p_{T}^{#mu#mu} < %.1f GeV/c, |y^{#mu#mu}| < %.1f ", ptLow, ptHigh, yHigh), text_x, text_y, text_color, text_size);
  else if (yLow != 0)
    drawText(Form("%.1f < p_{T}^{#mu#mu} < %.1f GeV/c; %.1f < |y^{#mu#mu}| < %.1f; ", ptLow, ptHigh, yLow, yHigh), text_x, text_y, text_color, text_size);
  ...
  if (mc_type == 1)
    drawText(Form("l_{#psi(2S)} < %.4f", ctauCutValue), text_x, text_y - y_diff * 7, text_color, text_size);
  else if (mc_type == 2)
    drawText(Form("l_{#psi(2S)} > %.4f", ctauCutValue), text_x, text_y - y_diff * 7, text_color, text_size);
  ...
  bool hasParAtLimit = HasFloatingParameterAtLimit_pp(fitMass->floatParsFinal(), paramsAtLimit);
  ...
  TFile *outFile;
  outFile = new TFile(Form("roots_2S_pp/%sMC/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP), "recreate");
  c_A->SaveAs(Form("figs_2S_pp/%sMC/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  //ws->Write();
