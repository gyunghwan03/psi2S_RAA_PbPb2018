		...
		)
{
    ...
    gSystem->mkdir(Form("./roots_MC/Pb_Mass"),kTRUE);
    gSystem->mkdir(Form("./figs"));
    gSystem->mkdir(Form("./figs/mc_Pb_Mass"),kTRUE);
	...
	double sigma_up=0.13; double sigma_lo=0; double x_up=3.; double x_lo=1.; double alpha_up=5; double alpha_lo=0; double n_up=5; double n_lo=0; double f_up=1; double f_lo=1e-6;
    ...
    double fit_limit = 3.9;

	if(ptLow==6.5&&ptHigh==9&&yLow==0) { N_Jpsi_high = 1e+6; x_lo=1; x_up = 5; fit_limit = 3.88;} //sigma_lo=0.034; sigma_up=0.038; }    
	else if(ptLow==6.5&&ptHigh==8.5&&yLow==0) { N_Jpsi_high = 1e+6; x_lo=0.1; x_up = 1; fit_limit = 3.88;} //sigma_lo=0.034; sigma_up=0.038; }    
	else if(ptLow==8.5&&ptHigh==9.5&&yLow==0) { N_Jpsi_high = 1e+6; x_lo=1; x_up = 3; fit_limit = 3.9; x_init = 1.33; f_init = 0.2776;} //sigma_lo=0.034; sigma_up=0.038; }    
    else if(ptLow==3.5&&ptHigh==6.5) { fit_limit = 3.9; x_lo=0.1; x_up = 5; }
    else if(ptLow==3&&ptHigh==4.5) { fit_limit = 3.9; x_lo=1; x_up = 5; x_init = 1.3; f_init = 0.2776;}
    else if(ptLow==3.5&&ptHigh==4.5) { fit_limit = 3.9; x_lo=1; x_up = 5; }
    else if(ptLow==4.5&&ptHigh==6.5) { fit_limit = 3.9; x_lo=1; x_up = 5; n_up = 15; n_1_init = 3.1; fit_limit = 3.88; f_init = 0.2776;}
    else if(ptLow==7.5&&ptHigh==8.5) { fit_limit = 3.9; x_lo=1; x_up = 5; f_init = 0.2776;}
    else if(ptLow==8.5&&ptHigh==9.5) { fit_limit = 3.9; x_lo=1; x_up = 5; f_init = 0.2776;}
    else if(ptLow==9.5&&ptHigh==11) { fit_limit = 3.9; x_lo=1; x_up = 5; f_init = 0.2776;}
    else if(ptLow==11&&ptHigh==13) { fit_limit = 3.9; x_lo=1; x_up = 5; }
	else if(ptLow==9&&ptHigh==12&&yLow==1.6) fit_limit = 3.9;
    else if(ptLow==12&&ptHigh==40) fit_limit = 3.9;
    else if(ptLow==15&&ptHigh==20) { fit_limit = 3.9; x_lo = 1.; x_up = 3.5; x_init = 2.4;}
	else if(ptLow>=12) { x_lo = 1.; f_init = 0.2776; fit_limit = 3.9; }
    else if(ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==20) { n_up = 15; n_1_init = 3.1; fit_limit = 3.82; x_lo=0.1; x_up = 5; sigma_1_init = 0.04; sigma_up = 0.14;}
	else if(ptLow==6.5&&cLow==40&&cHigh==60) { N_Jpsi_high = 1e+6; x_init = 1.3; }
	else if(ptLow==6.5&&ptHigh==40&&cLow==0&&cHigh==20) { cout << "HERE" << endl; N_Jpsi_high = 1e+6; x_init = 1.3; f_lo=0.1; f_up = 1.; fit_limit = 3.84; }
	else if(ptLow==6.5&&ptHigh==40&&cLow==20&&cHigh==40) { cout << "HERE" << endl; N_Jpsi_high = 1e+6; f_lo=0.1; f_up = 1.; x_lo = 1; x_up = 5.; fit_limit = 3.88; }
	else if(ptLow==6.5&&ptHigh==40&&cLow==60&&cHigh==80) { cout << "HERE" << endl; N_Jpsi_high = 1e+6; x_init = 1.3; f_lo=0.5; fit_limit = 3.84; }
	else if(ptLow==6.5&&ptHigh==40&&cLow==100&&cHigh==180) { cout << "HERE" << endl; N_Jpsi_high = 1e+6; x_lo =0.1; x_init = 0.4; f_lo=0.1; fit_limit = 3.88; }
    ...
    */
    ...
    TFile* outFile;
    outFile = new TFile(Form("./roots_MC/Pb_Mass/mc_MassFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP),"recreate");
    c_A->SaveAs(Form("./figs/mc_Pb_Mass/mc_Mass_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", kineLabel.Data(), fname.Data(), fEffW, fAccW, isPtW, isTnP));
    ...
    outFile->Close();
        fitMass->Print("v");
    // Fit 품질 점검: 파라미터가 lower/upper limit에 붙으면 FIT 실패로 판정
    bool limitHit = false;
    std::vector<std::string> limitHitParams;
    const RooArgList& finalPars = fitMass->floatParsFinal();
    for (int i = 0; i < finalPars.getSize(); ++i) {
        const RooRealVar* p = dynamic_cast<const RooRealVar*>(&finalPars[i]);
        if (!p) continue;
        // mean(m_{J/#Psi})는 고정/경계값이어도 실패 조건에서 제외
        if (std::string(p->GetName()) == "m_{J/#Psi}") continue;

        const double val = p->getVal();
        const double minV = p->getMin();
        const double maxV = p->getMax();
        const double tol = 1e-4 * std::max(1.0, std::fabs(maxV - minV));

        const bool atLower = std::fabs(val - minV) <= tol;
        const bool atUpper = std::fabs(val - maxV) <= tol;
        if (atLower || atUpper) {
            limitHit = true;
            limitHitParams.push_back(std::string(p->GetName()) + (atLower ? "(LOWER)" : "(UPPER)"));
        }
    }

    const bool minimizerOk = (fitMass->status() == 0);
    const bool covOk = (fitMass->covQual() >= 2);
    const bool fitSuccess = minimizerOk && covOk && !limitHit;

    cout << "\n[FitCheck] status=" << fitMass->status()
         << ", covQual=" << fitMass->covQual()
         << ", limitHit=" << (limitHit ? "YES" : "NO") << endl;
    if (limitHit) {
        cout << "[FitCheck] Parameters at limit:";
        for (size_t i = 0; i < limitHitParams.size(); ++i) {
            cout << (i == 0 ? " " : ", ") << limitHitParams[i];
        }
        cout << endl;
    }
    cout << "[FitCheck] FIT " << (fitSuccess ? "SUCCESS" : "FAIL") << endl;
