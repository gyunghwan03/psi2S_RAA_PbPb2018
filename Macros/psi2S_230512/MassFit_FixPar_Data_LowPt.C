...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  double paramslower[6] = {alpha_lower,   n_lower, sigma_lower, 0.0,  0.0,  0.0};
  double paramsupper[6] = {alpha_higher, n_higher, sigma_higher, 1.0, 1.0, 25.0};
...
//  ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A,VisualizeError(*fitMass,1),FillColor(kOrange));
  ...
  //ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A, Components(RooArgSet(*pdfMASS_bkg, *pdfMASS_Jpsi)), LineColor(44));
  ...
  drawText(Form("n_{#psi(2S)} = %.4f (fixed)",ws->var("n_1_A")->getVal()),text_x,text_y-y_diff*4,text_color,text_size);
  drawText(Form("#sigma_{1} = %.4f (fixed)",ws->var("sigma_1_A")->getVal()),text_x,text_y-y_diff*5,text_color,text_size);
