		...
		)
{
	...
	f1 = new TFile("../../../skimmedFiles/OniaRooDataSet_miniAOD_isMC0_Psi2S_cent0_200_Effw0_Accw0_PtW0_TnP0_230514.root");
	
	kineCut = Form("pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>3.3 && mass<4.1 && cBin>=%f && cBin<%f",ptLow, ptHigh, yLow, yHigh, cLow, cHigh);
	...
	double paramsupper[8] = {0.2,    1.0,     4.9, 5.9, 1.0,     25.0};
	double paramslower[8] = {0.01,   0.0,     0.1, 0.7, 0.0,    -25.0};
	//SIGNAL: initial params
	//double sigma_1_init = 0.04;
	//double x_init = 0.2;
	//double alpha_1_init = 1.6;
	//double n_1_init = 1.6;
	//double f_init = 0.2;
	double m_lambda_init = 2;

	//Bkg : initial params
	double sl1_init = 0.05;
	double sl2_init = 0.05;
	double sl3_init = 0.05;
	double sl4_init = 0.05;
	double sl5_init = 0.05;
	double sl6_init = 0.05;
	...
	if (ptLow==3.5&&ptHigh==6.5) {
		...
		sig_high = 800;
		bkg_high = 1000000;
	} else if(ptLow==6.5&&ptHigh==50 && cLow==0) {
		sig_high = 10000;
		bkg_high = 1000000;
	} else if(ptLow==6.5&&ptHigh==50 && cLow==20) {
		sig_high = 10000;
		bkg_high = 1000000;
		...
		sig_high = 1000;
		bkg_high = 1000000;
	} else if(ptLow==6.5&&ptHigh==50 && cLow==100) {
		sig_high = 800;
		bkg_high = 100000;
	}
	...
	//RooRealVar   *x_A = new RooRealVar("x_A","sigma ratio ", x_init, paramslower01], paramsupper[0]);
	//RooRealVar    sigma_1_A("sigma_1_A","width/sigma of the signal gaussian mass PDF",sigma_1_init, paramslower00], paramsupper[0]);
	//RooFormulaVar sigma_2_A("sigma_2_A","@0*@1",RooArgList(sigma_1_A, *x_A) );
	//RooRealVar    alpha_1_A("alpha_1_A","tail shift", alpha_1_init, paramslower02], paramsupper[0]);
	//RooFormulaVar alpha_2_A("alpha_2_A","1.0*@0",RooArgList(alpha_1_A) );
	//RooRealVar    n_1_A("n_1_A","power order", n_1_init , paramslower03], paramsupper[0]);
	//RooFormulaVar n_2_A("n_2_A","1.0*@0",RooArgList(n_1_A) );
	//RooRealVar   *f = new RooRealVar("f","cb fraction", f_init, paramslower04], paramsupper[0]);
