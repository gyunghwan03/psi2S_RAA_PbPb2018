    ...
    )
{
  ...
  f2 = new TFile(Form("../../ctau_eff/roots_1S_pp/decayL/PRMC/decay_hist_%s_m2.6-3.5.root", kineLabel.Data()));
  TH1D *h_Lcut=(TH1D*) f2->Get("h_Lcut");
  l_cut = h_Lcut->GetBinContent(1);
  kineCut = Form("ctau3D<%.5f && pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>2.6 && mass<3.5",l_cut, ptLow, ptHigh, yLow, yHigh);
  ...
  double paramsupper[6] = {alpha_higher, n_higher, 1., xA_higher, 1.0, 25.0};
  //first fit MC 
  ...
  //SIGNAL
  RooRealVar    mean("m_{J/#Psi}","mean of the signal gaussian mass PDF",pdgMass.JPsi, pdgMass.JPsi-0.3, pdgMass.JPsi+0.3) ;
  ...
  if(ptLow==12&&ptHigh==15&&yLow==1.6) NJpsi_limit = 1.0e+6;
  else if(ptLow==7&&ptHigh==8&&yLow==1.6) {NBkg_limit = 1.0e+6; }
  else if(ptLow==6.5&&ptHigh==9&&yLow==0) NBkg_limit = 1.0e+5;
  else if(ptLow==20&&ptHigh==25&&yLow==0) NBkg_limit = 1.0e+5;
