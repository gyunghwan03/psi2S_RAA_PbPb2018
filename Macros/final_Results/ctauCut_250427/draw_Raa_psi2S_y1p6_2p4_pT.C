...
void draw_Raa_psi2S_y1p6_2p4_pT(bool isSys = true)
{
    ...
    TFile *fEff_PbPbNP = new TFile("../../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_psi2s_PtW1_tnp1_20250225_22.root");
    ...
    for(int i=0;i<nPtBins;i++)
    {
		...
		cout << "Yield PbPb PR :\t" << yieldPbPb_PR << " +/- " << err_PbPbPR << "\tEff :\t" << eff_PbPbPR << "\tAcc :\t" << acc_PbPbPR << "\tctauEff :\t" << ctauEffPbPb_PR << "\tweight :\t" << weight_PbPbPR << endl;
		cout << "Yield pp NP :\t"   << yieldPP_NP   << " +/- " << err_PPNP   << "\tEff :\t" << eff_ppNP   << "\tAcc :\t" << acc_ppNP   << "\tctauEff :\t" << ctauEffPP_NP   << "\tweight :\t" << weight_ppNP << endl;
