...
void draw_Raa_JPsi_y0_1p6_pT_241014(bool isSys = true)
{
	...
	TFile *fEff_PbPbPR = new TFile("../../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_JPsi_PtWnomi_tnp1_Jpsiv2Wfunc_v4.root");
    TFile *fEff_PbPbNP = new TFile("../../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_JPsi_PtWnomi_tnp1_Jpsiv2Wfunc_v4.root");
    //TFile *fEff_ppPR = new TFile("../../Eff_Acc/roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtW1_tnp1_20240805.root");
    TFile *fEff_ppPR = new TFile("../../../Eff_Acc/roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtW1_tnp1_20241016.root");
    TFile *fEff_ppNP = new TFile("../../../Eff_Acc/roots/mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtW1_tnp1_20241016.root"); //Must Change
...
valErr getYield_pp(int i, int isPR){
    ...
    TFile* inf = new TFile(Form("../../Jpsi_L_cut/roots_1S_pp/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel[i].Data()));
...
valErr getCtauEff_pp(int i, int isPR){
    ...
    kineLabel[i] = getKineLabelpp(ptBins[i],ptBins[i+1],0,1.6,0.0);
    TString PR = (isPR==0) ? "PRMC" : "NPMC";
    TFile* f_ctau = new TFile(Form("../../../ctau_eff/roots_1S_pp/decayL/%s/decay_hist_%s_m2.6-3.5.root",PR.Data(), kineLabel[i].Data()));
    valErr ret; ret.val = 0; ret.err = 0;
    if (!f_ctau || f_ctau->IsZombie()) {
        cout << "Ctau file not opened! " << f_ctau->GetName() << endl;
        return ret;
    }
    TH1D* h_ctau = (TH1D*)f_ctau->Get("h_eff");
    if (!h_ctau) {
        cout << "h_eff histogram not found in file: " << f_ctau->GetName() << endl;
        f_ctau->Close();
        return ret;
    }
...
valErr getCtauEff_PbPb(int i, int isPR){
    ...
    TString kineLabel[7];
    kineLabel[i] = getKineLabel(ptBins[i],ptBins[i+1],0,1.6,0.0,0,180);
    TFile* f_ctau = new TFile(Form("../../../ctau_eff/roots_1S_Pb/decayL/%s/decay_hist_%s_m2.6-3.5.root",PR.Data(),kineLabel[i].Data()));
    cout << "Ctau file Name : " << f_ctau->GetName() << endl;
    valErr ret; ret.val = 0; ret.err = 0;
    if (!f_ctau || f_ctau->IsZombie()) {
        cout << "Ctau file not opened!" << endl;
        return ret;
    }
    TH1D* h_ctau = (TH1D*)f_ctau->Get("h_eff");
    if (!h_ctau) {
