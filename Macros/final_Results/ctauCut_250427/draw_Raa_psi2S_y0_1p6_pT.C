...
valErr getYield_pp(int i, int isPR){
    ...
    TFile* inf = new TFile(Form("../../psi2S_L_cut_250427/roots_2S_pp/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel[i].Data()));
...
valErr getCtauEff_pp(int i, int isPR){
	...
	TFile* f_ctau = new TFile(Form("../../../ctau_eff/roots_2S_pp/decayL/%s/decay_hist_%s_m3.3-4.1.root",PR.Data(), kineLabel[i].Data()));
...
valErr getCtauEff_PbPb(int i, int isPR){
    ...
    kineLabel[i] = getKineLabel(ptBins[i],ptBins[i+1],0,1.6,0.0,0,180);
	TFile* f_ctau = new TFile(Form("../../../ctau_eff/roots_2S_Pb/decayL/%s/decay_hist_%s_m3.3-4.1.root",PR.Data(),kineLabel[i].Data()));
	TH1D* h_ctau = (TH1D*)f_ctau->Get("h_eff");
	valErr ret;
	ret.val = h_ctau->GetBinContent(1);
	ret.err = h_ctau->GetBinError(1);
