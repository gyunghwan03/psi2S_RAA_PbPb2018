...
void compare_Npart(bool isSys=true)
{
	...
	for (int i=0; i<nCentBins_fwd; i++){
		...
		SysNP1S_fwd[i] = fwdNP_new[i]*(hSys1S_fwd_NP->GetBinContent(i+1));
	...
	TGraphErrors *g_fwdNPSys1S = new TGraphErrors(nCentBins_fwd,NpartBin_fwd,fwdNP_new,binWidth,SysNP1S_fwd);
	g_midNP1S = new TGraphErrors(nCentBins, NpartBin, midNP1S_new, 0, midNP1S_new_Err);
	g_fwdNP1S = new TGraphErrors(nCentBins_fwd, NpartBin_fwd, fwdNP_new, 0, fwdNP_new_Err);
	...
	TLegend *leg1 = new TLegend(0.35,0.69,0.62,0.54);
	...
	g_midNP1S->SetMarkerColor(kViolet+2);
	g_midNP1S->SetLineColor(kViolet+2);
	g_midNPSys1S->SetLineColor(kViolet-4);
	g_midNPSys1S->SetFillColorAlpha(kViolet-9,0.40);
