...
void compare_Npart_lJpsi(bool isSys=false)
{
	gStyle->SetOptStat(0);
	setTDRStyle();

	TFile *f_mid = new TFile("roots/RAA_JPsi_midRap_Npart.root"); 
	TFile *f_fwd = new TFile("roots/RAA_JPsi_forRap_Npart.root"); 
	...
	double NpartBin_fwd_old[nCentBins_fwd-1] = {32.7,160.3,311.5};
	...
	double fwdPR_lJpsi[nCentBins_fwd] = {
		0.490,
		0.398,
		0.298,
		0.226
	...
	for (int i=0; i<nCentBins_fwd; i++){
		fwdPR_Jpsi_old[i]     = h_PR_Jpsi_fwd_old->GetBinContent(nCentBins_fwd-i-1);
		fwdPR_Jpsi_old_Err[i] = h_PR_Jpsi_fwd_oldErr->GetBinContent(nCentBins_fwd-i-1);
		//fwdPR_Jpsi_old_Sys[i] = h_PR_Jpsi_fwd_oldSys->GetBinContent(nCentBins_fwd-i);
		//cout << "i : " << i << ", nCentBins-i-1 : " << nCentBins-i << " Old Val : " << midPR_old[i] << endl;
		cout << "i : " << i <<  " Old Val : " << fwdPR_Jpsi_old[i] << ", stat. Error : " << fwdPR_Jpsi_old_Err[i] << endl;//", syst. Error : " << fwdPR_Jpsi_old_Sys[i] << endl;
	}
	...
	TGraphErrors *g_fwdPR = new TGraphErrors(nCentBins_fwd, NpartBin_fwd, fwdPR_new, 0, fwdPR_new_Err);
	//TGraphErrors *g_Jpsi_fwdPR_old;
	...
	TGraphErrors *g_Jpsi_fwdPR_old = new TGraphErrors(nCentBins_fwd-1, NpartBin_fwd_old, fwdPR_Jpsi_old, 0, fwdPR_Jpsi_old_Err);

	TGraphErrors *g_midPR_lJpsi = new TGraphErrors(nCentBins, NpartBin, midPR_lJpsi, 0, 0);
	TGraphErrors *g_fwdPR_lJpsi = new TGraphErrors(nCentBins_fwd, NpartBin_fwd, fwdPR_lJpsi, 0, 0);
	...
	g_midPR_lJpsi->GetXaxis()->SetTitle("<N_{Part}>");
	...
	g_midPR_lJpsi->GetXaxis()->SetLimits(0.,400.);
	...
	TLegend *leg1 = new TLegend(0.45,0.58,0.66,0.68);
	leg1->SetTextSize(20);
	leg1->SetTextFont(43);
	leg1->SetBorderSize(0);
	leg1->AddEntry(g_midPR,"#bf{Prompt} J/#psi, New Data");
	leg1->AddEntry(g_midPR_lJpsi,"#bf{Prompt} J/#psi, #font[12]{l}_{J/#psi} cut");
	...
	g_fwdPR_lJpsi->GetXaxis()->SetTitle("<N_{Part}>");
	...
	g_fwdPR_lJpsi->GetXaxis()->SetLimits(0.,400.);
	...
	TLegend *leg2 = new TLegend(0.45,0.58,0.66,0.68);
	leg2->SetTextSize(20);
	leg2->SetTextFont(43);
	leg2->SetBorderSize(0);
	leg2->AddEntry(g_fwdPR,"#bf{Prompt} J/#psi, New Data");
	leg2->AddEntry(g_fwdPR_lJpsi,"#bf{Prompt} J/#psi, #font[12]{l}_{J/#psi} cut");
