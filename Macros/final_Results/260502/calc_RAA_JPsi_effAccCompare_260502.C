...
#include "JPsiEffAccInputs_260429.h"
...
namespace JPsiRAAEffAccCompare260502
{
...
TFile *OpenFitFile(const TString &path)
{
  ...
  if (!file || file->IsZombie())
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] failed to open " << path << std::endl;
  return file;
}
...
FitInput ReadFitValue(const TString &path, const char *histName, int bin)
{
  ...
  if (!hist)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] missing " << histName
              << " in " << path << std::endl;
    return out;
  }
...
double HistValue(TH1D *hist, int bin, const char *tag, bool enabled)
{
  ...
  if (!hist)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] null histogram for " << tag << std::endl;
    return 1.0;
  }
  const double value = hist->GetBinContent(bin);
  if (value <= 0.0)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] non-positive " << tag
...
TDirectory *GetOrMakeDirectory(TDirectory *parent, const TString &name)
{
  if (!parent)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] null parent directory for "
              << name << std::endl;
    return nullptr;
  }

  TDirectory *dir = parent->GetDirectory(name.Data());
  if (!dir)
    dir = parent->mkdir(name.Data());

  if (!dir)
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] failed to create directory "
              << name << " under " << parent->GetName() << std::endl;
  return dir;
}
...
{
  EffAccSet out;
  out.fEffPbPbPR = TakeFile(JPsiEffAcc260429::Open("mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_JPsi_PtWnomi_tnp1_260502_v5.root"));
  out.fEffPbPbNP = TakeFile(JPsiEffAcc260429::Open("mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_JPsi_PtWnomi_tnp1_260502_v5.root"));
  out.fEffPPPR = TakeFile(JPsiEffAcc260429::Open("mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtWnomi_tnp1_260504_ptBinScale.root"));
  out.fEffPPNP = TakeFile(JPsiEffAcc260429::Open("mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtWnomi_tnp1_260504_ptBinScale.root"));
  out.fAccPPPR = TakeFile(JPsiEffAcc260429::Open("acceptance_PromptJpsi_GenOnly_wgt1_pp_SysUp0_260503_v2_all4_extractCheck.root"));
  out.fAccPbPbPR = TakeFile(JPsiEffAcc260429::Open("acceptance_PromptJpsi_GenOnly_wgt1_PbPb_SysUp0_260503_v2_all4_extractCheck.root"));
  out.fAccPPNP = TakeFile(JPsiEffAcc260429::Open("acceptance_BtoJpsi_GenOnly_wgt1_pp_SysUp0_260503_v2_all4_extractCheck.root"));
  out.fAccPbPbNP = TakeFile(JPsiEffAcc260429::Open("acceptance_BtoJpsi_GenOnly_wgt1_PbPb_SysUp0_260503_v2_all4_extractCheck.root"));
  ...
  else
  {
    ...
    out.hEffPPPRInt.reset(JPsiEffAcc260429::BuildIntegratedRatioHist(out.fEffPPPR.get(), Form("hist_eff_num_%s", rap.Data()), Form("hist_eff_den_%s", rap.Data()), Form("hEff_ppPR_%s_int", rap.Data())));
    out.hEffPPNPInt.reset(JPsiEffAcc260429::BuildIntegratedRatioHist(out.fEffPPNP.get(), Form("hist_eff_num_%s", rap.Data()), Form("hist_eff_den_%s", rap.Data()), Form("hEff_ppNP_%s_int", rap.Data())));
...
void FillRAA(const AxisDef &axis, bool useEff, bool useAcc, TDirectory *outDir)
{
  ...
  const TString title = axis.label.Length() ? axis.label : axis.name;
  std::unique_ptr<TH1D> hRAAPR = MakeHist(axis, Form("hRAA_PR_%s", tag.Data()), title);
  std::unique_ptr<TH1D> hRAANP = MakeHist(axis, Form("hRAA_NP_%s", tag.Data()), title);
  std::unique_ptr<TH1D> hPbPbPR = MakeHist(axis, Form("hYieldPbPb_PR_%s", tag.Data()), title);
  std::unique_ptr<TH1D> hPbPbNP = MakeHist(axis, Form("hYieldPbPb_NP_%s", tag.Data()), title);
  std::unique_ptr<TH1D> hPPPR = MakeHist(axis, Form("hYieldPP_PR_%s", tag.Data()), title);
  std::unique_ptr<TH1D> hPPNP = MakeHist(axis, Form("hYieldPP_NP_%s", tag.Data()), title);
  ...
  TDirectory *caseDir = GetOrMakeDirectory(outDir, tag);
  if (!caseDir)
    return;
  caseDir->cd();
  hRAAPR->Write();
...
{
  TDirectory *axisDir = GetOrMakeDirectory(&outFile, axis.name);
  if (!axisDir)
    return;
                                             ...
                                             const char *errName, int nPoints)
{
  ...
  if (!file || file->IsZombie())
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] failed to open HIN file "
              << fileName << std::endl;
    return nullptr;
  }
  ...
  if (!hist || !err)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] missing HIN histogram "
                      ...
                      const char *componentTag, const char *componentLabel, bool isPrompt)
{
  if (!hists[0])
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] missing " << componentLabel
  ...
  const bool isMidPromptNpart = isPrompt && axis.isMid && !axis.isPt;
  const bool isFwdPromptNpart = isPrompt && !axis.isMid && !axis.isPt;
  const bool isMidNonpromptPt = !isPrompt && axis.isMid && axis.isPt;
  const bool isFwdNonpromptPt = !isPrompt && !axis.isMid && axis.isPt;
  const bool isMidNonpromptNpart = !isPrompt && axis.isMid && !axis.isPt;
  const bool isFwdNonpromptNpart = !isPrompt && !axis.isMid && !axis.isPt;
  TLegend *legCases = new TLegend(0.14, 0.73, 0.71, 0.89);
  //TLegend *legCases = new TLegend(0.18, 0.63, isFwdNonpromptNpart ? 0.95 : (axis.isPt ? 0.54 : 0.58), 0.89);
  legCases->SetBorderSize(0);
  legCases->SetFillStyle(0);
  for (int i = 0; i < 4; ++i)
    if (graphs[i])
      legCases->AddEntry(graphs[i].get(), caseLabels[i], "pe");
  const char *hinLabel = "HIN-16-025";
  if (isMidPromptNpart)
    hinLabel = "HIN-16-025 6.5 < p_{T} < 30 GeV/c";
  else if (isFwdPromptNpart)
    hinLabel = "HIN-16-025, 3 < p_{T} < 30 GeV/c";
  else if (isMidNonpromptPt)
    hinLabel = "HIN-16-025, |y| < 0.6";
  else if (isFwdNonpromptPt)
    hinLabel = "HIN-16-025, 1.8 < |y| < 2.4";
  else if (isMidNonpromptNpart)
    hinLabel = "HIN-16-025, 6.5 < p_{T} < 50 GeV/c, |y| < 0.6";
  else if (isFwdNonpromptNpart)
    hinLabel = "HIN-16-025 6.5 < p_{T} < 50 GeV/c, 1.8 < |y| < 2.4";
  const char *hinLowPtLabel = isFwdNonpromptNpart ? "HIN-16-025 3 < p_{T} < 6.5 GeV/c, 1.8 < |y| < 2.4" : "HIN-16-025 3 < p_{T} < 6.5 GeV/c";
  if (!hinGraphs.empty() && hinGraphs[0])
    legCases->AddEntry(hinGraphs[0].get(), hinLabel, "pe");
  if (hinGraphs.size() > 1 && hinGraphs[1])
    legCases->AddEntry(hinGraphs[1].get(), hinLowPtLabel, "pe");
  legCases->Draw();

  TLatex latex;
  latex.SetNDC();
  latex.SetTextSize(0.038);
  const TString label = axis.label.Length() ? axis.label : axis.name;
  latex.DrawLatex(0.18, 0.93, Form("J/#psi %s %s", label.Data(), componentLabel));

  canvas->SaveAs(Form("./figs/RAA_JPsi_effAccCompare_260502_%s_%s.pdf", axis.name.Data(), componentTag));
//  canvas->SaveAs(Form("./figs/RAA_JPsi_effAccCompare_260502_%s_%s.png", axis.name.Data(), componentTag));
}
...
void DrawAxisRAA(TFile &outFile, const AxisDef &axis)
{
  ...
  const int lineStyles[4] = {1, 1, 1, 1};
...
void calc_RAA_JPsi_effAccCompare_260502()
{
  using namespace JPsiRAAEffAccCompare260502;

  gStyle->SetOptStat(0);
  gSystem->mkdir("./figs", true);
  gSystem->mkdir("./roots", true);
  TFile outFile("./roots/RAA_JPsi_effAccCompare_260502.root", "RECREATE");
  if (outFile.IsZombie())
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260502] failed to create ./roots/RAA_JPsi_effAccCompare_260502.root" << std::endl;
    return;
  }
  ...
  std::cout << "\nWrote ./roots/RAA_JPsi_effAccCompare_260502.root" << std::endl;
  std::cout << "Wrote ./figs/RAA_JPsi_effAccCompare_260502_{midRap_pT,fwdRap_pT,midRap_Npart,fwdRap_Npart_4Bins}_{prompt,nonprompt}.{pdf}" << std::endl;
}
