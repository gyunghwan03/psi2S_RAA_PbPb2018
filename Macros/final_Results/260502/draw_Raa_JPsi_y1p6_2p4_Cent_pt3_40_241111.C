...
void draw_Raa_JPsi_y1p6_2p4_Cent_pt3_40_241111(bool isSys=true)
{
	...
	TFile *fSys = new TFile("../syst_summary_psi2S/syst_roots/total_syst.root"); //Must change
    //TFile *fSys = new TFile("../syst_summary/syst_roots/total_syst_NobFrac.root");
	TFile *fEff_PbPbPR = JPsiEffAcc260429::Open("mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_JPsi_PtWnomi_tnp1_260429_1d.root");
    TFile *fEff_PbPbNP = JPsiEffAcc260429::Open("mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_JPsi_PtWnomi_tnp1_260429_1d.root");
    ...
    TFile *fAcc_PbPbNP = JPsiEffAcc260429::Open("acceptance_BtoJpsi_GenOnly_wgt1_PbPb_SysUp0_260503_v2_all4_extractCheck.root");
    ...
    TH1D *hRAA_PR = new TH1D("hRAA_PR",";p_{T} (GeV/c);", nCentBins,centBin);
    TH1D *hRAA_NP = new TH1D("hRAA_NP",";p_{T} (GeV/c);", nCentBins,centBin);
    ...
    for(int i=0;i<nCentBins;i++)
    {
        ...
        */
        ...
        XPbPb_NP[i] = yieldPbPb_NP/(Nmb*Taa[i]*(double)(40.-3.)*(double)2*(2.4-1.6)*cfrac[i]);
...
*/
	...
	SetLegendStyle(leg);
	leg->AddEntry(gRaaPR,"Prompt J/#psi", "p");
	leg->AddEntry(gRaaNP,"Non Prompt J/#psi", "p");
	leg->Draw("SAME");
	jumSun(0,1,400,1);
	...
	cRAA->SaveAs(Form("figs/RAA_JPsi_y1p6_2p4_Cent_pt3_40_Sys%d_4Bins.pdf",isSys));

	TFile *f1 = new TFile("roots/RAA_JPsi_forRap_Npart_pt3_40_4Bins.root","recreate");
	f1->cd();
	hRAA_PR->Write();
	hRAA_NP->Write();
	f1->Close();
...
valErr getYield_pp(int i){
    TString kineLabel;
    kineLabel = getKineLabelpp(3.,40,1.6,2.4,0.0);
...
valErr getYield_PbPb(int i){
    ...
    TString kineLabel[5];
    kineLabel[i] = getKineLabel(3.,40,1.6,2.4,0.0,centBin[i],centBin[i+1]);
