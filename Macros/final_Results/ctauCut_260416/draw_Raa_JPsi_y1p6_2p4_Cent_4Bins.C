...
valErr getYield_PbPb_Int(int isPR=0);
//valErr getCtauEff_PbPb_Int(int isPR=0);
...
void draw_Raa_JPsi_y1p6_2p4_Cent_4Bins(bool isSys=true)
{
	...
	TFile *fEff_PbPbPR = new TFile("../../../Eff_Acc_260416/roots/mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_JPsi_PtWnomi_tnp1_ctauCut_260422_v7.root");
	TFile *fEff_PbPbNP = new TFile("../../../Eff_Acc_260416/roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_JPsi_PtWnomi_tnp1_ctauCut_260422_v7.root");
	//TFile *fEff_PbPbPR = new TFile("../../../Eff_Acc_260416/roots/mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_JPsi_PtWnomi_tnp1_ctauCut_260416_2exp.root");
	//TFile *fEff_PbPbNP = new TFile("../../../Eff_Acc_260416/roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_JPsi_PtWnomi_tnp1_ctauCut_260416_2exp.root");
    TFile *fEff_ppPR = new TFile("../../../Eff_Acc_260416/roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtWnomi_tnp1_ctauCut_260420_2exp.root");
    TFile *fEff_ppNP = new TFile("../../../Eff_Acc_260416/roots/mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtWnomi_tnp1_ctauCut_260420_2exp.root");
    TFile *fAcc_ppPR = new TFile("../../../Eff_Acc_260416/roots/acceptance_PromptJpsi_GenOnly_wgt1_pp_SysUp0_20260420.root");
    TFile *fAcc_PbPbPR = new TFile("../../../Eff_Acc_260416/roots/acceptance_PromptJpsi_GenOnly_wgt1_PbPb_SysUp0_20260420.root");
    TFile *fAcc_ppNP = new TFile("../../../Eff_Acc_260416/roots/acceptance_BtoJpsi_GenOnly_wgt1_pp_SysUp0_20260420.root"); 
    TFile *fAcc_PbPbNP = new TFile("../../../Eff_Acc_260416/roots/acceptance_BtoJpsi_GenOnly_wgt1_PbPb_SysUp0_20260420.root"); 
	...
	TH1D *hEff_PbPbPR_Integrated = (TH1D*) fEff_PbPbPR -> Get("mc_eff_Integrated_TnP1_PtW1_cent_0_to_180_absy1p6_2p4");
	TH1D *hEff_PbPbNP_Integrated = (TH1D*) fEff_PbPbNP -> Get("mc_eff_Integrated_TnP1_PtW1_cent_0_to_180_absy1p6_2p4");
    ...
    for(int i=0;i<nCentBins;i++)
    {
        ...
        //cout << "Cent Bin : " << centBin[i] << "-" << centBin[i+1] << endl;
			cout << Form("Cent %3.0f - %3.0f   Cent Frac : %7.4g", centBin[i], centBin[i+1], cfrac[i]) << endl;
			cout << Form("%-15s %12.7g +/- %10.4g   Eff : %10.5g   Acc : %10.5g   weight : %10.5g", "Yield pp PR :",   yieldPP_PR*weight_ppPR,     err_PPPR,   eff_ppPR,   acc_ppPR,   weight_ppPR) << endl;
			cout << Form("%-15s %12.7g +/- %10.4g   Eff : %10.5g   Acc : %10.5g   weight : %10.5g", "Yield PbPb PR :", yieldPbPb_PR*weight_PbPbPR, err_PbPbPR, eff_PbPbPR, acc_PbPbPR, weight_PbPbPR) << endl;
			cout << Form("%-15s %12.7g +/- %10.4g   Eff : %10.5g   Acc : %10.5g   weight : %10.5g", "Yield pp NP :",   yieldPP_NP*weight_ppNP,     err_PPNP,   eff_ppNP,   acc_ppNP,   weight_ppNP) << endl;
			cout << Form("%-15s %12.7g +/- %10.4g   Eff : %10.5g   Acc : %10.5g   weight : %10.5g", "Yield PbPb NP :", yieldPbPb_NP*weight_PbPbNP, err_PbPbNP, eff_PbPbNP, acc_PbPbNP, weight_PbPbNP) << endl;
...
*/
	...
	// Calculate RAA for full pT range (3.5-40 GeV/c, 0-180%)
	...
	// Use centrality 0-180% bin (bin 1) for PbPb
	...
	valErr yieldPbPb_PR_int = getYield_PbPb_Int(0);
	valErr yieldPbPb_NP_int = getYield_PbPb_Int(1);
	
	//valErr ctauEffPP_PR_int = getCtauEff_pp(0, 0);
	//valErr ctauEffPP_NP_int = getCtauEff_pp(0, 1);
	//valErr ctauEffPbPb_PR_int = getCtauEff_PbPb_Int(0);
	//valErr ctauEffPbPb_NP_int = getCtauEff_PbPb_Int(1);
	...
	double Xpp_PR_int = lumi_pp_scale * yieldPP_PR_corr / (lumi_pp*1e+2*(double)(40.-3.5)*2*(double)(2.4-1.6));
	double Xpp_NP_int = lumi_pp_scale * yieldPP_NP_corr / (lumi_pp*1e+2*(double)(40.-3.5)*2*(double)(2.4-1.6));
	double XPbPb_PR_int = yieldPbPb_PR_corr / (Nmb*Taa_int*(double)(40.-3.5)*(double)2*(2.4-1.6));
	double XPbPb_NP_int = yieldPbPb_NP_corr / (Nmb*Taa_int*(double)(40.-3.5)*(double)2*(2.4-1.6));
	...
	// Add integrated RAA values for pT 3.5-40
	hfwdPR_int -> SetBinContent(2,RaaPR_int);
	hfwdPR_int -> SetBinError(2,RaaPR_int_err);
	hfwdNP_int -> SetBinContent(2,RaaNP_int);
	hfwdNP_int -> SetBinError(2,RaaNP_int_err);
...
valErr getYield_pp(int i, int isPR){
	...
	TFile* inf = new TFile(Form("../../Jpsi_L_cut_260130/roots_1S_pp/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel.Data()));
...
valErr getYield_PbPb_Int(int isPR){
    TString kineLabel;
	TString PR = (isPR==0) ? "PRMC" : "NPMC";	
    kineLabel = getKineLabel(3.5,40,1.6,2.4,0.0,0,180);
    TFile *inf = new TFile(Form("../../Jpsi_L_cut_260130/roots_1S_Pb/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel.Data()));
	valErr ret; ret.val = 0; ret.err = 0;
	if (!inf || inf->IsZombie()) {
		cout << "파일을 열 수 없습니다: " << inf->GetName() << endl;
		return ret;
