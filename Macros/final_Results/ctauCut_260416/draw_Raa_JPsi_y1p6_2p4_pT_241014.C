...
void draw_Raa_JPsi_y1p6_2p4_pT_241014(bool isSys = false)
{
	//gROOT->SumW2();
    gStyle->SetOptStat(0);
    setTDRStyle();
    ...
    for(int i=0;i<nPtBins;i++)
    {
		...
		for (int isPR = 0; isPR < 2; isPR++)
		{
			...
			else if(isPR == 1)
			{
				...
				//if(i==3) yieldPbPb_NP = 250/(weight_PbPbNP);
	...
	cXPR->SaveAs("./figs/CrossSection_JPsi_PR_y1p6_2p4_pT.pdf");
	...
	cXNP->SaveAs("./figs/CrossSection_JPsi_NP_y1p6_2p4_pT.pdf");
	...
	cRAA->SaveAs(Form("./figs/RAA_JPsi_y1p6_2p4_pT_Sys%d.pdf",isSys));
...
valErr getYield_pp(int i, int isPR){
    ...
    TFile *inf = new TFile(Form("../../Jpsi_L_cut_260130/roots_1S_pp/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel[i].Data()));
    valErr ret;
    ret.val = 0;
    ret.err = 0;
    if (!inf || inf->IsZombie())
