...
void draw_Raa_JPsi_y1p6_2p4_pT_260519v2(bool isSys = false)
{
	//gROOT->SumW2();
    gStyle->SetOptStat(0);
    setTDRStyle();
    ...
    // pT axis -> B_nominal_2exp full-stat Eff/Acc (PtW on)
	...
	TH1D *hSys_PR = (TH1D*) fSys -> Get("fwd_pt_PR");
    TH1D *hSys_PR_pb = (TH1D *)fSys->Get("fwd_pt_PR_pb");
    TH1D *hSys_PR_pp = (TH1D *)fSys->Get("fwd_pt_PR_pp");
    TH1D *hSys_NP = (TH1D *)fSys->Get("fwd_pt_NP");
    TH1D *hSys_NP_pb = (TH1D *)fSys->Get("fwd_pt_NP_pb");
    TH1D *hSys_NP_pp = (TH1D *)fSys->Get("fwd_pt_NP_pp");
    ...
    double ptBin[nPtBins+1] = {3.5,6.5,9,12,40};
    ...
    for(int i=0;i<nPtBins;i++)
    {
        ...
        eff_ppPR=hEff_ppPR->GetBinContent(i+1);
        eff_ppNP=hEff_ppNP->GetBinContent(i+1);
	...
	gXpp_PR->GetXaxis()->SetLimits(0.,40);
	gXpp_PR->SetMinimum(1e-9);
	...
	cXPR->SaveAs("./figs/CrossSection_JPsi_260519v2_PR_y1p6_2p4_pT.pdf");
	...
	gXpp_NP->GetXaxis()->SetLimits(0.,40);
	gXpp_NP->SetMinimum(1e-9);
	...
	cXNP->SaveAs("./figs/CrossSection_JPsi_260519v2_NP_y1p6_2p4_pT.pdf");
	...
	gRaaPR->GetXaxis()->SetLimits(0.,40);
	gRaaPR->SetMinimum(0);
	...
	TLegend *leg = new TLegend(0.6,0.72,0.8,0.82);
    SetLegendStyle(leg);
	leg->AddEntry(gRaaPR,"Prompt J/#psi", "ep");
	leg->AddEntry(gRaaNP,"Non Prompt J/#psi", "ep");
	leg->Draw("SAME");
	jumSun(0,1,40,1);
	...
	cRAA->SaveAs(Form("./figs/RAA_JPsi_y1p6_2p4_pT_260519v2_Sys%d.pdf",isSys));

	TFile *f1 = new TFile("./roots/RAA_JPsi_forRap_pT.root","recreate");
	f1->cd();
	hRAA_PR->Write();
	hRAA_NP->Write();
	f1->Close();
...
valErr getYield_pp(int i){
    double ptBins[5] = {3.5,6.5,9,12,40};
    TString kineLabel[5];
    kineLabel[i] = getKineLabelpp(ptBins[i],ptBins[i+1],1.6,2.4,0.0);
    TFile* inf = new TFile(Form("../../pp_Jpsi/roots/2DFit_No_Weight/Mass/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", kineLabel[i].Data()));
...
valErr getFrac_PbPb(int i) {
    double ptBin[5] = {3.5,6.5,9,12,40};
    TString kineLabel[5];
...
valErr getFrac_pp(int i) {
    ...
    TFile* inf = new TFile(Form("../../pp_Jpsi/roots/2DFit_No_Weight/Final/2DFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", kineLabel[i].Data()));
    TH1D* fitResults = (TH1D*)inf->Get("2DfitResults");
