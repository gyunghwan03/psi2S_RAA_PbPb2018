...
namespace JPsiRAAEffAccCompare260519v2
{
...
TFile *OpenFitFile(const TString &path)
{
  ...
  if (!file || file->IsZombie())
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2] failed to open " << path << std::endl;
  return file;
}
...
FitInput ReadFitValue(const TString &path, const char *histName, int bin)
{
  ...
  if (!hist)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2] missing " << histName
              << " in " << path << std::endl;
    return out;
  }
...
double HistValue(TH1D *hist, int bin, const char *tag, bool enabled)
{
  ...
  if (!hist)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2] null histogram for " << tag << std::endl;
    return 1.0;
  }
  const double value = hist->GetBinContent(bin);
  if (value <= 0.0)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2] non-positive " << tag
...
{
  EffAccSet out;
  const TString tag = JPsiEffAcc260519v2::Tag();
  out.fEffPbPbPR = TakeFile(JPsiEffAcc260519v2::Open(Form("mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_JPsi_PtWnomi_tnp1_%s.root", tag.Data())));
  out.fEffPbPbNP = TakeFile(JPsiEffAcc260519v2::Open(Form("mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_JPsi_PtWnomi_tnp1_%s.root", tag.Data())));
  out.fEffPPPR = TakeFile(JPsiEffAcc260519v2::Open(Form("mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtWnomi_tnp1_%s.root", tag.Data())));
  out.fEffPPNP = TakeFile(JPsiEffAcc260519v2::Open(Form("mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtWnomi_tnp1_%s.root", tag.Data())));
  out.fAccPPPR = TakeFile(JPsiEffAcc260519v2::Open(Form("acceptance_PromptJpsi_GenOnly_wgt1_pp_SysUp0_%s.root", tag.Data())));
  out.fAccPbPbPR = TakeFile(JPsiEffAcc260519v2::Open(Form("acceptance_PromptJpsi_GenOnly_wgt1_PbPb_SysUp0_%s.root", tag.Data())));
  out.fAccPPNP = TakeFile(JPsiEffAcc260519v2::Open(Form("acceptance_BtoJpsi_GenOnly_wgt1_pp_SysUp0_%s.root", tag.Data())));
  out.fAccPbPbNP = TakeFile(JPsiEffAcc260519v2::Open(Form("acceptance_BtoJpsi_GenOnly_wgt1_PbPb_SysUp0_%s.root", tag.Data())));
                                             ...
                                             const char *errName, int nPoints)
{
  ...
  if (!file || file->IsZombie())
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2] failed to open HIN file "
              << fileName << std::endl;
    return nullptr;
  }
  ...
  if (!hist || !err)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2] missing HIN histogram "
                      ...
                      const char *componentTag, const char *componentLabel, bool isPrompt)
{
  if (!hists[0])
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2] missing " << componentLabel
  ...
  canvas->SaveAs(Form("./figs/RAA_JPsi_effAccCompare_260519v2_%s_%s.pdf", axis.name.Data(), componentTag));
  canvas->SaveAs(Form("./figs/RAA_JPsi_effAccCompare_260519v2_%s_%s.png", axis.name.Data(), componentTag));
}
...
void calc_RAA_JPsi_effAccCompare_260519v2()
{
  using namespace JPsiRAAEffAccCompare260519v2;

  gStyle->SetOptStat(0);
  gSystem->mkdir("./figs", true);
  TFile outFile("./roots/RAA_JPsi_effAccCompare_260519v2.root", "RECREATE");
  ...
  std::cout << "\nWrote ./roots/RAA_JPsi_effAccCompare_260519v2.root" << std::endl;
  std::cout << "Wrote ./figs/RAA_JPsi_effAccCompare_260519v2_{midRap_pT,fwdRap_pT,midRap_Npart,fwdRap_Npart_4Bins}_{prompt,nonprompt}.{pdf,png}" << std::endl;
}
