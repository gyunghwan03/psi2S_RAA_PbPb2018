...
namespace JPsiRAAEffAccCompare260519v2NoPtWNpart
{
...
TFile *OpenNoPtW(const char *fileName)
{
  ...
  if (!f || f->IsZombie())
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart] cannot open " << fileName << std::endl;
  return f;
}
...
TFile *OpenFitFile(const TString &path)
{
  ...
  if (!file || file->IsZombie())
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart] failed to open " << path << std::endl;
  return file;
}
...
FitInput ReadFitValue(const TString &path, const char *histName, int bin)
{
  ...
  if (!hist)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart] missing " << histName
              << " in " << path << std::endl;
    return out;
  }
...
double HistValue(TH1D *hist, int bin, const char *tag, bool enabled)
{
  ...
  if (!hist)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart] null histogram for " << tag << std::endl;
    return 1.0;
  }
  const double value = hist->GetBinContent(bin);
  if (value <= 0.0)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart] non-positive " << tag
                                             ...
                                             const char *errName, int nPoints)
{
  ...
  if (!file || file->IsZombie())
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart] failed to open HIN file "
              << fileName << std::endl;
    return nullptr;
  }
  ...
  if (!hist || !err)
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart] missing HIN histogram "
                      ...
                      const char *componentTag, const char *componentLabel, bool isPrompt)
{
  if (!hists[0])
  {
    std::cerr << "[calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart] missing " << componentLabel
  ...
  canvas->SaveAs(Form("./figs/RAA_JPsi_effAccCompare_260519v2_noPtW_Npart_%s_%s.pdf", axis.name.Data(), componentTag));
  canvas->SaveAs(Form("./figs/RAA_JPsi_effAccCompare_260519v2_noPtW_Npart_%s_%s.png", axis.name.Data(), componentTag));
}
...
void calc_RAA_JPsi_effAccCompare_260519v2_noPtW_Npart()
{
  using namespace JPsiRAAEffAccCompare260519v2NoPtWNpart;

  gStyle->SetOptStat(0);
  gSystem->mkdir("./figs", true);
  TFile outFile("./roots/RAA_JPsi_effAccCompare_260519v2_noPtW_Npart.root", "RECREATE");
  ...
  std::cout << "\nWrote ./roots/RAA_JPsi_effAccCompare_260519v2_noPtW_Npart.root" << std::endl;
  std::cout << "Wrote ./figs/RAA_JPsi_effAccCompare_260519v2_noPtW_Npart_{midRap_Npart,fwdRap_Npart_4Bins}_{prompt,nonprompt}.{pdf,png}" << std::endl;
}
