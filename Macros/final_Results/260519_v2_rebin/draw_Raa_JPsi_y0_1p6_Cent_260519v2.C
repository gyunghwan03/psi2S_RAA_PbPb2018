...
void draw_Raa_JPsi_y0_1p6_Cent_260519v2(bool isSys=false)
{
	...
	// Npart axis -> B_nominal_2exp full-stat Eff/Acc (PtW ON, option A)
	const TString tagB = JPsiEffAcc260519v2::Tag();
	TFile *fEff_PbPbPR = JPsiEffAcc260519v2::Open(Form("mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_JPsi_PtWnomi_tnp1_%s.root", tagB.Data()));
    TFile *fEff_PbPbNP = JPsiEffAcc260519v2::Open(Form("mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_JPsi_PtWnomi_tnp1_%s.root", tagB.Data()));
    TFile *fEff_ppPR = JPsiEffAcc260519v2::Open(Form("mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtWnomi_tnp1_%s.root", tagB.Data()));
    TFile *fEff_ppNP = JPsiEffAcc260519v2::Open(Form("mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtWnomi_tnp1_%s.root", tagB.Data()));
    TFile *fAcc_ppPR = JPsiEffAcc260519v2::Open(Form("acceptance_PromptJpsi_GenOnly_wgt1_pp_SysUp0_%s.root", tagB.Data()));
    TFile *fAcc_PbPbPR = JPsiEffAcc260519v2::Open(Form("acceptance_PromptJpsi_GenOnly_wgt1_PbPb_SysUp0_%s.root", tagB.Data()));
    TFile *fAcc_ppNP = JPsiEffAcc260519v2::Open(Form("acceptance_BtoJpsi_GenOnly_wgt1_pp_SysUp0_%s.root", tagB.Data()));
    TFile *fAcc_PbPbNP = JPsiEffAcc260519v2::Open(Form("acceptance_BtoJpsi_GenOnly_wgt1_PbPb_SysUp0_%s.root", tagB.Data()));


    TH1D *hEff_PbPbPR = (TH1D*) fEff_PbPbPR -> Get("mc_eff_vs_cent_TnP1_PtW1_pt_6p5_to_40_absy0_1p6");
    TH1D *hEff_PbPbNP = (TH1D*) fEff_PbPbNP -> Get("mc_eff_vs_cent_TnP1_PtW1_pt_6p5_to_40_absy0_1p6");
    TH1D *hEff_ppPR = JPsiEffAcc260519v2::BuildIntegratedRatioHist(fEff_ppPR, "hist_eff_num_mid", "hist_eff_den_mid", "hEff_ppPR_mid_260519v2");
    TH1D *hEff_ppNP = JPsiEffAcc260519v2::BuildIntegratedRatioHist(fEff_ppNP, "hist_eff_num_mid", "hist_eff_den_mid", "hEff_ppNP_mid_260519v2");
	...
	TH1D *hRAA_PR = new TH1D("hRAA_PR",";Centrality (%);", nCentBins,centBin);
	TH1D *hRAA_NP = new TH1D("hRAA_NP",";Centrality (%);", nCentBins,centBin);
	...
	for(int i=0;i<nCentBins;i++)
    {
        ...
        cout << "PbPb Yield	: " << yieldPbPb.val << ",	Yield Err	: " << yieldPbPb.err  << ",	b frac	: " << fracPbPb.val << ",	b frac Err	: " << fracPbPb.err <<  ",	Eff Prompt	: " << eff_PbPbPR << ",	Eff NonPrompt	: " << eff_PbPbNP <<  ",	Acc PR	: " << acc_PbPbPR  << ",	Acc NP	:	" << acc_PbPbNP << endl;
        cout << "pp Yield	: " << yieldPP.val << ",	Yield Err	: " << yieldPP.err  << ",	b frac	: " << fracPP.val << ",	b frac Err	: " << fracPP.err << ",	Eff Prompt	: " << eff_ppPR << ",	Eff NonPrompt	: "<< eff_ppNP <<  ",	Acc PR	: " << acc_ppPR << ",	Acc NP	:	" << acc_ppNP << endl;
		...
		XPbPb_PR[i] = yieldPbPb_PR/(Nmb*Taa[i]*(double)(40.-6.5)*2*(double)(1.6)*cfrac[i]);
		...
		XPbPb_NP[i] = yieldPbPb_NP/(Nmb*Taa[i]*(double)(40.-6.5)*2*(double)(1.6)*cfrac[i]);
...
*/
	...
	cXPR->SaveAs("./figs/CrossSection_PR_y0_1p6_Cent_260519v2.pdf");
	...
	cXNP->SaveAs("figs/CrossSection_NP_y0_1p6_Cent_260519v2.pdf");
	...
	pad1_2->cd();
	double midPR2S_int  = 0.356; double midPR2S_intErr = 0.01;
	double midNP2S_int  = 0.460; double midNP2S_intErr = 0.021;
	...
	cRAA->SaveAs(Form("./figs/RAA_JPsi_y0_1p6_Npart_260519v2_Sys%d.pdf",isSys));

	TFile *outFile = new TFile("./roots/RAA_JPsi_midRap_Npart.root","recreate");
	hRAA_PR->Write();
	hRAA_NP->Write();
	hmidPR_int->Write();
	hmidNP_int->Write();
...
valErr getYield_pp(int i){
    ...
    TFile* inf = new TFile(Form("../../pp_Jpsi/roots/2DFit_No_Weight/Mass/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", kineLabel.Data()));
...
valErr getFrac_PbPb(int i) {
    ...
    ret.val = fitResults->GetBinContent(1);
    ret.err = fitResults->GetBinError(1);
	cout << "kineLabel : " << kineLabel[i] << endl;
    return ret;
}
valErr getFrac_pp(int i) {
    TString kineLabel;
    kineLabel = getKineLabelpp(6.5,40,0,1.6,0.0);
    TFile* inf = new TFile(Form("../../pp_Jpsi/roots/2DFit_No_Weight/Final/2DFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", kineLabel.Data()));
    TH1D* fitResults = (TH1D*)inf->Get("2DfitResults");
