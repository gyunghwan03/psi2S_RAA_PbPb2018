...
void DoubleRatio_Charmonia_Npart_fwdRap_240807()
{
	gStyle->SetOptStat(0);
	setTDRStyle();
	...
	double centBin[nCentBins_fwd+1] = {0,10,30,50,90};
	...
	for (int i=0; i<nCentBins_fwd; i++){
		...
		cout << "cent." << centBin[nCentBins_fwd-i-1] << "-" << centBin[nCentBins_fwd-i] << "\tJpsi NP : " << fwdNP_Jpsi[i] << "\tpsi2S NP : " << fwdNP_2S[i] << "\tDouble Ratio : " << DR_fwdNP[i] << endl; 
	}
    ...
    leg1->SetTextFont(42);
	leg1->AddEntry(g_fwdPR,"#bf{Prompt}, 3.5 < p_{T} < 40 GeV/c, 1.6 < |y| < 2.4, Cent. 0-90%", "pe");
	leg1->AddEntry(g_old,"#bf{Prompt (Old)}, 3 < p_{T} < 30 GeV/c, 1.6 < |y| < 2.4, Cent. 0-100%", "pe");
	...
	double fwdPR2S_int; double fwdPR2S_intErr;
	double fwdPRJpsi_int; double fwdPRJpsi_intErr;

	TH1D *h_fwdPR_2S_int = (TH1D*) f_fwd_2S->Get("hfwdPR_int");
	TH1D *h_fwdPR_Jpsi_int = (TH1D*) f_fwd_Jpsi->Get("hfwdPR_int");
	fwdPR2S_int = h_fwdPR_2S_int->GetBinContent(2);
	fwdPR2S_intErr = h_fwdPR_2S_int->GetBinError(2);
	fwdPRJpsi_int = h_fwdPR_Jpsi_int->GetBinContent(2);
	fwdPRJpsi_intErr = h_fwdPR_Jpsi_int->GetBinError(2);
    ...
    hfwdPR_int->GetYaxis()->SetRangeUser(0,1.80);
	...
	c1->SaveAs("figs/DoubleRatio_Prompt_Npart_fwdRap_240807.pdf");
	c1->SaveAs("figs/DoubleRatio_Prompt_Npart_fwdRap_240807.png");
    ...
    leg2->SetTextFont(42);
	leg2->AddEntry(g_fwdNP,"#bf{NonPrompt}, 3.5 < p_{T} < 40 GeV/c, 1.6 < |y| < 2.4, Cent. 0-90%", "pe");
    ...
    double fwdNPJpsi_int = 0.424; double fwdNPJpsi_intErr = 0.0029;
    ...
    hfwdNP_int->GetYaxis()->SetRangeUser(0,1.);
	...
	c2->SaveAs("figs/DoubleRatio_NonPrompt_Npart_fwdRap_240807.pdf");
	c2->SaveAs("figs/DoubleRatio_NonPrompt_Npart_fwdRap_240807.png");
