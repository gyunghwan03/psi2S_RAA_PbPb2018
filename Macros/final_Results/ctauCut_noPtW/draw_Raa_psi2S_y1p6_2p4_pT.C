...
void draw_Raa_psi2S_y1p6_2p4_pT(bool isSys = true)
{
	...
	cXPR->SaveAs(Form("./figs/CrossSection_PR_y1p6_2p4_pT_%.f.pdf",40.));
	...
	cXNP->SaveAs(Form("./figs/CrossSection_NP_y1p6_2p4_pT_%.f.pdf",40.));
	...
	cRAA->SaveAs(Form("./figs/RAA_psi2S_y1p6_2p4_pT_%.f_Sys%d.pdf",40.,isSys));

	TFile *f1 = new TFile(Form("./roots/RAA_psi2S_forRap_pT_%.f.root",40.),"recreate");
	f1->cd();
	hRAA_PR->Write();
	hRAA_NP->Write();
	f1->Close();
