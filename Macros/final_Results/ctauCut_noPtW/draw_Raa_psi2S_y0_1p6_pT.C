...
void draw_Raa_psi2S_y0_1p6_pT(bool isSys = true)
{
	...
	for(int i=0;i<nPtBins;i++)
	{
		...
		//cout << "Prompt" << endl;
		//cout << "PbPb Yield	: " << yieldPbPb_PR << ",	Yield Err	: " << err_PbPbPR  << ",	PR Eff	: " << eff_PbPbPR << ",	NP Eff	: " << eff_PbPbNP<< ",	PR Acc	: " << acc_PbPbPR << ", NP Acc  : " << acc_PbPbNP<< endl;
		//cout << "pp Yield	: " << yieldPP.val << ",	Yield Err	: " << err_PPPR << ",	PR Eff	: " << eff_ppPR << ",	NP Eff	: " << eff_ppNP << ",	PR Acc	: " << acc_ppPR << ", NP Acc  : " << acc_ppNP << endl;
	...
	cXPR->SaveAs(Form("figs/CrossSection_PR_y0_1p6_pT_%.f.pdf",40.));
	...
	cXNP->SaveAs(Form("figs/CrossSection_NP_y0_1p6_pT_%.f.pdf",40.));
	...
	gSysPR->GetXaxis()->SetLimits(0.,40);
	gSysPR->SetMinimum(0);
	gSysPR->SetMaximum(1.44);
	gSysNP->GetXaxis()->SetLimits(0.,40);
	gSysNP->SetMinimum(0);
	...
	cRAA->SaveAs(Form("figs/RAA_psi2S_y0_1p6_pT_%.f_Sys%d.pdf",40.,isSys));

	TFile *outFile = new TFile(Form("./roots/RAA_psi2S_midRap_pT_%.f.root",40.),"recreate");
	hRAA_PR->Write();
	hRAA_NP->Write();
	outFile->Close();
...
valErr getYield_pp(int i, int isPR){
	...
	TFile* inf = TFile::Open(Form("../../psi2S_L_cut_250427/roots_2S_pp/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel[i].Data()));
	TH1D* fitResults = inf ? (TH1D*)inf->Get("fitResults") : nullptr;
	//cout << "file Name : " << (inf ? inf->GetName() : "(null)") << endl;

	valErr ret; 
	ret.val = fitResults ? fitResults->GetBinContent(1) : 0.0;
	ret.err = fitResults ? fitResults->GetBinError(1) : 0.0;
...
valErr getYield_PbPb(int i, int isPR){
	...
	TFile* inf = TFile::Open(Form("../../psi2S_L_cut_250427/roots_2S_Pb/%s/Mass_FixedFitResult_%s_PRw_Effw0_Accw0_PtW0_TnP0.root", PR.Data(), kineLabel[i].Data()));
	//cout << "file Name : " << (inf ? inf->GetName() : "(null)") << endl;
