...
void draw_Raa_JPsi_y1p6_2p4_Cent(bool isSys=true)
{
...
*/
	...
	cout << " " << endl;
	cout.precision(3);
	cout << "\\multicolumn{2}{c}{$1.6 < |y| < 2.4$, $3.5 < \\pt < 50$ (GeV/c)} \\\\ \\hline" << endl;
	for(int i=0;i<nCentBins;i++)
	{
	...
	cRAA->SaveAs(Form("figs/RAA_JPsi_y1p6_2p4_Cent_Sys%d.pdf",isSys));

	TFile *f1 = new TFile("roots/RAA_JPsi_forRap_Npart.root","recreate");
	f1->cd();
	hRAA_PR->Write();
	hRAA_NP->Write();
	f1->Close();
