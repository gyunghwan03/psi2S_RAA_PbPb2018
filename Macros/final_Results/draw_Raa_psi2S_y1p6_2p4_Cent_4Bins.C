...
*/
	...
	cXPR->SaveAs(Form("figs/CrossSection_PR_y1p6_2p4_Cent_pt3p5_%.f_4Bins.pdf",ptHigh));
	...
	TFile *f1 = new TFile(Form("roots/RAA_psi2S_forRap_Npart_pt3p5_%.f_4Bins.root",ptHigh),"recreate");
	f1->cd();
	hRAA_PR->Write();
	hRAA_NP->Write();
	f1->Close();
