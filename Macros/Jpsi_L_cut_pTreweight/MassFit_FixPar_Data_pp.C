...
void QuietListCleanupErrors(Int_t level, Bool_t abort_bool, const char* location, const char* msg)
{
  if (level >= kError) {
...
{
  ErrorHandlerFunc_t oldErrorHandler = GetErrorHandler();
  SetErrorHandler(QuietListCleanupErrors);
...
//  f1 = new TFile(Form("../../skimmedFiles/v2Cut_Nom/OniaRooDataSet_isMC0_Psi2S_%s_m3.3-4.1_OS_Effw%d_Accw%d_PtW%d_TnP%d_221013_root618.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  Int_t prevErrLevel = gErrorIgnoreLevel;
  gErrorIgnoreLevel = kFatal;
  f2 = TFile::Open(Form("../../Eff_Acc_root634/roots_1S_pp_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()), "READ");
  lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_pp_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data());
  gErrorIgnoreLevel = prevErrLevel;
  if (!f2 || f2->IsZombie()) {
    cout << "[WARN] Failed to open Eff_Acc_root634 ctau-cut file, trying Eff_Acc fallback." << endl;
    if (f2) { f2->Close(); delete f2; f2 = nullptr; }
    f2 = TFile::Open(Form("../../Eff_Acc/roots_1S_pp_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()), "READ");
    lcutFilePath = Form("../../Eff_Acc/roots_1S_pp_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data());
  }

  gSystem->mkdir(Form("roots_1S_pp/%s/", DATE.Data()), kTRUE);
  gSystem->mkdir(Form("figs_1S_pp/%s/", DATE.Data()), kTRUE);

  TH1D *h_Lcut = nullptr;
  if (yLow == 1.6 && yHigh == 2.4 && ptLow == 3.5 && ptHigh == 40)
  {
    lcutHistName = "h_ctau_int_for";
    TH1D* htmp = (TH1D *)f2->Get("h_ctau_int_for");
    if (htmp) {
      h_Lcut = (TH1D*)htmp->Clone("h_ctau_int_for_local");
      h_Lcut->SetDirectory(0);
    }
    lcutBin = 1;
  ...
  else if (yLow == 0 && yHigh == 1.6 && ptLow == 6.5 && ptHigh == 40)
  {
    ...
    TH1D* htmp = (TH1D *)f2->Get("h_ctau_int_mid");
    if (htmp) {
      h_Lcut = (TH1D*)htmp->Clone("h_ctau_int_mid_local");
      h_Lcut->SetDirectory(0);
    }
    lcutBin = 1;
    l_cut = h_Lcut->GetBinContent(1);
  ...
  else if (yLow == 1.6 && yHigh == 2.4)
  {
    ...
    TH1D* htmp = (TH1D *)f2->Get("hpt_ctau_1");
    if (htmp) {
      h_Lcut = (TH1D*)htmp->Clone("hpt_ctau_1_local");
      h_Lcut->SetDirectory(0);
    }
  ...
  else if (yLow == 0 && yHigh == 2.4)
  {
    ...
    TH1D* htmp = (TH1D *)f2->Get("hpt_ctau_2");
    if (htmp) {
      h_Lcut = (TH1D*)htmp->Clone("hpt_ctau_2_local_allY");
      h_Lcut->SetDirectory(0);
    }
    lcutBin = h_Lcut->GetXaxis()->FindBin(ptLow);
    l_cut = h_Lcut->GetBinContent(lcutBin);
  }
  else if (yLow == 0 && yHigh == 1.6)
  {
    lcutHistName = "hpt_ctau_2";
    TH1D* htmp = (TH1D *)f2->Get("hpt_ctau_2");
    if (htmp) {
      h_Lcut = (TH1D*)htmp->Clone("hpt_ctau_2_local");
      h_Lcut->SetDirectory(0);
    }
    lcutBin = h_Lcut->GetXaxis()->FindBin(ptLow);
    l_cut = h_Lcut->GetBinContent(lcutBin);
  ...
  TFile * f_fit = new TFile(Form("roots_MC/pp_Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  if(ptLow==6.5&&ptHigh==40&&PRw==1) { NJpsi_limit=1.6e+06; NBkg_limit = 2e+7; s1_init = 0.5; s2_init = -0.1; }
  else if(ptLow==6.5&&ptHigh==9&&yLow==1.6&&PRw==1) { NJpsi_limit=1.0e+07; NBkg_limit = 1e+7; }
  else if(ptLow==3.5&&ptHigh==6.5&&yLow==1.6&&PRw==1) { NJpsi_limit=1.0e+07; NBkg_limit = 1e+7; }
  else if(ptLow==6.5&&ptHigh==9&&PRw==2) { NJpsi_limit=1.0e+07; NBkg_limit = 1e+7; }
  else if(ptLow==3.5&&ptHigh==6.5&&PRw==2) { NJpsi_limit = 1e+7; }
  else if(ptLow==9&&ptHigh==12&&yLow==1.6&&PRw==2) { NJpsi_limit = 1e+7; NBkg_limit = 1e+7; s1_init = 0.1; }
  ...
  else if(ptLow==13&&ptHigh==15&&PRw==1) { NJpsi_limit = 1e+7; NBkg_limit = 1e+7; s1_init = 0.; }
  ...
  else if(ptLow==20&&ptHigh==22.5&&PRw==2) { NJpsi_limit = 1e+7; NBkg_limit = 1e+6; }
  ...
  else if(ptLow==25&&ptHigh==40&&PRw==2) { NJpsi_limit = 1e+7; NBkg_limit = 1e+6; }
  else if(ptLow==30&&ptHigh==40) { NJpsi_limit = 1e+7; NBkg_limit = 1e+6; }

  else if(ptLow==20&&ptHigh==40) { NJpsi_limit = 1e+7; NBkg_limit = 1e+6; }
  else if(ptLow==9&&ptHigh==40) { NJpsi_limit = 1e+7; NBkg_limit = 1e+6; }
  ...
  else if(ptLow==6.5&&ptHigh==40&&PRw==1) pdfMASS_bkg = new RooChebychev("pdfMASS_bkg","Background",*(ws->var("mass")),RooArgList(*sl1, *sl2, *sl3, *sl4));
...
//  ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A,VisualizeError(*fitMass,1),FillColor(kOrange));
  ...
  TFile* outFile;
  outFile = new TFile(Form("roots_1S_pp/%sMC/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP),"recreate");
  c_A->SaveAs(Form("figs_1S_pp/%sMC/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  //ws->Write();
