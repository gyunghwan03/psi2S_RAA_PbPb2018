    ...
    )
{
  ...
  gSystem->mkdir(Form("roots_1S_Pb"),kTRUE);
  gSystem->mkdir(Form("figs_1S_Pb"),kTRUE);
  gSystem->mkdir(Form("roots_1S_Pb/%sMC",fname.Data()),kTRUE);
  gSystem->mkdir(Form("figs_1S_Pb/%sMC",fname.Data()),kTRUE);
  gSystem->mkdir(Form("roots_1S_Pb/%sMC/Mass",fname.Data()),kTRUE);
  gSystem->mkdir(Form("figs_1S_Pb/%sMC/Mass",fname.Data()),kTRUE);
  ...
  f2 = new TFile(Form("../../Eff_Acc_pTreweight/roots_1S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()));
  f3 = new TFile(Form("../../Eff_Acc_pTreweight/roots_1S_PbPb_pTreweight/ctau3D_cut_centBin_%sMC_y0.0-2.4.root", fname.Data()));
  lcutFilePath = Form("../../Eff_Acc_pTreweight/roots_1S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data());
  TH1D *h_Lcut = nullptr;
  if(yLow==1.6&&yHigh==2.4&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("hpt_ctau_1"); lcutHistName = "hpt_ctau_1"; lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()); }
  else if(yLow==0&&yHigh==2.4&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("hpt_ctau_2"); lcutHistName = "hpt_ctau_2"; lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()); }
  else if(yLow==1.6&&yHigh==2.4&&ptLow==3.5&&ptHigh==40) { h_Lcut = (TH1D *)f3->Get("hcent_ctau_1"); lcutHistName = "hcent_ctau_1"; lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_PbPb_pTreweight/ctau3D_cut_centBin_%sMC_y0.0-2.4.root", fname.Data()); }
  else if(yLow==0&&yHigh==1.6&&ptLow==6.5&&ptHigh==40) { h_Lcut = (TH1D *)f3->Get("hcent_ctau_2"); lcutHistName = "hcent_ctau_2"; lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_PbPb_pTreweight/ctau3D_cut_centBin_%sMC_y0.0-2.4.root", fname.Data()); }
  else if(yLow==1.6&&yHigh==2.4&&ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("h_ctau_int_for"); lcutHistName = "h_ctau_int_for"; lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()); }
  else if(yLow==0&&yHigh==1.6&&ptLow==6.5&&ptHigh==40&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("h_ctau_int_mid"); lcutHistName = "h_ctau_int_mid"; lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_PbPb_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()); }
  if (!h_Lcut) {
    cout << "[ERROR] Failed to load ctau-cut histogram." << endl;
    return;
  }
  lcutBin = h_Lcut->GetXaxis()->FindBin(ptLow);
  l_cut = h_Lcut->GetBinContent(lcutBin);
...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  kineCut = Form("pt>%.2f && pt<%.2f && abs(y)>%.2f && abs(y)<%.2f && mass>%.1f && mass<%.1f && cBin>=%d && cBin<%d",ptLow, ptHigh, yLow, yHigh, massLow, massHigh, cLow, cHigh);
  ...
  TFile * f_fit = new TFile(Form("roots_MC/pp_Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root",kineLabelpp.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  if(ptLow==3.5&&cLow==0&&cHigh==20) sigma_MC_value = 0.045;

  double sigma_index = 5;
  if(ptLow==3.5&&cLow==0&&cHigh==20) sigma_index = 0.1;
  else if(ptLow==3.5&&cLow==20&&cHigh==60) sigma_index = 1;
  else if(ptLow==3.5&&cLow==60&&cHigh==100) sigma_index = 3;
  ...
  RooRealVar *sl1 = new RooRealVar("sl1","sl1", 0.01, -10., 10.); // 15<pt<50 v2==-1.2 : 0.01
  ...
  if(ptLow==25&&ptHigh==40&&PRw==1) NJpsi_limit = 1.0e+06;
  else if (ptLow==30&&ptHigh==40) NJpsi_limit = 3e+3;
  ...
  else if (ptLow==11&&ptHigh==13&&PRw==1) {NJpsi_limit = 5e+4;}
  ...
  fitMass->Print("V");
  cout << "sigma_lower = " << sigma_lower << "\tsigma_higher = " << sigma_higher << endl;
  ...
  TFile* outFile;
  outFile = new TFile(Form("roots_1S_Pb/%sMC/Mass_FixedFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP),"recreate");
  c_A->SaveAs(Form("figs_1S_Pb/%sMC/Mass_Fixed_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.pdf", fname.Data(), kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  pdfMASS_Tot->Write();
  pdfMASS_bkg->Write();
  datasetMass->Write();
  //ws->Write();
