...
//  f1 = new TFile(Form("../../skimmedFiles/v2Cut_Nom/OniaRooDataSet_isMC0_Psi2S_%s_m3.3-4.1_OS_Effw%d_Accw%d_PtW%d_TnP%d_221013_root618.root",kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  f2 = new TFile(Form("../../Eff_Acc_root634/roots_1S_pp_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data(), kineLabel.Data()));
  TH1D *h_Lcut = nullptr;
  if(yLow==1.6&&yHigh==2.4) { h_Lcut = (TH1D *)f2->Get("hpt_ctau_1"); lcutHistName = "hpt_ctau_1"; lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_pp_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()); }
  else if(yLow==0&&yHigh==2.4) { h_Lcut = (TH1D *)f2->Get("hpt_ctau_2"); lcutHistName = "hpt_ctau_2"; lcutFilePath = Form("../../Eff_Acc_root634/roots_1S_pp_pTreweight/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()); }
  ...
  RooRealVar    alpha_1_A("alpha_1_A","tail shift", 2.56 ,2.5,2.6);
  //RooRealVar    alpha_1_A("alpha_1_A","tail shift", 2.56 ,alpha_lower,alpha_higher);
  ...
  RooRealVar    n_1_A("n_1_A","power order", n_1_init, n_lower, n_higher);
  ...
  double s1_init = 0.1; double s2_init = 0.0; double s3_init = 0.0; double s4_init = 0.0;
  ...
  else if(ptLow==6.5&&ptHigh==40&&PRw==1) pdfMASS_bkg = new RooChebychev("pdfMASS_bkg","Background",*(ws->var("mass")),RooArgList(*sl1, *sl2, *sl3));
  ...
  RooRealVar *N_Bkg = new RooRealVar("N_Bkg","fraction of component 1 in bkg",1e+4,NBkg_limit);
  ...
  RooProdPdf model("model","model with constraint",RooArgSet(*pdfMASS_Jpsi,RooArgSet(alpha_constraint, n_constraint)));
  RooAddPdf* pdfMASS_Tot = new RooAddPdf("pdfMASS_Tot","Jpsi + Bkg",RooArgList(*pdfMASS_Jpsi, *pdfMASS_bkg),RooArgList(*N_Jpsi,*N_Bkg));
...
//  ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A,VisualizeError(*fitMass,1),FillColor(kOrange));
  ...
  drawText(Form("#alpha_{J/#psi} = %.4f #pm %.4f",ws->var("alpha_1_A")->getVal(),ws->var("alpha_1_A")->getError()),text_x,text_y-y_diff*3,text_color,text_size);
  ...
  drawText(Form("n_{J/#psi} = %.4f #pm %.4f",ws->var("n_1_A")->getVal(),ws->var("n_1_A")->getError()),text_x,text_y-y_diff*5,text_color,text_size);
