...
void mass_fit(float ptLow = 20, float ptHigh = 50, float yLow = 0, float yHigh = 1.6, int PRw = 1,  bool isSkipFit = false)
{
	cout << "=== start mass_fit() ===\n";
  TStopwatch time;
  time.Start();
  ...
  // Double-sided Crystal Ball using RooCBShape for both tails
  // Left tail: positive alpha (left side)
  RooCBShape CBLeft("CBLeft", "left-tail CB", *ws->var("mass"), massMean, massSigma1, alphaL, nL);
  // Right tail: use negative alpha value directly
  RooRealVar alphaRNeg("alphaRNeg", "negative alpha for right tail", -1.5, -5.0, -0.2);
  RooCBShape CBRight("CBRight", "right-tail CB", *ws->var("mass"), massMean, massSigma1, alphaRNeg, nR);
  
  // Combine left and right CB to form double-sided CB
  RooRealVar fDCB("fDCB", "fraction of left CB in DCB", 0.5, 1e-6, 1.0);
  RooAddPdf DCB("DCB", "Double Crystal Ball", RooArgList(CBLeft, CBRight), RooArgList(fDCB));
  ...
  if (isSkipFit && !gSystem->AccessPathName(Form("%s/mass_fit_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
    ...
    fitMass = massFitModel.fitTo(*redData, Offset(true), Save(), Extended(), PrintLevel(-1), PrintEvalErrors(-1), NumCPU(28) ); //, EvalBackend("legacy")
  ...
  { return fr ? fr->findObject(n) : nullptr; };
  {
    ...
    c.SaveAs(Form("%s/mass_fit_pT%.1f_%.1f_y%.1f_%.1f.png", figDir.Data(), ptLow, ptHigh, yLow, yHigh));
  }
  ...
  fitMass->Print("V");
  TFile outMass(Form("%s/mass_fit_pT%.1f_%.1f_PRw%d.root", rootDir.Data(), ptLow, ptHigh, PRw), "RECREATE");
