...
std::pair<string, string> get_histnames(bool get_pp, bool isPR, bool isFwd, float ptLow, float ptHigh){
	...
	if(get_pp) name_ref = Form("../../Eff_Acc/roots/TnP_Jpsi/mc_eff_pp_%s_pt%.1f_%.1f_y%.1f_%.1f_accYes_mass2.6_3.5_isTnP1_isPtWeight1_NOM.root", (isPR)? "PR" : "NP", ptLow, ptHigh, yl, yh);
	if(get_pp) name_cmp = Form("../../Eff_Acc/roots/TnP_Jpsi/mc_eff_pp_%s_pt%.1f_%.1f_y%.1f_%.1f_accYes_mass2.6_3.5_isTnP1_isPtWeight1_SYSTNP.root", (isPR)? "PR" : "NP", ptLow, ptHigh, yl, yh);
...
void TNPUnc_pp(bool incl_pp, bool incl_pbpb, bool dopt){
	...
	if( !dopt ){
		for( auto idx : ROOT::TSeqI(numbin_for)){
			hist_for_PR->SetBinContent(idx+1, getUncBin(incl_pp, true , true, dopt, 3.5, 40 )) ;
			hist_for_NP->SetBinContent(idx+1, getUncBin(incl_pp, false, true, dopt, 3.5, 40 )) ;
		}
		for( auto idx : ROOT::TSeqI(numbin_mid)){
			hist_mid_PR->SetBinContent(idx+1, getUncBin(incl_pp, true , false, dopt, 6.5, 40)) ;
			hist_mid_NP->SetBinContent(idx+1, getUncBin(incl_pp, false, false, dopt, 6.5, 40)) ;
		}
	}
