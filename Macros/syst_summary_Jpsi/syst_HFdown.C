...
void syst_HFdown()
{
    ...
    string syst_type = "HF_down";

    // Path label
    string nominal_path_pb = "../Jpsi/roots/2DFit_No_Weight/Final/";
    string syst_path_pb = "../Jpsi/systematics/" + syst_type + "/roots/2DFit_No_Weight/Final/";
    ...
    for (int i = 0; i < pb_mid_pt.size(); i++) {
         // Open input files
        string temp_input_path = nominal_path_pb + pb_mid_pt[i].c_str();
        ...
        // Compute uncertainty
        // Prompt
        double PR_uncert = compute_uncertainty(1,1, n_PR_Pb_nomi, n_PR_Pb_syst);
        double PR_uncert_pb = compute_uncertainty_pb(n_PR_Pb_nomi, n_PR_Pb_syst);
        
        // Non-prompt
        double NP_uncert = compute_uncertainty(1,1, n_NP_Pb_nomi, n_NP_Pb_syst);
        double NP_uncert_pb = compute_uncertainty_pb(n_NP_Pb_nomi, n_NP_Pb_syst);
    ...
    for (int i = 0; i < pb_fwd_pt.size(); i++) {
         // Open input files
        string temp_input_path = nominal_path_pb + pb_fwd_pt[i].c_str();
        pb_nominal_input = TFile::Open(temp_input_path.c_str());
    ...
    for (int i = 0; i < pb_mid_cent.size(); i++) {
        ...
        string temp_input_path = nominal_path_pb + pb_mid_cent[i].c_str();
        pb_nominal_input = TFile::Open(temp_input_path.c_str());
    ...
    for (int i = 0; i < pb_fwd_cent.size(); i++) {
        ...
        string temp_input_path = nominal_path_pb + pb_fwd_cent[i].c_str();
        pb_nominal_input = TFile::Open(temp_input_path.c_str());
