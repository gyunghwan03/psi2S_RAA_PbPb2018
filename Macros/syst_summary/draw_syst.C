...
void draw_syst()
{
	...
	for (int i=0; i<nFiles; i++) {
		h_mid_pt_PR[i]->GetYaxis()->SetRangeUser(0,0.4);
	...
	c_mid_pt_NP->cd();
	for (int i=0; i<nFiles; i++) {
		h_mid_pt_NP[i]->GetYaxis()->SetRangeUser(0,0.5);
	...
	c_fwd_pt_PR->cd();
	for (int i=0; i<nFiles; i++) {
		h_fwd_pt_PR[i]->GetYaxis()->SetRangeUser(0,0.7);
	...
	c_fwd_cent_PR->cd();
	for (int i=0; i<nFiles; i++) {
		h_fwd_cent_PR[i]->GetYaxis()->SetRangeUser(0,1);
	...
	c_fwd_cent_NP->cd();
	for (int i=0; i<nFiles; i++) {
		h_fwd_cent_NP[i]->GetYaxis()->SetRangeUser(0,1);
