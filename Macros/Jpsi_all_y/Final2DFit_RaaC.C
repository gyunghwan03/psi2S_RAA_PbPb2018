    ...
    )
{
  ...
  //f1 = new TFile(Form("OniaRooDataSet_miniAOD_isMC0_JPsi_cent0_200_Effw0_Accw0_PtW0_TnP0_230721.root"));
  ...
  //else if(ptLow==6.5&&ptHigh==50) fCTrue = new TFile(Form("roots/2DFit_%s/CtauTrue/cent/CtauTrueResult_Inclusive_pt6.5-50.0_y0.0-2.4_muPt0.0_centrality0-200.root",DATE.Data()));
  //fCTrue = new TFile(Form("roots/2DFit_%s/CtauTrue/cent/CtauTrueResult_Inclusive_pt3.5-40.0_y1.6-2.4_muPt0.0_centrality0-180.root",DATE.Data()));
  //fCTrue = new TFile(Form("roots/2DFit_%s/CtauTrue/cent/CtauTrueResult_Inclusive_pt6.5-40.0_y0.0-1.6_muPt0.0_centrality0-180.root",DATE.Data()));
  if(ptLow==3.5&&ptHigh==40.) fCTrue = new TFile(Form("roots/2DFit_%s/CtauTrue/cent/CtauTrueResult_Inclusive_pt3.5-40.0_y1.6-2.4_muPt0.0_centrality0-180.root",DATE.Data()));
  else if(ptLow==6.5&&ptHigh==40.) fCTrue = new TFile(Form("roots/2DFit_%s/CtauTrue/cent/CtauTrueResult_Inclusive_pt6.5-40.0_y0.0-1.6_muPt0.0_centrality0-180.root",DATE.Data()));
  else if(ptLow==6.5&&ptHigh==50.) fCTrue = new TFile(Form("roots/2DFit_%s/CtauTrue/CtauTrueResult_Inclusive_pt6.5-50.0_y0.0-2.4_muPt0.0_centrality0-180.root",DATE.Data()));
...
//  ctauErrMax = 0.06;}//0.124872
  ...
  //if(ptLow==6.5 && ptHigh==50 && cLow==100) ws->factory("b_Jpsi[0.37, 0.35, 0.43]");//NP fraction for Sig
  if(ptLow==6.5 && ptHigh==40 && cLow==100) ws->factory("b_Jpsi[0.37, 0.35, 0.43]");//NP fraction for Sig
  if(ptLow==6.5 && ptHigh==50 && cLow==0) ws->factory("b_Jpsi[0.38, 0.35, 0.43]");//NP fraction for Sig
  if(ptLow==6.5 && ptHigh==50 && cLow==140) ws->factory("b_Jpsi[0.37, 0.32, 0.5]");//NP fraction for Sig
  if(ptLow==6.5&&ptHigh==9) ws->factory("b_Jpsi[0.29, 0.28, 0.32]");
  if(ptLow==3&&ptHigh==4.5) ws->factory("b_Jpsi[0.17, 0.08, 0.2]");
  if(ptLow==5.5&&ptHigh==6.5) ws->factory("b_Jpsi[0.22, 0.17, 0.25]");
  else ws->factory("b_Jpsi[0.22, 1e-8, 1.0]");//NP fraction for Sig
  
  cout << "************************************************************************" << endl;
  cout << "*************************** START MASS FIT *****************************" << endl;
  cout << "************************************************************************" << endl;
  TFile * f_fit = new TFile(Form("roots/2DFit_%s/mc_Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root", DATE.Data(), kineLabel.Data(),fEffW,fAccW,isPtW,isTnP));
  ...
  //else if(ptLow==6.5&&cLow==100) ws->factory("b_Jpsi[0.39, 0.36, 0.42]");
  //else if(ptLow==6.5&&cLow==0) ws->factory("b_Jpsi[0.39, 0.36, 1.]");
  //else ws->factory("b_Jpsi[0.22, 1e-8, 1.0]");//NP fraction for Sig
  ...
  TCanvas* c_G =  new TCanvas("canvas_G","My plots",1108,104,550,520);
  c_G->cd();
