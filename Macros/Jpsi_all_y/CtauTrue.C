    ...
    )
{
  ...
  ws->factory("fDSS[0.32, 0., 1.]");
  ws->factory("fDSS1[0.314, 0., 1.]");}
  ...
  ws->factory("lambdaDSS[0.3315, 1e-6, 1.0]");
  ws->factory("lambdaDSS2[0.5018, 1e-6, 1.0]");
  ...
  ws->factory("fDSS[0.8, 0., 1.]");
  ws->factory("fDSS1[0.8, 0., 1.]");}
  //else if(ptLow>=6.5&&ptHigh<=25.){
  //ws->factory("lambdaDSS[0.3315, 1e-6, 1.0]");
  //ws->factory("lambdaDSS2[0.5968, 1e-6, 1.0]");
  //ws->factory("lambdaDSS3[0.4854, 1e-6, 1.0]");
  //ws->factory("fDSS[0.55, 0., 1.]");
  //ws->factory("fDSS1[0.4, 0., 1.]"); }
  else if(ptLow>=6.5&&ptHigh<=25.){
  ws->factory("lambdaDSS[0.3315, 1e-6, 1.0]");
  ws->factory("lambdaDSS2[0.5968, 1e-6, 1.0]");
  ws->factory("lambdaDSS3[0.4854, 1e-6, 1.0]");
  ws->factory("fDSS[0.8245, 0., 1.]");
  ws->factory("fDSS1[0.4971, 0., 1.]"); }
  //11-13
  //ws->factory("fDSS[0.8245, 0., 1.]");
  //ws->factory("fDSS1[0.4971, 0., 1.]"); }
  //9.5-11
  //ws->factory("fDSS[0.822, 0., 1.]");
  //ws->factory("fDSS1[0.4972, 0., 1.]"); }
  //8.5-9.5
  //ws->factory("fDSS[0.8, 0., 1.]");
  //ws->factory("fDSS1[0.492, 0., 1.]"); }
  //7.5-8.5
  //ws->factory("fDSS[0.8, 0., 1.]");
  //ws->factory("fDSS1[0.487, 0., 1.]"); }
  //6.5-7.5
  //ws->factory("fDSS[0.8, 0., 1.]");
  //ws->factory("fDSS1[0.482, 0., 1.]"); }
  else if(ptLow>=25&&ptHigh<=50.){
  ws->factory("lambdaDSS[0.3115, 1e-6, 1.0]");
  ws->factory("lambdaDSS2[0.5968, 1e-6, 1.0]");
  ws->factory("lambdaDSS3[0.5854, 1e-4, 1.0]");
  ws->factory("fDSS[0.8, 0., 1.]");
  ws->factory("fDSS1[0.44, 0., 1.]"); }
  //ws->factory("fDSS1[0.341111, 0., 1.]"); }
  else {
  ws->factory("lambdaDSS[0.3315, 1e-6, 1.0]");
  ws->factory("lambdaDSS2[0.5968, 1e-6, 1.0]");
  ws->factory("lambdaDSS3[0.4854, 1e-6, 1.0]");
  ws->factory("fDSS[0.45, 0., 1.]");
  ws->factory("fDSS1[0.4, 0., 1.]"); }
  /*
  ws->factory("lambdaDSS[0.5769, 0.5768, 0.577]");
  ws->factory("lambdaDSS2[0.3680, 0.3675, 0.3681]");
  ws->factory("lambdaDSS3[.582, 0.581, 0.583]");
