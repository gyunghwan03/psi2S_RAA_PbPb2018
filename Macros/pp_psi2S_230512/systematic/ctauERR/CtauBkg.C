    ...
    )
{
  ...
  RooAddPdf* pdfCTAUERR_Tot = (RooAddPdf*)fCErr->Get("pdfCTAUERR_Tot");
  ...
  ws->data("dataw_Tot")->getRange(*ws->var("ctau3DErr"), ctauErrMin, ctauErrMax);
  cout<<"####################################" <<endl;
  ...
  ws->factory("lambdaDF_Bkg[0.9, 1e-6, 1]");
  ws->factory("lambdaDSS_Bkg[0.4, 1e-6, 1.]");}
  else if(ptLow==6.5&&ptHigh==50){
  ws->factory("b_Bkg[0.07, 0., 1.]");//NP fraction for bkg
  ws->factory("fDFSS[0.02, 0., 1.]");
  ws->factory("fDLIV[0.01, 0., 1]");
  ...
  ws->factory("lambdaDDS_Bkg[0.576, 1e-6, 1.]");
  ...
  ws->factory("b_Bkg[0.7, 0., 1.]");//NP fraction for bkg
  ...
  ws->factory("b_Bkg[0.23, 0., 1.]");//NP fraction for bkg
  ws->factory("fDFSS[0.61, 0., 1.]");
  ws->factory("fDLIV[0.44, 0., 1]");
  ws->factory("lambdaDDS_Bkg[0.4, 1e-3, 1.]");
  ws->factory("lambdaDF_Bkg[0.54, 1e-3, 1]");
  ws->factory("lambdaDSS_Bkg[0.23, 1e-3, 1.]");}
  ...
  if ( (ptLow==15&&ptHigh==20) || (ptLow==6.5&&ptHigh==50) ) {
    ws->var("N_Bkg")->setMax(1000000);
  }
  ...
  ws->factory(Form("RooExtendPdf::%s(%s,%s)", "pdfTot_Bkg", "pdfCTAUCOND_Bkg", "N_Bkg"));//N_Bkg is number of bkg from dataw_Tot

  RooProdPdf pdfPR("pdfCTAU_BkgPR", "", *ws->pdf("pdfCTAUERR_Tot"), Conditional( *ws->pdf("pdfCTAUCOND_BkgPR"), RooArgList(*ws->var("ctau3D")) ));
  ws->import(pdfPR);
  RooProdPdf pdfNoPR("pdfCTAU_BkgNoPR", "", *ws->pdf("pdfCTAUERR_Tot"), Conditional( *ws->pdf("pdfCTAUCOND_BkgNoPR"), RooArgList(*ws->var("ctau3D")) ));
  ws->import(pdfNoPR);
  ...
  TH1D* hTot = (TH1D*)ws->data("dataw_Tot")->createHistogram(("hTot"), *ws->var("ctau3D"),Binning(nCtauBins,ctauLow,ctauHigh));
...
//  if(ptLow==3&&ptHigh==6.5) {ctauMin=-2.; ctauMax=3.65;}
  if (ptLow==9&&ptHigh==12) ctauMin=-2.5;
    ...
    ctauMin = -0.55; ctauMax = 2.6;
  } else if(ptLow==25&&ptHigh==30) ctauMin = -1.5;
  else if(ptLow==30&&ptHigh==50) ctauMin = -1.5;
  ...
  RooDataSet* dataToFit = (RooDataSet*)dataw_Tot->reduce(Form("ctau3D>=%.f&&ctau3D<=%.f",ctauMin, ctauMax))->Clone("dataw_Tot");
//  RooDataSet* dataToFit = (RooDataSet*)dataw_Tot->reduce(Form("ctau3D>=%.f&&ctau3D<=%.f",-2.0, 4.0))->Clone("dataw_Tot");
  ...
  double normBkg = 1.0;
  if (ws->data("dataw_Tot")){normBkg = ws->data("dataToFit")->sumEntries()*normDSTot/ws->data("dataw_Tot")->sumEntries();}

  cout << "normDSTot = " << normDSTot << " normBkg : " << normBkg << endl;

  bool isWeighted = ws->data("dataw_Tot")->isWeighted();
  //isWeighted = false;
  ...
  ws->data("dataw_Tot")->plotOn(myPlot2_E, Name("data_ctauBkg"), DataError(RooAbsData::SumW2), XErrorSize(0), MarkerColor(kBlack), LineColor(kBlack), MarkerSize(0.7));
  ws->pdf("pdfCTAU_Bkg_Tot")->plotOn(myPlot2_E, Name("ctauBkg_Tot"),  Normalization(normBkg, RooAbsReal::NumEvent), NumCPU(nCPU),
      ProjWData(RooArgSet(*ws->var("ctau3DErr")), *ws->data("dataw_Tot"), kTRUE),
  ...
  if (ws->pdf("pdfCTAU_BkgPR")) {ws->pdf("pdfCTAU_Bkg_Tot")->plotOn(myPlot2_E,Name("BKGPR"),Components(RooArgSet(*ws->pdf("pdfCTAU_BkgPR"))), ProjWData(RooArgSet(*ws->var("ctau3DErr")), *ws->data("dataw_Tot"), kTRUE), Normalization(normBkg, RooAbsReal::NumEvent), LineColor(kRed+2), Precision(1e-4), NumCPU(nCPU));}
  if (ws->pdf("pdfCTAU_BkgNoPR")) {ws->pdf("pdfCTAU_Bkg_Tot")->plotOn(myPlot2_E,Name("BKGNoPR"),Components(RooArgSet(*ws->pdf("pdfCTAU_BkgNoPR"))), ProjWData(RooArgSet(*ws->var("ctau3DErr")), *ws->data("dataw_Tot"), kTRUE), Normalization(normBkg, RooAbsReal::NumEvent), LineColor(kOrange+10), Precision(1e-4), NumCPU(nCPU));}
  ws->pdf("pdfCTAU_Bkg_Tot")->plotOn(myPlot2_E,Name("PDF"),  Normalization(normBkg, RooAbsReal::NumEvent), NumCPU(nCPU), ProjWData(RooArgSet(*ws->var("ctau3DErr")), *ws->data("dataw_Tot"), kTRUE), LineColor(kBlack), Precision(1e-4));

  myPlot2_E->GetYaxis()->SetRangeUser(10e-2, 10e7);
  TH1* h = ws->data("dataw_Tot")->createHistogram("hist", *ws->var("ctau3D"), Binning(myPlot_E->GetNbinsX(),myPlot_E->GetXaxis()->GetXmin(),myPlot_E->GetXaxis()->GetXmax()));
