...
dir_path = '2DFitResult_pt12.0-15.0_y0.0-1.6_muPt0.0_PRw_Effw0_Accw0_PtW0_TnP0.root'# need slash at last
infile = TFile(dir_path)
fit_result = infile.Get('fitresult_pdfCTAUMASS_Tot_dsToFit')
...
for idx in range(len(floating_list)):
    if (floating_list[idx].getVal() - floating_list[idx].getError() -100 > floating_list[idx].getRange()[0]) and (floating_list[idx].getVal() + floating_list[idx].getError() < floating_list[idx].getRange()[1]) :
        a = '[Okay]'
    else:
        a = '\033[31m' + '[BAAAAAD]'
    print(f'{a} // Variable: {floating_list[idx].GetName()} \033[0m // Final Value : {floating_list[idx].getVal()} // Lower Limit : {floating_list[idx].getRange()[0]} // Upper Limit : {floating_list[idx].getRange()[1]}')
