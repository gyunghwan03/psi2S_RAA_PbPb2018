    ...
    )
{
  ...
  TString kineCut;
  f2 = new TFile(Form("../../Eff_Acc/roots_1S_PbPb/ctau3D_cut_ptBin_%sMC_y0.0-2.4.root", fname.Data()));
  f3 = new TFile(Form("../../Eff_Acc/roots_1S_PbPb/ctau3D_cut_centBin_%sMC_y0.0-2.4.root", fname.Data()));
  TH1D *h_Lcut;
  if(yLow==1.6&&yHigh==2.4&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("hpt_ctau_1"); }
  else if(yLow==0&&yHigh==1.6&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("hpt_ctau_2"); }
  else if(yLow==1.6&&yHigh==2.4&&ptLow==3.5&&ptHigh==40) { h_Lcut = (TH1D *)f3->Get("hcent_ctau_1"); }
  else if(yLow==0&&yHigh==1.6&&ptLow==6.5&&ptHigh==40) { h_Lcut = (TH1D *)f3->Get("hcent_ctau_2"); }
  else if(yLow==1.6&&yHigh==2.4&&ptLow==3.5&&ptHigh==40&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("h_ctau_int_for"); }
  else if(yLow==0&&yHigh==1.6&&ptLow==6.5&&ptHigh==40&&cLow==0&&cHigh==180) { h_Lcut = (TH1D *)f2->Get("h_ctau_int_mid"); }
  l_cut = h_Lcut->GetBinContent(h_Lcut->GetXaxis()->FindBin(ptLow));
...
//  f1 = new TFile("/Users/hwan/tools/2019/CMS/JPsi/Jpsi_v2_PbPb2018/skimmedFiles/vnCut/OniaRooDataSet_isMC0_JPsi_pt3.0-4.5_y1.6-2.4_muPt0.0_centrality20-120_m2.6-3.5_OS_Effw0_Accw0_PtW1_TnP1_211110.root");
  ...
  else if(ptLow==3.5&&ptHigh==40&&cLow==60&&cHigh==100) { NJpsi_limit = 5e+5;}
