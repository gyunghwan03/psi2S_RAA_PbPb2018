...
void mass_fit_wide(float ptLow = 3, float ptHigh = 10000, float yLow = 1.6, float yHigh = 2.4, int PRw = 1, bool isSkipFit = false)
{
	cout << "=== start mass_fit_wide() ===\n";
  TStopwatch time;
  time.Start();
  ...
  TString rootDirMc = Form("mc_roots%s/pT%.1f_%.1f_y%.1f_%.1f", userLabel.Data(), ptLow, ptHigh, yLow, yHigh);
  ...
  // --- data ---
  //string fileNameData = "../roodataset/RooDataSet_miniAOD_isMC0_Jpsi_cent0_200_Effw0_Accw0_PtW0_TnP0_Run399651_399664_DCS.root";
  string fileNameData = "../../skimmedFiles/OniaRooDataSet_isMC0_JPsi_pp_y0.00_2.40_Effw0_Accw0_PtW1_TnP1_230209.root";
  ...
  RooRealVar massSigma12("massSigma12", "ratio of sigma 1 vs 2", 1.5, 0.5, 20);
  ...
  RooRealVar alphaLR("alphaLR", "", 2.0, 0.01, 10.0);
  ...
  // Gauss
  RooRealVar massSigma1G("massSigma1G", "ratio of sigma1 vs sigmaG", 1.1, 1, 100);
  ...
  RooAddPdf massSigPdf("massSigPdf", "", RooArgList(CB1, CB2, massG,massG2), RooArgList(fCB1,fCB2,fGaus1), true);
  ...
  RooChebychev massBkgPdf("massBkgPdf", "", *ws->var("mass"), RooArgList(massSl1, massSl2)); // low pt: 9.5-11, 11-13

  // --- fit ---
  RooRealVar nSigMass("nSigMass", "mass signal yield", 2000, 1e2, 1e8);
  RooRealVar nBkgMass("nBkgMass", "mass bkg yield", 1e6, 1e2, 2e7);
  ...
  // --- use mc result ---
  //TFile finMC(Form("roots_MC/Mass/mc_MassFitResult_%s_PRw_Effw%d_Accw%d_PtW%d_TnP%d.root",kineLabel.Data(),0,0,0,0), "READ");
  TFile finMC(Form("%s/mc_mass_pT%.1f_%.1f.root", rootDirMc.Data(), ptLow, ptHigh), "READ");
  ...
  syncVar(fCB2, *fitMcMass);
  syncVar(fGaus1, *fitMcMass);

  syncVar(massSigma12, *fitMcMass);
  syncVar(massSigma1G, *fitMcMass);
  syncVar(massSigma1G2, *fitMcMass);
  ...
  massSigma12.setConstant();
  massSigma1G.setConstant();
  massSigma1G2.setConstant();
  ...
  if (isSkipFit && !gSystem->AccessPathName(Form("%s/mass_fit_pT%.1f_%.1f.root", rootDir.Data(), ptLow, ptHigh), kReadPermission)) {
    ...
    fitMass = massFitModel.fitTo(*redData, Offset(true), Save(), Extended(), PrintLevel(-1), PrintEvalErrors(-1),  NumCPU(nCPU) ); //, EvalBackend("legacy")
  ...
  { return fr ? fr->findObject(n) : nullptr; };
  {
    ...
    // --- CMS/info latex ---
    {
      ...
      int k = 0;
      tx.DrawLatex(x, y0 + dy * k++, "CMS PbPb #sqrt{s_{NN}} = 5.36 TeV");
      tx.DrawLatex(x, y0 + dy * k++, "Run399651-399664 (DCS Json)");
      if (yLow == 0)
