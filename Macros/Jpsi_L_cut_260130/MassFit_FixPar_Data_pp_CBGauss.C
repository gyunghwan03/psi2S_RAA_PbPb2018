...
void MassFit_FixPar_Data_pp_CBGauss(
    ...
    )
{
  ...
  TFile * f_fit = new TFile(Form("./roots_MC/DCBGauss/mc_MassFitResult_%s_%sw_Effw%d_Accw%d_PtW%d_TnP%d.root", kineLabel.Data(), "PR", fEffW, fAccW, isPtW, isTnP));
  ...
  Double_t f_MC_value = ws_fit->var("f_CB2")->getVal();
  Double_t f_MC_value_err = ws_fit->var("f_CB2")->getError();
  Double_t kG_MC_value = ws_fit->var("kG")->getVal();
  Double_t kG_MC_value_err = ws_fit->var("kG")->getError();
  Double_t sigma_MC_value = ws_fit->var("sigma_1_A")->getVal();
  Double_t sigma_MC_value_err = ws_fit->var("sigma_1_A")->getError();
  Double_t fG_MC_value = ws_fit->var("fG")->getVal();
  Double_t fG_MC_value_err = ws_fit->var("fG")->getError();
  ...
  //Double_t sigmaG_lower = sigmaG_MC_value-(sigma_index*sigmaG_MC_value_err);
  //Double_t sigmaG_higher = sigmaG_MC_value+(sigma_index*sigmaG_MC_value_err);
  ...
  //SIGNAL
  RooRealVar    mean_1("m_{J/#Psi}1","mean of the signal gaussian mass PDF",pdgMass.JPsi, pdgMass.JPsi-0.05, pdgMass.JPsi+0.05);
  RooRealVar    mean_2("m_{J/#Psi}2","mean of the signal gaussian mass PDF",pdgMass.JPsi, pdgMass.JPsi-0.05, pdgMass.JPsi+0.05);
  RooRealVar    mean_G("m_{J/#Psi}G","mean of the signal gaussian mass PDF",pdgMass.JPsi, pdgMass.JPsi-0.05, pdgMass.JPsi+0.05);
  ...
  RooRealVar    alpha_1_A("alpha_1_A","tail shift", alpha_1_init, alpha_1_init*0.7, alpha_1_init*1.3);
  RooFormulaVar alpha_2_A("alpha_2_A","1.0*@0",RooArgList(alpha_1_A));
  RooRealVar    n_1_A("n_1_A","power order", n_1_init, n_1_init*0.7, n_1_init*1.3);
  RooFormulaVar n_2_A("n_2_A","1.0*@0",RooArgList(n_1_A) );
  RooRealVar   *f_CB2 = new RooRealVar("f_CB2","cb fraction", f_init, f_init*0.7, f_init*1.3);
  RooRealVar   kG("kG","Gauss / CB sigma ratio", kG_MC_value, kG_MC_value*0.7, kG_MC_value*1.3 );
  RooRealVar   *fG = new RooRealVar("fG","Gaussian fraction in signal", fG_MC_value, fG_MC_value*0.7, fG_MC_value*1.3 );
  //Set up crystal ball shapes
  RooCBShape* cb_1_A = new RooCBShape("cball_1_A", "cystal Ball", *(ws->var("mass")), mean_1, sigma_1_A, alpha_1_A, n_1_A);
  //DOUBLE CRYSTAL BALL
  RooCBShape* cb_2_A = new RooCBShape("cball_2_A", "cystal Ball", *(ws->var("mass")), mean_2, sigma_2_A, alpha_2_A, n_2_A);
  RooAddPdf* pdfDoubleCB =
      new RooAddPdf("pdfDoubleCB","Double Crystal Ball",
                    RooArgList(*cb_2_A, *cb_1_A),  // 첫 번째가 f_CB2에 해당
                    RooArgList(*f_CB2));
  //Gaussian
  RooFormulaVar sigmaG("sigmaG","@0*@1", RooArgList(sigma_1_A, kG) );
  RooGaussian* gauss = new RooGaussian("gauss","Gaussian PDF", *(ws->var("mass")), mean_G, sigmaG);
  //pdfMASS_Jpsi = new RooAddPdf("pdfMASS_Jpsi","Signal ",RooArgList(*cb_1_A,*cb_2_A), RooArgList(*f) );
  //pdfMASS_Jpsi = new RooAddPdf("pdfMASS_Jpsi","Signal",RooArgList(sigG,*gauss), RooArgList(*fG,*f_CB2) );
  RooAddPdf* pdfMASS_Jpsi =
      new RooAddPdf("pdfMASS_Jpsi","Signal (DoubleCB + Gauss)",
                    RooArgList(*gauss, *pdfDoubleCB), // 첫 번째가 fG
                    RooArgList(*fG));
  ...
  else if(ptLow==6.5&&ptHigh==40) pdfMASS_bkg = new RooChebychev("pdfMASS_bkg","Background",*(ws->var("mass")),RooArgList(*sl1, *sl2));
  ...
  ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A, Name("cb_1_A"),Components(RooArgSet(*pdfMASS_bkg, *cb_1_A)), LineColor(44));
  ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A, Name("cb_2_A"),Components(RooArgSet(*pdfMASS_bkg, *cb_2_A)), LineColor(46));
  ws->pdf("pdfMASS_Tot")->plotOn(myPlot2_A, Name("gauss"),Components(RooArgSet(*pdfMASS_bkg, *gauss)), LineColor(8));
  ...
  leg_B->AddEntry(myPlot2_A->findObject("cb_1_A"),"J/#psi CB","l");
  leg_B->AddEntry(myPlot2_A->findObject("gauss"),"J/#psi Gauss","l");
  leg_B->Draw("same");
  ...
  drawText(Form("m_{J/#psi}1 = %.4f #pm %.4f",ws->var("m_{J/#Psi}1")->getVal(),ws->var("m_{J/#Psi}1")->getError()),text_x,text_y-y_diff*2,text_color,text_size);
  drawText(Form("m_{J/#psi}2 = %.4f #pm %.4f",ws->var("m_{J/#Psi}2")->getVal(),ws->var("m_{J/#Psi}2")->getError()),text_x,text_y-y_diff*2,text_color,text_size);
  drawText(Form("m_{J/#psi}G = %.4f #pm %.4f",ws->var("m_{J/#Psi}G")->getVal(),ws->var("m_{J/#Psi}G")->getError()),text_x,text_y-y_diff*2,text_color,text_size);
  drawText(Form("#alpha_{J/#psi} = %.4f (fixed)",ws->var("alpha_1_A")->getVal()),text_x,text_y-y_diff*3,text_color,text_size);
  drawText(Form("f_{J/#psi} = %.4f (fixed)",ws->var("f_CB2")->getVal()),text_x,text_y-y_diff*4,text_color,text_size);
