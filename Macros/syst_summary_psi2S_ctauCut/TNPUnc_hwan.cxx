...
std::pair<string, string> get_histnames(bool get_pbpb, bool isPR, bool isFwd, float ptLow, float ptHigh, int cbinLow, int cbinHigh ){
	string name_ref ="";
	//if( isPR && get_pbpb ) name_ref = "../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_psi2s_PtW1_tnp1_20230729.root";
	//if(!isPR && get_pbpb) name_ref = "../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_psi2S_PtW1_tnp1_20230729.root";
	//if( isPR && get_pbpb ) name_ref = "../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_psi2s_PtW1_tnp1_20240405_ppWfunc.root";
	//if(!isPR && get_pbpb) name_ref = "../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_psi2S_PtW1_tnp1_20240322_ppWfunc.root";
	//if( isPR && get_pbpb ) name_ref = "../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_prompt_pbpb_psi2s_PtW1_tnp1_20240322.root";
	//if(!isPR && get_pbpb) name_ref = "../../Eff_Acc/roots/mc_eff_vs_pt_cent_0_to_180_rap_nprompt_pbpb_psi2S_PtW1_tnp1_20231226.root";
	string name_cmp="";
	double yl, yh;
	if( isFwd){ yl=1.6; yh=2.4;}
	if(!isFwd){ yl=0.0; yh=1.6;}
	//if(get_pbpb) name_cmp = Form("../../Eff_Acc/roots/TnP/mc_eff_PbPb_%s_pt%.1f_%.1f_y%.1f_%.1f_accYes_mass2.0_4.8_cent%d_%d_isTnP1_isPtWeight1_SYSTNP.root", (isPR)? "PR" : "NP", ptLow, ptHigh, yl, yh, cbinLow, cbinHigh);
	if(get_pbpb) name_ref = Form("../../Eff_Acc/roots/TnP/mc_eff_PbPb_%s_pt%.1f_%.1f_y%.1f_%.1f_accYes_mass3.4_4.1_cent%d_%d_isTnP1_isPtWeight1_NOM.root", (isPR)? "PR" : "NP", ptLow, ptHigh, yl, yh, cbinLow, cbinHigh);
	if(get_pbpb) name_cmp = Form("../../Eff_Acc/roots/TnP/mc_eff_PbPb_%s_pt%.1f_%.1f_y%.1f_%.1f_accYes_mass3.4_4.1_cent%d_%d_isTnP1_isPtWeight1_SYSTNP.root", (isPR)? "PR" : "NP", ptLow, ptHigh, yl, yh, cbinLow, cbinHigh);
	return std::make_pair(name_ref, name_cmp);
};

void TNPUnc_hwan(bool incl_pp, bool incl_pbpb, bool dopt){
	string tag_kine = (dopt) ? "pt" : "cent"; 
	string tag_coll = (incl_pp) ? (incl_pbpb) ? "" : "_ppOnly" : "_PbPbOnly";
	TFile* output = new TFile(Form("./syst_roots/syst_%s_TNP%s.root", tag_kine.c_str(), tag_coll.c_str()),"recreate");
	
	const size_t numbin_for =  (dopt) ? 5 : 4;
	const size_t numbin_mid =  (dopt) ? 7 : 8;
	double ptBin_for[6] = {0,3.5,6.5,9,12,40};
    double ptBin_mid[8] = {0,6.5,9,12,15,20,25,40};
    //double centBin_for[5] = {0,20,60,100,180};
	//double centBin_mid[9] = {0,10,20,30,40,60,80,100,180};
	...
	auto getUncBin = [&](bool isPbPb, bool isPR, bool isFwd, bool isPT, float ptLow, float ptHigh, int cbinLow, int cbinHigh){
		...
		string name_reco_ref = "hreco_tnp_idNom_trkNom_trgNom";
		...
		for( int cv : {1,2,3}){
			...
			for( int md : {1,2,3,4,5}){
				...
				std::cout <<"NAME : " << the_string.Data() <<  " cv : " << cv <<  " COUNT_TAG : " << COUNT_TAG << std::endl;
	...
	if( !dopt ){
		for( auto idx : ROOT::TSeqI(numbin_for)){
			hist_for_PR->SetBinContent(idx+1, getUncBin(incl_pbpb, true , true, dopt, 3.5, 50 , (int) centBin_for[idx]*2, (int) centBin_for[idx+1]*2 )) ;
			hist_for_NP->SetBinContent(idx+1, getUncBin(incl_pbpb, false, true, dopt, 3.5, 50 , (int) centBin_for[idx]*2, (int) centBin_for[idx+1]*2 )) ;
		}
		for( auto idx : ROOT::TSeqI(numbin_mid)){
			hist_mid_PR->SetBinContent(idx+1, getUncBin(incl_pbpb, true , false, dopt, 6.5, 50 , (int) centBin_mid[idx]*2, (int) centBin_mid[idx+1]*2 )) ;
			hist_mid_NP->SetBinContent(idx+1, getUncBin(incl_pbpb, false, false, dopt, 6.5, 50 , (int) centBin_mid[idx]*2, (int) centBin_mid[idx+1]*2 )) ;
		}
	}
