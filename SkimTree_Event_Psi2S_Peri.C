...
void SkimTree_Event_Psi2S_Peri(int nevt=-1, bool isMC = false, int kTrigSel = kTrigJpsi, int hiHFBinEdge = 0, int PDtype = 2) 
{
  ...
  TString fnameDataReRecoPeri = "/work2/Oniatree/ReReco/DoubleMuonPeri/OniaTree_miniAOD_HIDoubleMuonPsiPeriPD_addQVect_merged.root";
...
//  mytree->SetBranchAddress("rpAng", rpAng, &b_rpAng);
  ...
  TFile* newfile;
  newfile = new TFile(Form("OniaFlowSkim_%sTrig_%sPD_miniAOD_isMC%d_%s_230513.root",fTrigName[trigIndx].Data(),fPD.Data(),isMC,fCentSelHF.Data()),"recreate");
