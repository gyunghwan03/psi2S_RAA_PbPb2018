...
// v6 : Reco_QQ_size == 1 is applied only to the reco numerator loop,
//      not to the GEN denominator.
...
static bool IsAcceptanceQQ2018(double pt, double eta)
{
...
static double findNcoll(int hiBin)
{
...
static double GetPtWeight(TF1 *fW, double pt, double y)
{
  (void)y;
  if (!fW)
    return 1.0;
...
static bool PassMuonQualityLikeOniaEff(Int_t selectionType, Int_t nTrkWMea, Int_t nPixWMea,
...
static bool ComputePbPbTnPWeight(double muPlPt, double muPlEta, ULong64_t muPlTrig,
...
static bool ComputePbPbTnPWeightPpLike(double muPlPt, double muPlEta,
...
static bool BranchUsesShort(TTree *tree, const char *branchName)
{
  if (!tree)
    return false;
...
static TH1D *BuildWeightedEfficiencyHist(TH1D *num, TH1D *den, const char *name, const char *title)
{
  if (!num || !den)
    return nullptr;
...
static TH2D *BuildWeightedEfficiencyHist(TH2D *num, TH2D *den, const char *name, const char *title)
{
  if (!num || !den)
    return nullptr;
...
void get_Eff_JPsi_pbpb_hwan_v6(
    int state = 2,
    bool isTnP = true, int isPtWeight = 0,
    bool isMC = true,
    float ptLow = 3.0, float ptHigh = 50.0,
    float yLow = 0.0, float yHigh = 2.4,
    float cLowInput = 0, float cHighInput = 180,
    long nEvt = -1,
    bool isNCollW = true, bool isGenW = true,
    bool requireRecoMuMatched = true,
    bool useLegacy4PtWInputs = false, bool usePpLikeTnP = false,
    const char *ptWeightOverrideDir = "",
    const char *outputTag = "func_260513_2exp_v6")
{
  cout << "Start get_Eff_JPsi_pbpb_hwan_v6()\n";
  // Enable Sumw2 before any fill so weighted histograms keep statistical errors.
  TH1::SetDefaultSumw2(kTRUE);

  if (state != 1 && state != 2)
  {
    cout << "[ERROR] state must be 1(prompt) or 2(nonprompt). Got " << state << "\n";
    return;
  }

  // v4-compatible entry point:
  //   state=1/2 selects prompt/nonprompt, isTnP toggles TnP.
  //   isPtWeight=0 means nominal pT weight, matching the v4 convention.
  //   isPtWeight=-999 disables pT weight for explicit cross-checks.
  const bool isPr = (state == 1);
  const bool isTnPW = isTnP;
  const bool isPtW = (isPtWeight != -999);
  if (isPtWeight != 0 && isPtWeight != -999)
    cout << "[WARN] pT-weight up/down systematics are not implemented in v6; using nominal pT weight.\n";
  if (!isMC)
    cout << "[WARN] v6 is an MC efficiency macro. isMC=false is ignored.\n";
  if (ptLow != 3.0f || ptHigh != 50.0f || yLow != 0.0f || yHigh != 2.4f)
    cout << "[WARN] pt/y arguments are kept for v4 call compatibility; v6 uses efficiency_1d reduced phase space.\n";
  ...
  const TString ptWeightOverrideDirUsed = ptWeightOverrideDir ? TString(ptWeightOverrideDir) : TString("");
  if (isPtW)
  {
    if (useLegacy4PtWInputs)
      cout << "[INFO] using legacy split-rapidity pT weight search order.\n";

    const TString baseDir = resolveBaseDir();
    const TString compareDir = baseDir + "/../compareDataToMC";
    const TString hanseulDir = "/data/users/hanseul/jpsi_2603/compareDataToMC/roots";
    const TString overrideDir = ptWeightOverrideDirUsed;
    const bool usePtWeightOverride = overrideDir.Length() > 0;
    ...
    if (usePtWeightOverride)
    {
      cout << "[INFO] using pT-weight override directory: " << overrideDir << "\n";
      if (isPr)
      {
        ptWeightMidPaths = {Form("%s/ratioDataMC_AA_Jpsi_DATA_mid.root", overrideDir.Data())};
        ptWeightFwdPaths = {Form("%s/ratioDataMC_AA_Jpsi_DATA_fwd.root", overrideDir.Data())};
      }
      else
      {
        ptWeightMidPaths = {Form("%s/ratioDataMC_AA_BtoJpsi_DATA_mid.root", overrideDir.Data())};
        ptWeightFwdPaths = {Form("%s/ratioDataMC_AA_BtoJpsi_DATA_fwd.root", overrideDir.Data())};
      }
    }
    else if (isPr)
    {
      ptWeightMidPaths = {
          Form("%s/ratioDataMC_AA_Jpsi_DATA_y0_1p6_260505_2exp.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_Jpsi_DATA_All_y_211218.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_Jpsi_DATA_y0_1p6_260503.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_Jpsi_DATA_ctauCut_y0_2p4_260410.root", hanseulDir.Data()),
      ...
      ptWeightFwdPaths = {
          Form("%s/ratioDataMC_AA_Jpsi_DATA_y1p6_2p4_260505_2exp.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_Jpsi_DATA_Forward_y_211218.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_Jpsi_DATA_ctauCut_y1p6_2p4_260410.root", hanseulDir.Data()),
    ...
    {
      ptWeightMidPaths = {
          Form("%s/ratioDataMC_AA_BtoJpsi_DATA_y0_1p6_260505_2exp.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_Jpsi_DATA_All_y_211218.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_BtoJpsi_DATA_ctauCut_y0_2p4_260410.root", hanseulDir.Data()),
      ...
      ptWeightFwdPaths = {
          Form("%s/ratioDataMC_AA_BtoJpsi_DATA_y1p6_2p4_260505_2exp.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_Jpsi_DATA_Forward_y_211218.root", compareDir.Data()),
          Form("%s/ratioDataMC_AA_BtoJpsi_DATA_ctauCut_y1p6_2p4_260410.root", hanseulDir.Data()),
  ...
  // User-defined centrality range. The pt/y phase space stays fixed to match efficiency_1d.C.
  const int cLow = static_cast<int>(cLowInput);
  const int cHigh = static_cast<int>(cHighInput);
  ...
  const TString stateLabel = isPr ? "prompt" : "nprompt";
  const TString ptWeightLabel = isPtW ? "nomi" : "1";
  const TString noPtWeightLabel = isPtW ? "" : "_noPtW";
  const TString outputTagStr = (outputTag && TString(outputTag).Length() > 0)
                                   ? TString(outputTag)
                                   : TString("func_260505_2exp_v6");
  std::string outFilePath = Form(
      "%s/mc_eff_vs_pt_cent_%d_to_%d_rap_%s_pbpb_JPsi_PtW%s_tnp%d_%s%s.root",
      outDir.Data(), cLow, cHigh, stateLabel.Data(), ptWeightLabel.Data(),
      isTnPW ? 1 : 0, outputTagStr.Data(), noPtWeightLabel.Data());
  ...
  TH2D *hist_eff2d_den_mid_fine_noptw = new TH2D("hist_eff2d_den_mid_fine_noptw", ";p_{T} (GeV/c);Centrality", 67, 6.5, 40.0, nCentBinsMid, centBinsMid);
  TH2D *hist_eff2d_num_mid_fine_noptw = new TH2D("hist_eff2d_num_mid_fine_noptw", ";p_{T} (GeV/c);Centrality", 67, 6.5, 40.0, nCentBinsMid, centBinsMid);
  TH2D *hist_eff2d_den_fwd_fine_noptw = new TH2D("hist_eff2d_den_fwd_fine_noptw", ";p_{T} (GeV/c);Centrality", 73, 3.5, 40.0, nCentBinsFwd, centBinsFwd);
  TH2D *hist_eff2d_num_fwd_fine_noptw = new TH2D("hist_eff2d_num_fwd_fine_noptw", ";p_{T} (GeV/c);Centrality", 73, 3.5, 40.0, nCentBinsFwd, centBinsFwd);
  ...
  for (long iev = 0; iev < nEvt; ++iev)
  {
    ...
    for (Int_t irqq = 0; irqq < Gen_QQ_size; ++irqq)
    {
      ...
      if (std::abs(gen_y) < 1.6)
      {
        ...
        hist_eff2d_den_mid_fine_noptw->Fill(gen_pt, Centrality, denWeightNoPt);
      }
      else
      {
        ...
        hist_eff2d_den_fwd_fine_noptw->Fill(gen_pt, Centrality, denWeightNoPt);
      }
    ...
    for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
    {
      ...
      if (absRecoY < 1.6)
      {
        ...
        hist_eff2d_num_mid_fine_noptw->Fill(pt_, Centrality, recoWeightNumNoPt);
      }
      else if (absRecoY < 2.4)
      {
        ...
        hist_eff2d_num_fwd_fine_noptw->Fill(pt_, Centrality, recoWeightNumNoPt);
      }
  ...
  const int legacyPtWLabel = 1;
  hist_eff_mid->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_%d_to_%d_absy0_1p6",
                             isTnPW ? 1 : 0, legacyPtWLabel, cLow, cHigh));
  hist_eff_fwd->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_cent_%d_to_%d_absy1p6_2p4",
                             isTnPW ? 1 : 0, legacyPtWLabel, cLow, cHigh));
  hist_eff_cent_mid->SetName(Form("mc_eff_vs_cent_TnP%d_PtW%d_pt_6p5_to_40_absy0_1p6",
                                  isTnPW ? 1 : 0, legacyPtWLabel));
  hist_eff_cent_fwd->SetName(Form("mc_eff_vs_cent_TnP%d_PtW%d_pt_3_to_40_absy1p6_2p4",
                                  isTnPW ? 1 : 0, legacyPtWLabel));
  ...
  TNamed metaPtWeightOverrideDir("meta_ptweight_override_dir", ptWeightOverrideDirUsed.Data());
  ...
  hist_eff2d_den_mid_fine_noptw->Write();
  hist_eff2d_num_mid_fine_noptw->Write();
  hist_eff2d_den_fwd_fine_noptw->Write();
  hist_eff2d_num_fwd_fine_noptw->Write();
  hist_eff_mid->Write();
  hist_eff_fwd->Write();
  ...
  cout << "Finish get_Eff_JPsi_pbpb_hwan_v6()\n";
}
