...
void Run_Eff_Jpsi_pp_hwan_v6(long nEvt = -1, bool isPr = true,
                             bool isTnPW = true, bool isGenW = true,
                             bool isPtW = true, 
                             bool requireRecoMuMatched = true,
                             const char *ptWeightOverrideDir = "",
                             const char *outputTag = "260513") // always true like PbPb
{
  ...
  if (isPtW)
  {
    ...
    const bool useOverride = ptWeightOverrideDirUsed.Length() > 0;

    std::vector<std::string> ptWeightMidPaths;
    std::vector<std::string> ptWeightFwdPaths;
    if (useOverride)
    {
      cout << "[PtW] using pp efficiency pT-weight override directory: " << ptWeightOverrideDirUsed << "\n";
      if (isPr)
      {
        ptWeightMidPaths = {Form("%s/ratioDataMC_pp_Jpsi_DATA_mid.root", ptWeightOverrideDirUsed.Data())};
        ptWeightFwdPaths = {Form("%s/ratioDataMC_pp_Jpsi_DATA_fwd.root", ptWeightOverrideDirUsed.Data())};
      }
      else
      {
        ptWeightMidPaths = {Form("%s/ratioDataMC_pp_BtoJpsi_DATA_mid.root", ptWeightOverrideDirUsed.Data())};
        ptWeightFwdPaths = {Form("%s/ratioDataMC_pp_BtoJpsi_DATA_fwd.root", ptWeightOverrideDirUsed.Data())};
      }
    }
    else if (isPr)
    {
      ptWeightMidPaths = {
          Form("%s/ratioDataMC_pp_Jpsi_DATA_noCtau_y0_1p6_260514_2exp.root", compareDir.Data()),
          Form("%s/ratioDataMC_pp_Jpsi_DATA_noCtau_y0_1p6_260513.root", compareDir.Data())};
      ptWeightFwdPaths = {
          Form("%s/ratioDataMC_pp_Jpsi_DATA_noCtau_y1p6_2p4_260514_2exp.root", compareDir.Data()),
          Form("%s/ratioDataMC_pp_Jpsi_DATA_noCtau_y1p6_2p4_260513.root", compareDir.Data())};
    }
    else
    {
      ptWeightMidPaths = {
          Form("%s/ratioDataMC_pp_BtoJpsi_DATA_noCtau_y0_1p6_260514_2exp.root", compareDir.Data()),
          Form("%s/ratioDataMC_pp_BtoJpsi_DATA_noCtau_y0_1p6_260513.root", compareDir.Data())};
      ptWeightFwdPaths = {
          Form("%s/ratioDataMC_pp_BtoJpsi_DATA_noCtau_y1p6_2p4_260514_2exp.root", compareDir.Data()),
          Form("%s/ratioDataMC_pp_BtoJpsi_DATA_noCtau_y1p6_2p4_260513.root", compareDir.Data())};
    }
  ...
  const TString outputTagStr = (outputTag && TString(outputTag).Length() > 0)
                                   ? TString(outputTag)
                                   : TString("260513");
  // Note: original convention had a trailing "_v6.root" suffix; dropped so
  // the output filename matches the efficiency_1d_pp.C convention expected
  // by the 260519_v2 draw + compare macros.
  std::string outFilePath = Form(
      "%s/mc_eff_vs_pt_rap_%s_pp_Jpsi_PtW%s_tnp%d_%s%s.root",
      outDir.Data(), stateLabel.Data(), ptWeightLabel.Data(), isTnPW ? 1 : 0,
      outputTagStr.Data(), noPtWeightLabel.Data());
  ...
  // Match JpsiaccStudy_v2.C: integrated bins are made by summing the
  // width-scaled pT-bin numerator/denominator contents directly.
  TH1D *hInt_gen_1 = CollapseToSingleBinHist(hist_eff_den_fwd, "hInt_gen_1", ";p_{T} integrated;Counts", 0, 40);
  TH1D *hInt_gen_2 = CollapseToSingleBinHist(hist_eff_den_mid, "hInt_gen_2", ";p_{T} integrated;Counts", 0, 40);
  TH1D *hInt_reco_1 = CollapseToSingleBinHist(hist_eff_num_fwd, "hInt_reco_1", ";p_{T} integrated;Counts", 0, 40);
  TH1D *hInt_reco_2 = CollapseToSingleBinHist(hist_eff_num_mid, "hInt_reco_2", ";p_{T} integrated;Counts", 0, 40);
  TH1D *hInt_eff_1 = BuildWeightedEfficiencyHist(hInt_reco_1, hInt_gen_1, "hInt_eff_1", ";p_{T} integrated;Efficiency");
  TH1D *hInt_eff_2 = BuildWeightedEfficiencyHist(hInt_reco_2, hInt_gen_2, "hInt_eff_2", ";p_{T} integrated;Efficiency");
  ...
  hist_eff_mid->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy0_1p6",
                             isTnPW ? 1 : 0, legacyPtWLabel));
  hist_eff_fwd->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy1p6_2p4",
                             isTnPW ? 1 : 0, legacyPtWLabel));
  hInt_eff_1->SetName(Form("mc_eff_Integrated_TnP%d_PtW%d_absy1p6_2p4",
                           isTnPW ? 1 : 0, legacyPtWLabel));
  hInt_eff_2->SetName(Form("mc_eff_Integrated_TnP%d_PtW%d_absy0_1p6",
                           isTnPW ? 1 : 0, legacyPtWLabel));
                             ...
                             const char *outputTag = "260513")
{
  ...
  if (state != 1 && state != 2)
  {
    cout << "[ERROR] state must be 1 (prompt) or 2 (nonprompt).\n";
    return;
  }
  if (isPtWeight != 0)
    cout << "[WARN] efficiency_1d_pp.C has no pT-weight up/down mode; nominal pT weights are used.\n";

  Run_Eff_Jpsi_pp_hwan_v6(-1, state == 1, isTnP, true, true, true,
                          ptWeightOverrideDir, outputTag);
}
