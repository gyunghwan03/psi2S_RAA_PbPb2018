...
void get_Eff_psi_pbpb_ctauCut_NP_v2(
  ...
  TFile *fPtW1 = new TFile("../compareDataToMC/ratioDataMC_AA_Btopsi2S_DATA_ctauCut_y0_1p6_251103.root","read");
  TFile *fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_Btopsi2S_DATA_ctauCut_y1p6_2p4_251103.root","read");
  ...
  double ptBin_for[5] = {3.5,6.5,9,12,40};
  double ptBin_mid[7] = {6.5,9,12,15,20,25,40};
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
  ...
  //for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      if(! (JP_Reco->Pt()>0&&JP_Reco->Pt()<40&&fabs(JP_Reco->Rapidity())<2.4&&IsAcceptanceQQ(mupl_Reco->Pt(),fabs(mupl_Reco->Eta()))&&IsAcceptanceQQ(mumi_Reco->Pt(),fabs(mumi_Reco->Eta()))) ) continue;
          ...
          if(Rapidity > 1.6 && Rapidity <2.4 && JP_Reco->Pt() > 3.5 && JP_Reco->Pt() < 40){
            for(int i=0; i < 4 ; i++) {
              ...
              if(Centrality > cLow_tmp && Centrality < cHigh_tmp) {
                ...
                if(Reco_QQ_ctau3D[irqq] > l_cut) hcent_reco_1->Fill(Centrality, weight*tnp_weight*pt_weight);
              }
            }
            if(Centrality > 0 && Centrality < 180){
              double l_cut = l_cut_for_int;
              if(Reco_QQ_ctau3D[irqq] > l_cut) hInt_reco_1->Fill(1, weight*tnp_weight*pt_weight);
            }
          }
          if(Rapidity < 1.6 && JP_Reco->Pt() > 6.5 && JP_Reco->Pt() < 40){
            for(int i=0; i < 6 ; i++) {
              ...
              if(Centrality > cLow_tmp && Centrality < cHigh_tmp) {
                ...
                if(Reco_QQ_ctau3D[irqq] > l_cut) hcent_reco_2->Fill(Centrality, weight*tnp_weight*pt_weight);
              }
            }
            if(Centrality > 0 && Centrality < 180){
              double l_cut = l_cut_mid_int;
              if(Reco_QQ_ctau3D[irqq] > l_cut) hInt_reco_2->Fill(1, weight*tnp_weight*pt_weight);
            }
          }
...
//draw same
  ...
  if(state==2) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_psi2S_PtW%d_tnp%d_ctauCut_260127.root",cLow,cHigh,isPtWeight,isTnP);
