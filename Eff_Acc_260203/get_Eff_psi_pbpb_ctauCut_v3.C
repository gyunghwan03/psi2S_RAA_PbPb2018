...
static double EvalPtWeight(bool isPtWeight, double rap, double pt, TF1* fptw1, TF1* fptw2)
{
  if(!isPtWeight) return 1.0;
  double absRap = fabs(rap);
  if(absRap < 1.6) return fptw1->Eval(pt);
  if(absRap < 2.4) return fptw2->Eval(pt);
  return 1.0;
}
...
void get_Eff_psi_pbpb_ctauCut_v3_prompt(
  int state=1,
...
void get_Eff_psi_pbpb_ctauCut_v3_nonprompt(
  float ptLow = 3.0, float ptHigh = 40.0,
  float yLow = 0.0, float yHigh = 2.4,
  float cLow = 0, float cHigh = 180,
  bool isTnP = true, bool isPtWeight = true,
...
void get_Eff_psi_pbpb_ctauCut_v3(
  int state=1,
  float ptLow = 3.0, float ptHigh = 40.0,
  float yLow = 0.0, float yHigh = 2.4,
  float cLow = 0, float cHigh = 180,
  ...
  if(state==1){
    get_Eff_psi_pbpb_ctauCut_v3_prompt(state, ptLow, ptHigh, yLow, yHigh, cLow, cHigh, isTnP, isPtWeight);
  } else {
    get_Eff_psi_pbpb_ctauCut_v3_nonprompt(ptLow, ptHigh, yLow, yHigh, cLow, cHigh, isTnP, isPtWeight, state);
  }
}
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
  ...
  //if(Gen_QQ_size > 0) cout<<"weight : "<<weight<<", Gen_QQ_size : "<<Gen_QQ_size<<", Reco_QQ_size : "<<Reco_QQ_size<<endl;
    ...
    //cout<<"Rapidity_g : "<<Rapidity_g<<", pt : "<<JP_Gen->Pt()<<endl;
      ...
      pt_weight = EvalPtWeight(isPtWeight, JP_Gen->Rapidity(), JP_Gen->Pt(), fptw1, fptw2);
      ...
      if(state!=1 || (JP_Gen->Pt() > 3.5 && JP_Gen->Pt() < 40)){
    ...
    //cout<<"Reco QQ pt : "<<JP_Reco->Pt()<<endl;
      ...
      pt_weight = EvalPtWeight(isPtWeight, JP_Reco->Rapidity(), JP_Reco->Pt(), fptw1, fptw2);
...
//draw same
  ...
  TString outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_psi2S_PtW%d_tnp%d_ctauCut_260223.root",cLow,cHigh,isPtWeight,isTnP);
  if(state==2) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_psi2S_PtW%d_tnp%d_ctauCut_260223.root",cLow,cHigh,isPtWeight,isTnP);
