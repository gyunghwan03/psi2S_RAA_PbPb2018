...
void get_Eff_JPsi_pbpb_ctauCut_v6(
    int state = 2,
...
)
{
      ...
      if (HLTPass == true && HLTFilterPass == true)
      {
          ...
          if (Rapidity < 1.6)
          {
            ...
            for(int i=1; i < 7 ; i++) {
              ...
              if(pt > ptLow && pt < ptHigh) {
                ...
                if(state==1) { 
                  if(Reco_QQ_ctau3D[irqq] < l_cut) { hpt_reco_2->Fill(pt, weight * tnp_weight * pt_weight); hpt_reco_2_noW->Fill(pt, 1.0); }
        ...
        if (Rapidity > 1.6 && Rapidity < 2.4)
        {
          ...
          if (JP_Reco->Pt() > 3.5 && JP_Reco->Pt() < 40)
          {
            ...
            if(Centrality > 0 && Centrality < 180)
            {
              ...
              if(state==1) { if(Reco_QQ_ctau3D[irqq] < l_cut) { hInt_reco_1->Fill(1, weight * tnp_weight * pt_weight); hInt_reco_1_noW->Fill(1, 1.0); } }
              else if(state==2) { if(Reco_QQ_ctau3D[irqq] > l_cut) { hInt_reco_1->Fill(1, weight * tnp_weight * pt_weight); hInt_reco_1_noW->Fill(1, 1.0); } }
            }
          }
        }
        if (Rapidity < 1.6 && JP_Reco->Pt() > 6.5 && JP_Reco->Pt() < 40)
        {
          ...
          if(Centrality > 0 && Centrality < 180)
          {
            ...
            if(state==1) { if(Reco_QQ_ctau3D[irqq] < l_cut) { hInt_reco_2->Fill(1, weight * tnp_weight * pt_weight); hInt_reco_2_noW->Fill(1, 1.0); } }
            else if(state==2) { if(Reco_QQ_ctau3D[irqq] > l_cut) { hInt_reco_2->Fill(1, weight * tnp_weight * pt_weight); hInt_reco_2_noW->Fill(1, 1.0); } }
          }
        }
      }
        if (Rapidity < 2.4 && JP_Reco->Pt() > 6.5 && JP_Reco->Pt() < 50)
  ...
  hpt_eff_0_noW->Divide(hpt_eff_0_noW, hpt_gen_0_noW, 1, 1, "B");
  ...
  */
  ...
  TString outFileName;
  if (state == 1 && applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_JPsi_PtW%s_tnp%d_ctauCut_260310_2exp.root", cLow, cHigh, ptSys.Data(), isTnP);
  else if (state == 1 && !applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_JPsi_PtW%d_tnp%d_ctauCut_260310_noPtW.root", cLow, cHigh, 1, isTnP);
  else if (state == 2 && applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_JPsi_PtW%s_tnp%d_ctauCut_260310_2exp.root", cLow, cHigh, ptSys.Data(), isTnP);
  else if (state == 2 && !applyPtWeight) outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_JPsi_PtW%d_tnp%d_ctauCut_260310_noPtW.root", cLow, cHigh, 1, isTnP);
