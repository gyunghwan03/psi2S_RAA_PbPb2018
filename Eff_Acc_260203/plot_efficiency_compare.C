...
const TString kPlotOutputDir = "plot_outputs_eff_compare";
...
void draw_pp_with_single_integrated_rightpad(const TString& ptHistName,
  ...
  gStyle->SetOptStat(0);
  EnsureOutputDir();
  TCanvas* c = new TCanvas(Form("c_%s", outName.Data()), "", 1100, 520);
  TPad* padL = new TPad(Form("padL_%s", outName.Data()), "", 0.00, 0.00, 0.76, 1.00);
  TPad* padR = new TPad(Form("padR_%s", outName.Data()), "", 0.76, 0.00, 1.00, 1.00);
  padL->SetTicks(1, 1);
  padR->SetTicks(1, 1);
  padL->SetRightMargin(0.0);
  ...
  hFrame->SetTitle("");
  hFrame->GetYaxis()->SetTitle("Integrated Efficiency");
  ...
  TLegend* legR = new TLegend(0.12, 0, 0.94, 0.90);
  legR->SetBorderSize(0);
  legR->SetFillStyle(0);
  legR->SetTextSize(0.040);
  legR->AddEntry(hIntA, "J/#psi (260219)", "lep");
  legR->AddEntry(hIntB, "J/#psi (260310)", "lep");
  legR->Draw();
