...
void get_Eff_Jpsi_pp_hwan_0313(
  ...
  TString inputMC1;
  if(state==1) inputMC1 = "/disk1/Oniatree/Jpsi/OniaTreeMC_Jpsi_Pythia8_nonUL_5p02TeV_merged.root"; // pp prompt
  else if (state==2) inputMC1 = "/disk1/Oniatree/Jpsi/BtoJpsiJpsi_miniAOD94X_20Jun_v1.root";
  TChain* mytree;
  ...
  //jpsi
  //TFile *fPtW1 = new TFile("../compareDataToMC/ratioDataMC_pp_JPsi_DATA_y1p6_2p4_250101.root", "read"); //vv3
  //before 250121
  TFile *fPtW1 = new TFile("../compareDataToMC/ratioDataMC_pp_JPsi_DATA_y1p6_2p4_241016.root", "read"); //v4
  TFile *fPtW2 = new TFile("../compareDataToMC/ratioDataMC_pp_JPsi_DATA_y0_1p6_241008.root", "read");

  TFile *fPtW3 = new TFile("./ratioDataMC_pp_250219_y1p6_2p4.root", "read");
  TFile *fPtW4 = new TFile("./ratioDataMC_pp_250219_y0_1p6.root", "read");
  //affter 250121
  //TFile *fPtW1 = new TFile("../compareDataToMC/ratioDataMC_pp_JPsi_DATA_y1p6_2p4_250121.root", "read"); //v4
  //TFile *fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_Jpsi_DATA_All_y_211218.root", "read");
  ...
  TH1D* hInt_gen_0 = new TH1D("hInt_gen_0", "",1,0,50);
  TH1D* hInt_gen_1 = new TH1D("hInt_gen_1", "",1,0,40);
  TH1D* hInt_gen_2 = new TH1D("hInt_gen_2", "",1,0,40);

  TH1D* hInt_reco_0 = new TH1D("hInt_reco_0", "",1,0,50);
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
  ...
  // for(int iev=0; iev<300000 ; ++iev)
  {
    ...
    for (int igen = 0; igen < Gen_QQ_size; igen++)
    {
      ...
      if (!(JP_Gen->Pt() > 0 && JP_Gen->Pt() < 50 && fabs(JP_Gen->Rapidity()) < 2.4 && IsAcceptanceQQ(mupl_Gen->Pt(), fabs(mupl_Gen->Eta())) && IsAcceptanceQQ(mumi_Gen->Pt(), fabs(mumi_Gen->Eta()))))
        continue;
      ...
      //if(pt_weight != pt_weight_c && iev%100 == 0) cout << "pt_weight : " << pt_weight << "  / "<< " pt_weight_c : " << pt_weight_c << "  / " << "  PT  " << JP_Gen->Pt() << " n " << iev << endl;
      //cout << ">>>>> Event " << iev << " / " << nevt << " (" << (double)(pt_weight) << ")" << endl;
      if(irms==0){
      ...
      if (Rapidity_g < 2.4 && Rapidity_g > 1.6 && JP_Gen->Pt() > 3. && JP_Gen->Pt() < 6.5)
      {
      ...
      {
        hpt_gen_0->Fill(JP_Gen->Pt(), weight * pt_weight);
        hInt_gen_0->Fill(1, weight * pt_weight);
        //hpt_gen_0->Fill(JP_Gen->Pt(), weight * pt_weight);
      }
      if (Rapidity_g > 1.6 && Rapidity_g < 2.4 && JP_Gen->Pt() > 3.5 && JP_Gen->Pt() < 50)
      {
        hpt_gen_1->Fill(JP_Gen->Pt(), weight * pt_weight_c);
        hInt_gen_1->Fill(1, weight * pt_weight_c);
      }
      if (Rapidity_g < 1.6 && JP_Gen->Pt() > 6.5 && JP_Gen->Pt() < 50)
      {
        hpt_gen_2->Fill(JP_Gen->Pt(), weight * pt_weight_c);
        hInt_gen_2->Fill(1, weight * pt_weight_c);
      }
      }
    ...
    for (Int_t irqq = 0; irqq < Reco_QQ_size; ++irqq)
    {
      ...
      if (!(JP_Reco->Pt()>0.&& JP_Reco->Pt() < 50 && fabs(JP_Reco->Rapidity()) < 2.4 && IsAcceptanceQQ(mupl_Reco->Pt(), fabs(mupl_Reco->Eta())) && IsAcceptanceQQ(mumi_Reco->Pt(), fabs(mumi_Reco->Eta()))))
        continue;
	  counts[2]++;
      ...
      if (HLTPass == true && HLTFilterPass == true)
      {
        ...
        {
          hpt_reco_0->Fill(JP_Reco->Pt(), weight * tnp_weight * pt_weight);
          hInt_reco_0->Fill(1, weight * tnp_weight * pt_weight);
	        //counts[4]++;
          //hpt_reco_0->Fill(JP_Reco->Pt(), weight * tnp_weight * pt_weight);
        }
        if (Rapidity > 1.6 && Rapidity < 2.4 && JP_Reco->Pt() > 3.5 && JP_Reco->Pt() < 50)
        {
          hpt_reco_1->Fill(JP_Reco->Pt(), weight * tnp_weight * pt_weight_c);
          hInt_reco_1->Fill(1, weight * tnp_weight * pt_weight_c);
        }
        if (Rapidity < 1.6 && JP_Reco->Pt() > 6.5 && JP_Reco->Pt() < 50)
        {
          hpt_reco_2->Fill(JP_Reco->Pt(), weight * tnp_weight * pt_weight_c);
          hInt_reco_2->Fill(1, weight * tnp_weight * pt_weight_c);
        }
  ...
  hInt_eff_0 = (TH1D*)hInt_reco_0->Clone("hInt_eff_0");
  hInt_eff_1 = (TH1D*)hInt_reco_1->Clone("hInt_eff_1");
  hInt_eff_2 = (TH1D*)hInt_reco_2->Clone("hInt_eff_2");

  hInt_eff_0->Divide(hInt_eff_0, hInt_gen_0, 1, 1, "B");
...
//draw same
  ...
  hpt_eff_0 ->SetName(Form("mc_eff_vs_pt_TnP%d_PtW%d_absy0_2p4",isTnP, isPtWeight));
  ...
  hInt_eff_0 ->SetName(Form("mc_eff_Integrated_TnP%d_PtW%d_absy0_2p4",isTnP, isPtWeight));
  ...
  //if (state == 2)
  TString  outFileName;
  if(state==1) outFileName = Form("./roots/mc_eff_vs_pt_rap_prompt_pp_Jpsi_PtW%d_tnp%d_20250313.root", isPtWeight, isTnP);
  else if(state==2) outFileName = Form("./roots/mc_eff_vs_pt_rap_nprompt_pp_Jpsi_PtW%d_tnp%d_20250313.root", isPtWeight, isTnP);
