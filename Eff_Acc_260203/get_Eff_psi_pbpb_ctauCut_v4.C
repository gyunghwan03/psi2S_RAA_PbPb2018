...
static double EvalPtWeight(bool isPtWeight, double rap, double pt, TF1* fptw)
{
  if(!isPtWeight) return 1.0;
  double absRap = fabs(rap);
  if(absRap < 2.4) return fptw->Eval(pt);
  return 1.0;
}
...
void get_Eff_psi_pbpb_ctauCut_v4_prompt(
  int state=1,
...
void get_Eff_psi_pbpb_ctauCut_v4_nonprompt(
  float ptLow = 3.0, float ptHigh = 40.0,
  float yLow = 0.0, float yHigh = 2.4,
  float cLow = 0, float cHigh = 180,
  bool isTnP = true, bool isPtWeight = true,
...
void get_Eff_psi_pbpb_ctauCut_v4(
  int state=1,
  float ptLow = 3.0, float ptHigh = 40.0,
  float yLow = 0.0, float yHigh = 2.4,
  float cLow = 0, float cHigh = 180,
  ...
  if(state==1){
    get_Eff_psi_pbpb_ctauCut_v4_prompt(state, ptLow, ptHigh, yLow, yHigh, cLow, cHigh, isTnP, isPtWeight);
  } else {
    get_Eff_psi_pbpb_ctauCut_v4_nonprompt(ptLow, ptHigh, yLow, yHigh, cLow, cHigh, isTnP, isPtWeight, state);
  }
}
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
  ...
  //if(Gen_QQ_size > 0) cout<<"weight : "<<weight<<", Gen_QQ_size : "<<Gen_QQ_size<<", Reco_QQ_size : "<<Reco_QQ_size<<endl;
    ...
    //cout<<"Rapidity_g : "<<Rapidity_g<<", pt : "<<JP_Gen->Pt()<<endl;
      ...
      pt_weight = EvalPtWeight(isPtWeight, JP_Gen->Rapidity(), JP_Gen->Pt(), fptw);
    ...
    //cout<<"Reco QQ pt : "<<JP_Reco->Pt()<<endl;
      ...
      pt_weight = EvalPtWeight(isPtWeight, JP_Reco->Rapidity(), JP_Reco->Pt(), fptw);
