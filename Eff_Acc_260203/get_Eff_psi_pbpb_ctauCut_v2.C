...
void get_Eff_psi_pbpb_ctauCut_v2(
  int state=1,
  ...
  //input files
  //PbPb
  TString inputMC1 = "/data/Oniatree/miniAOD/OniaTree_Run2_PbPb2018MC_Prompt_forPsi2S_miniAOD112X_muonSelGlb_13May23v2.root";
  if(state==2) inputMC1 = "/data/Oniatree/miniAOD/OniaTreeMC_miniAOD_psi2S_BtoPsi2S_pThat2_HydjetMB_Glb_5p02TeV_merged.root";	//PbPb_non prompt
...
//  TH2D* hpt_tnp_trig = new TH2D("hpt_tnp_trig","hpt_tnp_trig",numBins,ptBin,40,0,2);
  ...
  {
    if(iev%10000000==0) cout << ">>>>> EVENT " << iev << " / " << mytree->GetEntries() <<  " ("<<(int)(100.*iev/mytree->GetEntries()) << "%)" << endl;
    ...
    for(int igen = 0; igen<Gen_QQ_size; igen++){
		...
		if(Rapidity_g > 1.6 && Rapidity_g <2.4){
			...
			if(JP_Gen->Pt() > 3.5 && JP_Gen->Pt() < 40) { hInt_gen_1->Fill(1,weight*pt_weight); hcent_gen_1->Fill( Centrality,weight*pt_weight); }
		}
    ...
    for (Short_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      if(! (JP_Reco->Pt()>3.5&&JP_Reco->Pt()<40&&fabs(JP_Reco->Rapidity())<2.4&&IsAcceptanceQQ(mupl_Reco->Pt(),fabs(mupl_Reco->Eta()))&&IsAcceptanceQQ(mumi_Reco->Pt(),fabs(mumi_Reco->Eta()))) ) continue;
      ...
      if(isTnP){
        ...
        tnp_weight = tnp_weight * tnp_weight_muid_pbpb(mupl_Reco->Pt(), mupl_Reco->Eta(), tnp_ids) * tnp_weight_muid_pbpb(mumi_Reco->Pt(), mumi_Reco->Eta(), tnp_ids); //mu id
        tnp_weight = tnp_weight * tnp_weight_trk_pbpb(mupl_Reco->Eta(), tnp_trks) * tnp_weight_trk_pbpb(mumi_Reco->Eta(), tnp_trks); //inner tracker
...
//draw same
  ...
  TString outFileName = Form("./roots/mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_prompt_pbpb_psi2s_PtW%d_tnp%d_ctauCut_260127.root",cLow,cHigh,isPtWeight,isTnP);
  if(state==2) outFileName = Form("mc_eff_vs_pt_cent_%0.0f_to_%0.0f_rap_nprompt_pbpb_psi2S_PtW%d_tnp%d_ctauCut_260127.root",cLow,cHigh,isPtWeight,isTnP);
