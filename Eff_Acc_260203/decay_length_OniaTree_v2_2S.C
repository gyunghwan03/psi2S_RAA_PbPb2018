...
// v4 : change TnP part to makeMuonSkimTree_psi2S.C

void decay_length_OniaTree_v2_2S(
		int state = 2,
		...
		)
{
	...
	TString kineLabel = Form("state%d_pt%.1f-%.1f_y%.1f-%.1f_cent%.0f-%.0f_m%.1f-%.1f", 
			state, ptLow, ptHigh, yLow, yHigh, cLow, cHigh, massLow, massHigh);
	...
	//TString fNPMC_path = "/data/Oniatree/miniAOD/OniaTreeMC_miniAOD_psi2S_BtoPsi2S_pThat2_HydjetMB_Glb_5p02TeV_merged.root"; // PbPb_non prompt
	TString fPRMC_path = "/data/Oniatree/miniAOD/OniaTree_Run2_PbPb2018MC_Prompt_forPsi2S_miniAOD112X_muonSelGlb_13May23v2.root";
	...
	// TString outFileName = "mc_eff_vs_pt_cent_prompt_pbpb_psi2S.root";
	// if(state==2) outFileName = "mc_eff_vs_pt_cent_nprompt_pbpb_psi2S.root";
	...
	// ratioDataMC_AA_psi2S_DATA_y0.0-2.4_210915.root
	// TFile *fPtW1 = new TFile("./roots/ratioDataMC_AA_psi2S_DATA_All_y_211218.root","read");
	// TFile *fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_psi2S_DATA_y1p6_2p4_250116.root","read"); //v2
	// before
	// TFile *fPtW1 = new TFile("./roots/ratioDataMC_AA_psi2S_DATA_y0_1p6_211201.root", "read");
	// TFile *fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_psi2S_DATA_y1p6_2p4_241126.root", "read"); // v4

	// affter 250121
	// TFile *fPtW1 = new TFile("../compareDataToMC/ratioDataMC_AA_psi2S_DATA_All_y_211218.root", "read");
	// TFile *fPtW1 = new TFile("../compareDataToMC/ratioDataMC_AA_psi2S_DATA_y0_1p6_211201.root", "read");
	// TFile *fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_psi2S_DATA_y1p6_2p4_241126.root", "read"); // v4
	// psi2S v2 weighting function
	//TFile *fPtW1 = new TFile("./ratioDataMC_AA_psi2S_DATA_All_y_211218.root", "read");
	//TFile *fPtW2 = new TFile("./ratioDataMC_AA_psi2S_DATA_Forward_y_211218.root", "read");
	...
	for (int iev = 0; iev < treePRMC->GetEntries(); ++iev) {
		...
		for (Int_t irqq = 0; irqq < Reco_QQ_size_pr; irqq++) {
			...
			if (Reco_QQ_VtxProb_pr[irqq] < 0.01) continue;
	...
	for (int iev = 0; iev < treeNPMC->GetEntries(); ++iev) {
		...
		for (Int_t irqq = 0; irqq < Reco_QQ_size_np; irqq++) {
			...
			if (Reco_QQ_VtxProb_np[irqq] < 0.01) continue;
	...
	for (int bins = 0; bins < h_decayPRMC->GetNbinsX(); bins++) {
		if (state == 1) { // Prompt selection: find cut where PR efficiency ~90%
			...
			lcuth = new TLine(xmin, h_decayPRMC->Integral(0, bins) / totalPRMC,	h_decayPRMC->GetBinCenter(bins), h_decayPRMC->Integral(0, bins) / totalPRMC);
			lresi = new TLine(xmin, h_decayNPMC->Integral(0, bins) / totalNPMC, h_decayNPMC->GetBinCenter(bins), 1 - (h_decayNPMC->Integral(0, bins) / totalNPMC));
		}
			...
			lcutv = new TLine(h_decayPRMC->GetBinCenter(bins), 0, h_decayPRMC->GetBinCenter(bins), 1);
			lcuth = new TLine(xmin, h_decayNPMC->Integral(0, bins) / totalNPMC, h_decayNPMC->GetBinCenter(bins), h_decayNPMC->Integral(0, bins) / totalNPMC);
			lresi = new TLine(xmin, h_decayPRMC->Integral(0, bins) / totalPRMC, h_decayPRMC->GetBinCenter(bins), h_decayPRMC->Integral(0, bins) / totalPRMC);
		}
	}
	...
	if (lcutv) {
		...
		if (state == 1) {
			...
			cout << "Prompt #psi(2S) efficiency: " << pr_eff * 100 << "%" << endl;
			cout << "Non-prompt #psi(2S) residual: " << np_res * 100 << "%" << endl;
			...
			cout << "Non-prompt #psi(2S) efficiency: " << np_res * 100 << "%" << endl;
			cout << "Prompt #psi(2S) residual: " << pr_eff * 100 << "%" << endl;
			cout << "Apply cut: ctau3D >= " << ctau3D_cut << endl;
			cout << "============================================" << endl;
		}
	} else {
	...
	if (lcutv) {
		if (state == 1) {
			...
			drawText(Form("PR #psi(2S) eff: %.3f", pr_eff), pos_x_mass, pos_y - pos_y_diff * 3.5, kBlue, text_size);
			drawText(Form("NP #psi(2S) res: %.3f", np_res), pos_x_mass, pos_y - pos_y_diff * 4, kRed + 2, text_size);
		} else if (state == 2) {
			drawText(Form("ctau3D cut: %.4f mm", ctau3D_cut), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
			drawText(Form("NP #psi(2S) eff: %.3f", np_res), pos_x_mass, pos_y - pos_y_diff * 3.5, kRed + 2, text_size);
			drawText(Form("PR #psi(2S) res: %.3f", pr_eff), pos_x_mass, pos_y - pos_y_diff * 4, kBlue, text_size);
		}
	}
	...
	leg->SetFillStyle(0);
	if (state == 1) {
		leg->AddEntry(h_deffPRMC, "Prompt #psi(2S) (CDF)", "l");
		leg->AddEntry(h_deffNPMC, "Non-Prompt #psi(2S) (1-CDF)", "l");
	} else {
		leg->AddEntry(h_deffNPMC, "Non-Prompt #psi(2S) (CDF)", "l");
		leg->AddEntry(h_deffPRMC, "Prompt #psi(2S) (1-CDF)", "l");
	}
	leg->Draw();
	...
	leg2->SetFillStyle(0);
	leg2->AddEntry(h_decayPRMC, "Prompt #psi(2S) MC", "l");
	leg2->AddEntry(h_decayNPMC, "Non-Prompt #psi(2S) MC", "l");
	leg2->Draw();
	...
	if(type == 1 || type == 3)
	{
		...
		int npmc_pass_count = 0;
		int npmc_total_qq = 0;
		for (int iev = 0; iev < treeNPMC->GetEntries(); ++iev) {
			if (iev % 1000000 == 0) cout << "NPMC pt-binned: " << iev << " / " << treeNPMC->GetEntries() << ", passed: " << npmc_pass_count << endl;
			treeNPMC->GetEntry(iev);
			bool HLTPass_np = ((HLTriggers_np & ((ULong64_t)pow(2, kTrigSel))) == ((ULong64_t)pow(2, kTrigSel)));
		...
		cout << "NPMC Summary: Total QQ pairs = " << npmc_total_qq << ", Passed all cuts = " << npmc_pass_count << endl;
		...
		if(type == 1 || type == 3)
		{
			...
			for (int ipt = 0; ipt < nPtBins_for; ipt++) {
				...
				TH1D* h_eff_np_res_pt_for = (TH1D*)h_decay_np_pt_for[ipt]->Clone(Form("h_eff_np_res_pt_for_%d", ipt));

				// Calculate CDF for NPMC (res), 1-CDF for PRMC (eff) and NPMC (eff)
				for (int ib = 1; ib <= h_eff_pr_pt_for->GetNbinsX(); ib++) {
					h_eff_pr_pt_for->SetBinContent(ib, 1.0 - h_decay_pr_pt_for[ipt]->Integral(0, ib) / totalPR_pt);  // Prompt Efficiency (1-CDF)
					h_eff_np_pt_for->SetBinContent(ib, 1.0 - h_decay_np_pt_for[ipt]->Integral(0, ib) / totalNP_pt);  // Nonprompt Efficiency (1-CDF)
					h_eff_np_res_pt_for->SetBinContent(ib, h_decay_np_pt_for[ipt]->Integral(0, ib) / totalNP_pt);  // Nonprompt res (CDF)
				}

				TCanvas* c_eff_pt_for = new TCanvas(Form("c_eff_pt_for_%d", ipt), "", 800, 600);
				c_eff_pt_for->SetGrid();

				h_eff_np_res_pt_for->SetLineColor(kRed);
				h_eff_np_res_pt_for->SetLineWidth(2);
				h_eff_np_res_pt_for->GetXaxis()->SetTitle("ctau3D (mm)");
				h_eff_np_res_pt_for->GetYaxis()->SetTitle("Efficiency");
				h_eff_np_res_pt_for->GetYaxis()->SetRangeUser(0, 1.1);
				h_eff_np_res_pt_for->GetXaxis()->SetRangeUser(-1, 3);
				h_eff_np_res_pt_for->Draw("hist");

				h_eff_pr_pt_for->SetLineColor(kBlue);
				h_eff_pr_pt_for->SetLineWidth(2);
				h_eff_pr_pt_for->Draw("hist same");

				h_eff_np_pt_for->SetLineColor(kGreen+2);
				h_eff_np_pt_for->SetLineWidth(2);
				h_eff_np_pt_for->Draw("hist same");
				...
				TLegend* leg_pt_for = new TLegend(0.15, 0.70, 0.50, 0.88);
				leg_pt_for->SetBorderSize(0);
				leg_pt_for->SetFillStyle(0);
				leg_pt_for->AddEntry(h_eff_pr_pt_for, "Prompt Efficiency", "l");
				leg_pt_for->AddEntry(h_eff_np_res_pt_for, "Nonprompt res", "l");
				leg_pt_for->AddEntry(h_eff_np_pt_for, "Nonprompt Efficiency", "l");
				leg_pt_for->Draw();
				...
				if (state == 1) {
					drawText(Form("PR eff: %.3f", pr_eff_ptBin_for[ipt]), 0.50, 0.60, 4, 16);
					drawText(Form("NP res: %.3f", np_res_ptBin_for[ipt]), 0.50, 0.55, 2, 16);
					drawText(Form("NP eff: %.3f", 1-np_res_ptBin_for[ipt]), 0.50, 0.50, 8, 16);
				} else {
					drawText(Form("PR eff: %.3f", 1-pr_eff_ptBin_for[ipt]), 0.50, 0.60, 4, 16);
					drawText(Form("NP res: %.3f", 1-np_res_ptBin_for[ipt]), 0.50, 0.55, 2, 16);
					drawText(Form("NP eff: %.3f", np_res_ptBin_for[ipt]), 0.50, 0.50, 8, 16);
				}

				TString stateStr = (state == 1) ? "PRMC" : "NPMC";
			c_eff_pt_for->SaveAs(Form("./figs_ctau3D_v2/eff_ptBin_for_%s_pt%.1f-%.1f_m%.1f-%.1f.pdf", stateStr.Data(), ptBin_for[ipt], ptBin_for[ipt+1], massLow, massHigh));
			c_eff_pt_for->SaveAs(Form("./figs_ctau3D_v2/eff_ptBin_for_%s_pt%.1f-%.1f_m%.1f-%.1f.png", stateStr.Data(), ptBin_for[ipt], ptBin_for[ipt+1], massLow, massHigh));
				delete h_eff_pr_pt_for;
				delete h_eff_np_pt_for;
				delete h_eff_np_res_pt_for;
				delete c_eff_pt_for;
				delete line_pt_for;
				delete leg_pt_for;
				} else {
			...
			for (int ipt = 0; ipt < nPtBins_mid; ipt++) {
				...
				TH1D* h_eff_np_res_pt_mid = (TH1D*)h_decay_np_pt_mid[ipt]->Clone(Form("h_eff_np_res_pt_mid_%d", ipt));

				for (int ib = 1; ib <= h_eff_pr_pt_mid->GetNbinsX(); ib++) {
					h_eff_pr_pt_mid->SetBinContent(ib, 1.0 - h_decay_pr_pt_mid[ipt]->Integral(0, ib) / totalPR_pt);  // Prompt Efficiency (1-CDF)
					h_eff_np_pt_mid->SetBinContent(ib, 1.0 - h_decay_np_pt_mid[ipt]->Integral(0, ib) / totalNP_pt);  // Nonprompt Efficiency (1-CDF)
					h_eff_np_res_pt_mid->SetBinContent(ib, h_decay_np_pt_mid[ipt]->Integral(0, ib) / totalNP_pt);  // Nonprompt res (CDF)
				}

				TCanvas* c_eff_pt_mid = new TCanvas(Form("c_eff_pt_mid_%d", ipt), "", 800, 600);
				c_eff_pt_mid->SetGrid();

				h_eff_np_res_pt_mid->SetLineColor(kRed);
				h_eff_np_res_pt_mid->SetLineWidth(2);
				h_eff_np_res_pt_mid->GetXaxis()->SetTitle("ctau3D (mm)");
				h_eff_np_res_pt_mid->GetYaxis()->SetTitle("Efficiency");
				h_eff_np_res_pt_mid->GetYaxis()->SetRangeUser(0, 1.1);
				h_eff_np_res_pt_mid->GetXaxis()->SetRangeUser(-1, 3);
				h_eff_np_res_pt_mid->Draw("hist");

				h_eff_pr_pt_mid->SetLineColor(kBlue);
				h_eff_pr_pt_mid->SetLineWidth(2);
				h_eff_pr_pt_mid->Draw("hist same");

				h_eff_np_pt_mid->SetLineColor(kGreen+2);
				h_eff_np_pt_mid->SetLineWidth(2);
				h_eff_np_pt_mid->Draw("hist same");
				...
				TLegend* leg_pt_mid = new TLegend(0.15, 0.70, 0.50, 0.88);
				leg_pt_mid->SetBorderSize(0);
				leg_pt_mid->SetFillStyle(0);
				leg_pt_mid->AddEntry(h_eff_pr_pt_mid, "Prompt Efficiency", "l");
				leg_pt_mid->AddEntry(h_eff_np_res_pt_mid, "Nonprompt res", "l");
				leg_pt_mid->AddEntry(h_eff_np_pt_mid, "Nonprompt Efficiency", "l");
				leg_pt_mid->Draw();
				...
				if (state == 1) {
					drawText(Form("PR eff: %.3f", pr_eff_ptBin_mid[ipt]), 0.50, 0.60, 4, 16);
					drawText(Form("NP res: %.3f", np_res_ptBin_mid[ipt]), 0.50, 0.55, 2, 16);
					drawText(Form("NP eff: %.3f", 1-np_res_ptBin_mid[ipt]), 0.50, 0.50, 8, 16);
				} else {
					drawText(Form("PR eff: %.3f", 1-pr_eff_ptBin_mid[ipt]), 0.50, 0.60, 4, 16);
					drawText(Form("NP res: %.3f", 1-np_res_ptBin_mid[ipt]), 0.50, 0.55, 2, 16);
					drawText(Form("NP eff: %.3f", np_res_ptBin_mid[ipt]), 0.50, 0.50, 8, 16);
				}

				TString stateStr = (state == 1) ? "PRMC" : "NPMC";
			c_eff_pt_mid->SaveAs(Form("./figs_ctau3D_v2/eff_ptBin_mid_%s_pt%.1f-%.1f_m%.1f-%.1f.pdf", stateStr.Data(), ptBin_mid[ipt], ptBin_mid[ipt+1], massLow, massHigh));
			c_eff_pt_mid->SaveAs(Form("./figs_ctau3D_v2/eff_ptBin_mid_%s_pt%.1f-%.1f_m%.1f-%.1f.png", stateStr.Data(), ptBin_mid[ipt], ptBin_mid[ipt+1], massLow, massHigh));
				delete h_eff_pr_pt_mid;
				delete h_eff_np_pt_mid;
				delete h_eff_np_res_pt_mid;
				delete c_eff_pt_mid;
					delete line_pt_mid;
					delete leg_pt_mid;
				} else {
		...
		if(type == 2 || type == 3) {
				...
				for (Int_t irqq = 0; irqq < Reco_QQ_size_pr; irqq++) {
					...
					if (rapidity >= 1.6 && rapidity < 2.4 && JP_pr->Pt() >= 3.5 && JP_pr->Pt() < 40) {
						...
						if (Centrality_pr >= 0 && Centrality_pr < 180) {
							h_decay_pr_int_for->Fill(ctau3D_val);
						}
						// Fill cent bins
						for (int icent = 0; icent < nCentBins_for; icent++) {
							if (Centrality_pr >= centBin_for[icent] && Centrality_pr < centBin_for[icent+1]) {
								h_decay_pr_cent_for[icent]->Fill(ctau3D_val);
							}
						}
					}
					...
					if (rapidity < 1.6 && JP_pr->Pt() >= 6.5 && JP_pr->Pt() < 40) {
						// Fill integrated histogram (cent 0-180)
						if (Centrality_pr >= 0 && Centrality_pr < 180) {
							h_decay_pr_int_mid->Fill(ctau3D_val);
						}
						// Fill cent bins
						for (int icent = 0; icent < nCentBins_mid; icent++) {
							if (Centrality_pr >= centBin_mid[icent] && Centrality_pr < centBin_mid[icent+1]) {
								h_decay_pr_cent_mid[icent]->Fill(ctau3D_val);
							}
						}
					}
				}
				...
				for (Int_t irqq = 0; irqq < Reco_QQ_size_np; irqq++) {
					...
					if (rapidity >= 1.6 && rapidity < 2.4 && JP_np->Pt() >= 3.5 && JP_np->Pt() < 40) {
						...
						if (Centrality_np >= 0 && Centrality_np < 180) {
							h_decay_np_int_for->Fill(ctau3D_val);
						}
						// Fill cent bins
						for (int icent = 0; icent < nCentBins_for; icent++) {
							if (Centrality_np >= centBin_for[icent] && Centrality_np < centBin_for[icent+1]) {
								h_decay_np_cent_for[icent]->Fill(ctau3D_val);
							}
						}
					}
					// Mid rapidity (0-1.6): check pt and fill cent bins + integrated
					if (rapidity < 1.6 && JP_np->Pt() >= 6.5 && JP_np->Pt() < 40) {
						...
						for (int icent = 0; icent < nCentBins_mid; icent++) {
							if (Centrality_np >= centBin_mid[icent] && Centrality_np < centBin_mid[icent+1]) {
								h_decay_np_cent_mid[icent]->Fill(ctau3D_val);
							}
						}
					}
				}
			...
			if (totalPR_int_for > 0 && totalNP_int_for > 0) {
				...
				leg_int_for->AddEntry(h_eff_np_int_for, "Non-Prompt #psi(2S) (CDF)", "l");
				leg_int_for->AddEntry(h_eff_pr_int_for, "Prompt #psi(2S) (1-CDF)", "l");
				leg_int_for->Draw();
				...
				if (state == 1) {
					drawText(Form("NP #psi(2S) eff: %.3f", np_res_int_for), 0.50, 0.60, 4, 16);
					drawText(Form("PR #psi(2S) res: %.3f", 1-pr_eff_int_for), 0.50, 0.55, 2, 16);
				} else {
					drawText(Form("NP #psi(2S) eff: %.3f", 1-np_res_int_for), 0.50, 0.60, 4, 16);
					drawText(Form("PR #psi(2S) res: %.3f", pr_eff_int_for), 0.50, 0.55, 2, 16);
				}

				TString stateStr = (state == 1) ? "PRMC" : "NPMC";
			c_eff_int_for->SaveAs(Form("./figs_ctau3D_v2/eff_integrated_for_%s_pt3.5-40_cent0-90_m%.1f-%.1f.pdf", stateStr.Data(), massLow, massHigh));
			c_eff_int_for->SaveAs(Form("./figs_ctau3D_v2/eff_integrated_for_%s_pt3.5-40_cent0-90_m%.1f-%.1f.png", stateStr.Data(), massLow, massHigh));
			...
			if (totalPR_int_mid > 0 && totalNP_int_mid > 0) {
				...
				leg_int_mid->AddEntry(h_eff_np_int_mid, "Non-Prompt #psi(2S) (CDF)", "l");
				leg_int_mid->AddEntry(h_eff_pr_int_mid, "Prompt #psi(2S) (1-CDF)", "l");
				leg_int_mid->Draw();
				...
				if (state == 1) {
					drawText(Form("NP #psi(2S) eff: %.3f", np_res_int_mid), 0.50, 0.60, 4, 16);
					drawText(Form("PR #psi(2S) res: %.3f", 1-pr_eff_int_mid), 0.50, 0.55, 2, 16);
				} else {
					drawText(Form("NP #psi(2S) eff: %.3f", 1-np_res_int_mid), 0.50, 0.60, 4, 16);
					drawText(Form("PR #psi(2S) res: %.3f", pr_eff_int_mid), 0.50, 0.55, 2, 16);
				}

				TString stateStr = (state == 1) ? "PRMC" : "NPMC";
			c_eff_int_mid->SaveAs(Form("./figs_ctau3D_v2/eff_integrated_mid_%s_pt6.5-40_cent0-90_m%.1f-%.1f.pdf", stateStr.Data(), massLow, massHigh));
			c_eff_int_mid->SaveAs(Form("./figs_ctau3D_v2/eff_integrated_mid_%s_pt6.5-40_cent0-90_m%.1f-%.1f.png", stateStr.Data(), massLow, massHigh));
			...
			if (state == 1) {
				rootFileName_int = Form("./roots_ctau3D_v2/ctau3D_cut_integrated_PRMC_m%.1f-%.1f.root", massLow, massHigh);
			} else {
				rootFileName_int = Form("./roots_ctau3D_v2/ctau3D_cut_integrated_NPMC_m%.1f-%.1f.root", massLow, massHigh);
			}
			...
			for (int icent = 0; icent < nCentBins_for; icent++) {
				...
				if (totalPR > 0 && totalNP > 0) {
					...
					leg_cent_for->AddEntry(h_eff_np_cent_for, "Non-Prompt #psi(2S) (CDF)", "l");
					leg_cent_for->AddEntry(h_eff_pr_cent_for, "Prompt #psi(2S) (1-CDF)", "l");
					leg_cent_for->Draw();
					...
					if (state == 1) {
						drawText(Form("NP #psi(2S) eff: %.3f", np_res_centBin_for[icent]), 0.50, 0.60, 4, 16);
						drawText(Form("PR #psi(2S) res: %.3f", 1-pr_eff_centBin_for[icent]), 0.50, 0.55, 2, 16);
					} else {
						drawText(Form("NP #psi(2S) eff: %.3f", 1-np_res_centBin_for[icent]), 0.50, 0.60, 4, 16);
						drawText(Form("PR #psi(2S) res: %.3f", pr_eff_centBin_for[icent]), 0.50, 0.55, 2, 16);
					}

					TString stateStr = (state == 1) ? "PRMC" : "NPMC";
				c_eff_cent_for->SaveAs(Form("./figs_ctau3D_v2/eff_centBin_for_%s_cent%.0f-%.0f_m%.1f-%.1f.pdf", stateStr.Data(), centLow_bin/2, centHigh_bin/2, massLow, massHigh));
				c_eff_cent_for->SaveAs(Form("./figs_ctau3D_v2/eff_centBin_for_%s_cent%.0f-%.0f_m%.1f-%.1f.png", stateStr.Data(), centLow_bin/2, centHigh_bin/2, massLow, massHigh));
			...
			for (int icent = 0; icent < nCentBins_mid; icent++) {
				...
				if (totalPR > 0 && totalNP > 0) {
					...
					leg_cent_mid->AddEntry(h_eff_np_cent_mid, "Non-Prompt #psi(2S) (CDF)", "l");
					leg_cent_mid->AddEntry(h_eff_pr_cent_mid, "Prompt #psi(2S) (1-CDF)", "l");
					leg_cent_mid->Draw();
					...
					if (state == 1) {
						drawText(Form("NP #psi(2S) eff: %.3f", np_res_centBin_mid[icent]), 0.50, 0.60, 4, 16);
						drawText(Form("PR #psi(2S) res: %.3f", 1-pr_eff_centBin_mid[icent]), 0.50, 0.55, 2, 16);
					} else {
						drawText(Form("NP #psi(2S) eff: %.3f", 1-np_res_centBin_mid[icent]), 0.50, 0.60, 4, 16);
						drawText(Form("PR #psi(2S) res: %.3f", pr_eff_centBin_mid[icent]), 0.50, 0.55, 2, 16);
					}

					TString stateStr = (state == 1) ? "PRMC" : "NPMC";
				c_eff_cent_mid->SaveAs(Form("./figs_ctau3D_v2/eff_centBin_mid_%s_cent%.0f-%.0f_m%.1f-%.1f.pdf", stateStr.Data(), centLow_bin/2, centHigh_bin/2, massLow, massHigh));
				c_eff_cent_mid->SaveAs(Form("./figs_ctau3D_v2/eff_centBin_mid_%s_cent%.0f-%.0f_m%.1f-%.1f.png", stateStr.Data(), centLow_bin/2, centHigh_bin/2, massLow, massHigh));
		...
		if (state == 1) {
			drawText("Prompt #psi(2S) selection", 0.2, 0.85, 1, 18);
			drawText("(90% PR efficiency)", 0.2, 0.80, 1, 16);
		} else {
			drawText("Non-Prompt #psi(2S) selection", 0.2, 0.85, 1, 18);
		...
		c_cent_ctau->Update();
		if (state == 1) {
			c_cent_ctau->SaveAs(Form("./figs_ctau3D_v2/cent_vs_ctau3D_cut_PRMC_m%.1f-%.1f.pdf", massLow, massHigh));
			c_cent_ctau->SaveAs(Form("./figs_ctau3D_v2/cent_vs_ctau3D_cut_PRMC_m%.1f-%.1f.png", massLow, massHigh));
		} else {
			c_cent_ctau->SaveAs(Form("./figs_ctau3D_v2/cent_vs_ctau3D_cut_NPMC_m%.1f-%.1f.pdf", massLow, massHigh));
			c_cent_ctau->SaveAs(Form("./figs_ctau3D_v2/cent_vs_ctau3D_cut_NPMC_m%.1f-%.1f.png", massLow, massHigh));
		}
		...
		if (state == 1) {
			rootFileName_centBin_for = Form("./roots_ctau3D_v2/ctau3D_cut_centBin_for_PRMC_pt3.5-40.0_m%.1f-%.1f.root", massLow, massHigh);
			rootFileName_centBin_mid = Form("./roots_ctau3D_v2/ctau3D_cut_centBin_mid_PRMC_pt6.5-40.0_m%.1f-%.1f.root", massLow, massHigh);
		} else {
			rootFileName_centBin_for = Form("./roots_ctau3D_v2/ctau3D_cut_centBin_for_NPMC_pt3.5-40.0_m%.1f-%.1f.root", massLow, massHigh);
			rootFileName_centBin_mid = Form("./roots_ctau3D_v2/ctau3D_cut_centBin_mid_NPMC_pt6.5-40.0_m%.1f-%.1f.root", massLow, massHigh);
		}
		...
		c_pt_ctau->Update();
		if (state == 1) {
			c_pt_ctau->SaveAs(Form("./figs_ctau3D_v2/pt_vs_ctau3D_cut_PRMC_m%.1f-%.1f.pdf", massLow, massHigh));
			c_pt_ctau->SaveAs(Form("./figs_ctau3D_v2/pt_vs_ctau3D_cut_PRMC_m%.1f-%.1f.png", massLow, massHigh));
		} else {
			c_pt_ctau->SaveAs(Form("./figs_ctau3D_v2/pt_vs_ctau3D_cut_NPMC_m%.1f-%.1f.pdf", massLow, massHigh));
			c_pt_ctau->SaveAs(Form("./figs_ctau3D_v2/pt_vs_ctau3D_cut_NPMC_m%.1f-%.1f.png", massLow, massHigh));
		}

		// Save pt bin ctau3D cut values to ROOT file
		TString rootFileName_ptBin;
		if (state == 1) {
			rootFileName_ptBin = Form("./roots_ctau3D_v2/ctau3D_cut_ptBin_PRMC_cent%.0f-%.0f_m%.1f-%.1f.root", cLow, cHigh, massLow, massHigh);
		} else {
			rootFileName_ptBin = Form("./roots_ctau3D_v2/ctau3D_cut_ptBin_NPMC_cent%.0f-%.0f_m%.1f-%.1f.root", cLow, cHigh, massLow, massHigh);
		}
