...
void decay_length_OniaTree_v2_PbPb_2S(
		int state = 1,
		...
		)
{
	...
	// Create directories
	gSystem->mkdir("./roots_2S_PbPb/");
	gSystem->mkdir("./roots_2S_PbPb/decayL");
	gSystem->mkdir("./roots_2S_PbPb/decayL/PRMC");
	gSystem->mkdir("./roots_2S_PbPb/decayL/NPMC");

	gSystem->mkdir("./figs_2S_PbPb/");
	gSystem->mkdir("./figs_2S_PbPb/decayL");
	gSystem->mkdir("./figs_2S_PbPb/decayL/PRMC");
	gSystem->mkdir("./figs_2S_PbPb/decayL/NPMC");

	// Input files - PbPb collision
	TString fPRMC_path = "/data/Oniatree/miniAOD/OniaTree_Run2_PbPb2018MC_Prompt_forPsi2S_miniAOD112X_muonSelGlb_13May23v2.root";
	TString fNPMC_path = "/data/Oniatree/miniAOD/OniaTree_Run2_PbPb2018MC_BToPsi_forPsi2S_miniAOD112X_muonSelGlb_13May23v2.root"; // PbPb_non prompt
	...
	TFile *fPtW2;
	if(state==1){
		fPtW1 = new TFile("../compareDataToMC/ratioDataMC_AA_psi2S_DATA_ctauCut_y0_1p6_251103.root", "read");
		fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_psi2S_DATA_ctauCut_y1p6_2p4_251103.root", "read");
	}
	else if(state==2){
		fPtW1 = new TFile("../compareDataToMC/ratioDataMC_AA_Btopsi2S_DATA_ctauCut_y0_1p6_251103.root", "read");
		fPtW2 = new TFile("../compareDataToMC/ratioDataMC_AA_Btopsi2S_DATA_ctauCut_y1p6_2p4_251103.root", "read");
	}
	...
	const int nCentBins_for = (sizeof(centBin_for) / sizeof(centBin_for[0])) - 1;
	const int nCentBins_mid = (sizeof(centBin_mid) / sizeof(centBin_mid[0])) - 1;
	...
	TH1D *hcent_ctau_1 = new TH1D("hcent_ctau_1", "hcent_ctau_1", nCentBins_for, centBin_for);
	TH1D *hcent_ctau_2 = new TH1D("hcent_ctau_2", "hcent_ctau_2", nCentBins_mid, centBin_mid);
	...
	for(int bins=0; bins<h_decayPRMC->GetNbinsX(); bins++){
		if(state==1){
			...
			cout<<bins<<"th bin"<<", lpsi2S: "<<h_decayPRMC->GetBinCenter(bins)<<", PR eff: "<<h_decayPRMC->Integral(0,bins)/h_decayPRMC->GetEntries()*100<<"%"<<endl;
			...
			cout<<bins<<"th bin"<<", lpsi2S: "<<h_decayPRMC->GetBinCenter(bins)<<", NP eff: "<<(1-h_decayNPMC->Integral(0,bins)/h_decayNPMC->GetEntries())*100<<"%"<<endl;
			lcutv = new TLine(h_decayPRMC->GetBinCenter(bins), 0, h_decayPRMC->GetBinCenter(bins), 1);
	...
	if(state==1){
		...
		drawText(Form("NP #psi(2S) res: %.3f", 1 - lresi->GetY1()),pos_x_mass,pos_y-pos_y_diff*4, kRed+2,text_size);
		c_decayL->Update();
		c_decayL->SaveAs(Form("figs_2S_PbPb/decayL/PRMC/decay_%s.pdf",kineLabel.Data()));
	}
	else if(state==2){
		drawText(Form("L_{#psi(2S)} cut: %.4f", lcutv->GetX1()),pos_x_mass,pos_y-pos_y_diff*3,text_color,text_size);
		...
		c_decayL->Update();
		c_decayL->SaveAs(Form("figs_2S_PbPb/decayL/NPMC/decay_%s.pdf",kineLabel.Data()));
	}

	// Save histograms
	TFile *wf;
	if(state==1){
		wf = new TFile(Form("roots_2S_PbPb/decayL/PRMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
	}
	else if(state==2){
		wf = new TFile(Form("roots_2S_PbPb/decayL/NPMC/decay_hist_%s.root", kineLabel.Data()),"recreate");
	}
	wf->cd();
	...
	// save the l_psi2S_cut
	auto h_Lcut = new TH1D("h_Lcut", "l_psi2S_cut", 1, 0, 1);
	h_Lcut->SetBinContent(1, lcutv->GetX1());
	h_Lcut->Write();

	auto h_eff = new TH1D("h_eff", "eff", 1, 0, 1);
	if(state==1) h_eff->SetBinContent(1, lcuth->GetY1());
	else if(state==2) h_eff->SetBinContent(1, 1-lcuth->GetY1());
	h_eff->Write();

	auto h_res = new TH1D("h_res", "res", 1, 0, 1);
	if(state==1) h_res->SetBinContent(1, lresi->GetY1());
	else if(state==2) h_res->SetBinContent(1, 1-lresi->GetY1());
	h_res->Write();
	...
	if (type == 1 || type == 3) {
		...
		// Integrated-bin results (centrality 0-180)
		...
		// Integrated histograms (fixed pT ranges, cent 0-180)
		...
		for (Long64_t iev = 0; iev < treePRMC->GetEntries(); ++iev) {
			...
			for (int iQQ = 0; iQQ < Reco_QQ_size_pr; iQQ++) {
				...
				if (rap >= 1.6 && rap < 2.4) {
					...
					if (Centrality_pr >= 0 && Centrality_pr < 180 && pt_pr > 3.5 && pt_pr < 40.0) {
				...
				if (rap < 1.6) {
					...
					if (Centrality_pr >= 0 && Centrality_pr < 180 && pt_pr > 6.5 && pt_pr < 40.0) {
		...
		for (Long64_t iev = 0; iev < treeNPMC->GetEntries(); ++iev) {
			...
			for (int iQQ = 0; iQQ < Reco_QQ_size_np; iQQ++) {
				...
				if (rap >= 1.6 && rap < 2.4) {
					...
					if (Centrality_np >= 0 && Centrality_np < 180 && pt_np > 3.5 && pt_np < 40.0) {
				...
				if (rap < 1.6) {
					...
					if (Centrality_np >= 0 && Centrality_np < 180 && pt_np > 6.5 && pt_np < 40.0) {
		...
		// Calculate ctau3D cut for integrated bins (cent 0-180)
		if (type == 3) {
			cout << endl << "Integrated bins (cent 0-180):" << endl;
			{
		...
		for (int ipt = 0; ipt < nPtBins_for; ipt++) {
			...
			cout<<bins<<"th bin"<<", lpsi2S: "<<h_decay_pr_pt_for[ipt]->GetBinCenter(bins)<<", PR eff: "<<h_decay_pr_pt_for[ipt]->Integral(0,bins)/h_decay_pr_pt_for[ipt]->GetEntries()*100<<"%"<<endl;
			...
			if (state == 1) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_ptBin_for[ipt]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				drawText(Form("PR #psi(2S) eff: %.3f", pr_eff_ptBin_for[ipt]), pos_x_mass, pos_y - pos_y_diff * 3.5, text_color, text_size);
				drawText(Form("NP #psi(2S) res: %.3f", np_res_ptBin_for[ipt]), pos_x_mass, pos_y - pos_y_diff * 4, kRed + 2, text_size);
				c_decayL_for->Update();
				c_decayL_for->SaveAs(Form("figs_2S_PbPb/decayL/PRMC/decay_state%d_pt%.1f-%.1f_y1.6-2.4_m%.1f-%.1f.pdf", state, ptBin_for[ipt], ptBin_for[ipt+1], massLow, massHigh));
			} else if (state == 2) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_ptBin_for[ipt]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				drawText(Form("NP #psi(2S) eff: %.3f", np_res_ptBin_for[ipt]), pos_x_mass, pos_y - pos_y_diff * 3.5, kRed + 2, text_size);
				drawText(Form("PR #psi(2S) res: %.3f", pr_eff_ptBin_for[ipt]), pos_x_mass, pos_y - pos_y_diff * 4, text_color, text_size);
				c_decayL_for->Update();
				c_decayL_for->SaveAs(Form("figs_2S_PbPb/decayL/NPMC/decay_state%d_pt%.1f-%.1f_y1.6-2.4_m%.1f-%.1f.pdf", state, ptBin_for[ipt], ptBin_for[ipt+1], massLow, massHigh));
			}
			delete c_decayL_for;
		}
		...
		for (int ipt = 0; ipt < nPtBins_mid; ipt++) {
			...
			cout<<bins<<"th bin"<<", lpsi2S: "<<h_decay_pr_pt_mid[ipt]->GetBinCenter(bins)<<", PR eff: "<<h_decay_pr_pt_mid[ipt]->Integral(0,bins)/h_decay_pr_pt_mid[ipt]->GetEntries()*100<<"%"<<endl;
			...
			h_deff_pr_mid->GetYaxis()->SetRangeUser(0, 1.02);
			h_deff_np_mid->GetYaxis()->SetRangeUser(0, 1.02);
			...
			if (state == 1) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_ptBin_mid[ipt]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				drawText(Form("PR #psi(2S) eff: %.3f", pr_eff_ptBin_mid[ipt]), pos_x_mass, pos_y - pos_y_diff * 3.5, text_color, text_size);
				drawText(Form("NP #psi(2S) res: %.3f", np_res_ptBin_mid[ipt]), pos_x_mass, pos_y - pos_y_diff * 4, kRed + 2, text_size);
				c_decayL_mid->Update();
				c_decayL_mid->SaveAs(Form("figs_2S_PbPb/decayL/PRMC/decay_state%d_pt%.1f-%.1f_y0.0-1.6_m%.1f-%.1f.pdf", state, ptBin_mid[ipt], ptBin_mid[ipt+1], massLow, massHigh));
			} else if (state == 2) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_ptBin_mid[ipt]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				drawText(Form("NP #psi(2S) eff: %.3f", np_res_ptBin_mid[ipt]), pos_x_mass, pos_y - pos_y_diff * 3.5, kRed + 2, text_size);
				drawText(Form("PR #psi(2S) res: %.3f", pr_eff_ptBin_mid[ipt]), pos_x_mass, pos_y - pos_y_diff * 4, text_color, text_size);
				c_decayL_mid->Update();
				c_decayL_mid->SaveAs(Form("figs_2S_PbPb/decayL/NPMC/decay_state%d_pt%.1f-%.1f_y0.0-1.6_m%.1f-%.1f.pdf", state, ptBin_mid[ipt], ptBin_mid[ipt+1], massLow, massHigh));
			}
			delete c_decayL_mid;
		}
		...
		if (state == 1) {
			rootFileNamePt = Form("./roots_2S_PbPb/ctau3D_cut_ptBin_PRMC_y%.1f-%.1f.root", yLow, yHigh);
		} else {
			rootFileNamePt = Form("./roots_2S_PbPb/ctau3D_cut_ptBin_NPMC_y%.1f-%.1f.root", yLow, yHigh);
		}
	...
	//============================================================================
	// Calculate ctau3D cut for each centrality bin
	//============================================================================
	if (type == 2 || type == 3) {
		cout << endl;
		cout << "============================================" << endl;
		cout << "Calculating ctau3D cut for each centrality bin..." << endl;
		cout << "============================================" << endl;
		...
		// Fill PRMC histograms for all centrality bins
		cout << "Filling PRMC histograms for centrality bins..." << endl;
		for (Long64_t iev = 0; iev < treePRMC->GetEntries(); ++iev) {
			if (iev % 10000000 == 0) cout << "PRMC cent-binned: " << iev << " / " << treePRMC->GetEntries() << endl;
			treePRMC->GetEntry(iev);
			...
			for (int iQQ = 0; iQQ < Reco_QQ_size_pr; iQQ++) {
				...
				if (rap >= 1.6 && rap < 2.4 && pt_pr >= 3.5 && pt_pr < 40) {
					for (int icent = 0; icent < nCentBins_for; icent++) {
						if (Centrality_pr >= centBin_for[icent] && Centrality_pr < centBin_for[icent+1]) {
							h_decay_pr_cent_for[icent]->Fill(ctau);
						}
					}
				}

				if (rap < 1.6 && pt_pr >= 6.5 && pt_pr < 40) {
					for (int icent = 0; icent < nCentBins_mid; icent++) {
						if (Centrality_pr >= centBin_mid[icent] && Centrality_pr < centBin_mid[icent+1]) {
							h_decay_pr_cent_mid[icent]->Fill(ctau);
						}
					}
				}
			}
		...
		// Fill NPMC histograms for all centrality bins
		cout << "Filling NPMC histograms for centrality bins..." << endl;
		for (Long64_t iev = 0; iev < treeNPMC->GetEntries(); ++iev) {
			if (iev % 10000000 == 0) cout << "NPMC cent-binned: " << iev << " / " << treeNPMC->GetEntries() << endl;
			treeNPMC->GetEntry(iev);
			...
			for (int iQQ = 0; iQQ < Reco_QQ_size_np; iQQ++) {
				...
				if (rap >= 1.6 && rap < 2.4 && pt_np >= 3.5 && pt_np < 40) {
					for (int icent = 0; icent < nCentBins_for; icent++) {
						if (Centrality_np >= centBin_for[icent] && Centrality_np < centBin_for[icent+1]) {
							h_decay_np_cent_for[icent]->Fill(ctau);
						}
					}
				}

				if (rap < 1.6 && pt_np >= 6.5 && pt_np < 40) {
					for (int icent = 0; icent < nCentBins_mid; icent++) {
						if (Centrality_np >= centBin_mid[icent] && Centrality_np < centBin_mid[icent+1]) {
							h_decay_np_cent_mid[icent]->Fill(ctau);
						}
					}
				}
			}
		...
		// Calculate ctau3D cut for forward centrality bins
		cout << endl << "Forward rapidity (1.6-2.4) centrality bins:" << endl;
		for (int icent = 0; icent < nCentBins_for; icent++) {
			double totalPR = h_decay_pr_cent_for[icent]->GetEntries();
			double totalNP = h_decay_np_cent_for[icent]->GetEntries();

			TH1D* h_deff_pr_cent_for = new TH1D(Form("h_deff_pr_cent_for_%d", icent), ";l_{J/#psi};Efficiency", nbins, xmin, xmax);
			TH1D* h_deff_np_cent_for = new TH1D(Form("h_deff_np_cent_for_%d", icent), ";l_{J/#psi};Efficiency", nbins, xmin, xmax);
			TLine *lcutv_for = nullptr;
			TLine *lcuth_for = nullptr;
			TLine *lresi_for = nullptr;

			if (totalPR < 10 || totalNP < 10) {
				cout << Form("cent bin %.0f-%.0f: Not enough entries (PR=%d, NP=%d)",
					centBin_for[icent], centBin_for[icent+1], (int)totalPR, (int)totalNP) << endl;
				continue;
			}

			for (int bins = 0; bins < h_decay_pr_cent_for[icent]->GetNbinsX(); bins++) {
				if (state == 1) {
					h_deff_pr_cent_for->SetBinContent(bins, h_decay_pr_cent_for[icent]->Integral(0, bins) / totalPR);
					h_deff_np_cent_for->SetBinContent(bins, 1 - (h_decay_np_cent_for[icent]->Integral(0, bins) / totalNP));
					...
					lcutv_for = new TLine(h_decay_pr_cent_for[icent]->GetBinCenter(bins), 0, h_decay_pr_cent_for[icent]->GetBinCenter(bins), 1);
					lcuth_for = new TLine(xmin, pr_eff_centBin_for[icent], h_decay_pr_cent_for[icent]->GetBinCenter(bins), pr_eff_centBin_for[icent]);
					lresi_for = new TLine(xmin, np_res_centBin_for[icent], h_decay_pr_cent_for[icent]->GetBinCenter(bins), np_res_centBin_for[icent]);
				} else if (state == 2) {
					h_deff_np_cent_for->SetBinContent(bins, h_decay_np_cent_for[icent]->Integral(0, bins) / totalNP);
					h_deff_pr_cent_for->SetBinContent(bins, 1 - (h_decay_pr_cent_for[icent]->Integral(0, bins) / totalPR));
					...
					lcutv_for = new TLine(h_decay_pr_cent_for[icent]->GetBinCenter(bins), 0, h_decay_pr_cent_for[icent]->GetBinCenter(bins), 1);
					lcuth_for = new TLine(xmin, h_decay_np_cent_for[icent]->Integral(0, bins) / totalNP, h_decay_pr_cent_for[icent]->GetBinCenter(bins), h_decay_np_cent_for[icent]->Integral(0, bins) / totalNP);
					lresi_for = new TLine(xmin, h_decay_pr_cent_for[icent]->Integral(0, bins) / totalPR, h_decay_pr_cent_for[icent]->GetBinCenter(bins), h_decay_pr_cent_for[icent]->Integral(0, bins) / totalPR);
				}
			}

			hcent_ctau_1->SetBinContent(icent+1, ctau3D_cut_centBin_for[icent]);
			cout << Form("  cent %.0f-%.0f: ctau cut = %.4f, PR eff = %.3f, NP res = %.3f",
				centBin_for[icent], centBin_for[icent+1], ctau3D_cut_centBin_for[icent],
				pr_eff_centBin_for[icent], np_res_centBin_for[icent]) << endl;

			// Draw and save plots for this centrality bin
			TCanvas* c_decayL_cent_for = new TCanvas(Form("c_decayL_cent_for_%d", icent), "", 600, 600);
			h_deff_pr_cent_for->GetYaxis()->SetRangeUser(0, 1.02);
			h_deff_np_cent_for->GetYaxis()->SetRangeUser(0, 1.02);
			h_deff_pr_cent_for->Draw("l");
			h_deff_np_cent_for->Draw("same");
			h_deff_np_cent_for->SetLineColor(kRed+2);
			jumSun(xmin, 1, xmax, 1, 1, 1);
			if (lcutv_for) lcutv_for->Draw("same");

			drawText(Form("Centrality %.0f-%.0f%s", centBin_for[icent]/2, centBin_for[icent+1]/2, perc.Data()), pos_x_mass, pos_y, text_color, text_size);
			drawText(Form("%.1f < p_{T}^{#mu#mu} < %.1f GeV/c", 3.5, 40.0), pos_x_mass, pos_y - pos_y_diff, text_color, text_size);
			drawText(Form("%.1f < |y^{#mu#mu}| < %.1f", 1.6, 2.4), pos_x_mass, pos_y - pos_y_diff * 1.5, text_color, text_size);
			drawText("|#eta^{#mu}| < 2.4", pos_x_mass, pos_y - pos_y_diff * 2.0, text_color, text_size);

			if (state == 1) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_centBin_for[icent]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				drawText(Form("PR #psi(2S) eff: %.3f", pr_eff_centBin_for[icent]), pos_x_mass, pos_y - pos_y_diff * 3.5, text_color, text_size);
				drawText(Form("NP #psi(2S) res: %.3f", np_res_centBin_for[icent]), pos_x_mass, pos_y - pos_y_diff * 4, kRed + 2, text_size);
				c_decayL_cent_for->Update();
				c_decayL_cent_for->SaveAs(Form("figs_2S_PbPb/decayL/PRMC/decay_state%d_cent%.0f-%.0f_y1.6-2.4_m%.1f-%.1f.pdf",
					state, centBin_for[icent], centBin_for[icent+1], massLow, massHigh));
			} else if (state == 2) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_centBin_for[icent]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				drawText(Form("NP #psi(2S) eff: %.3f", np_res_centBin_for[icent]), pos_x_mass, pos_y - pos_y_diff * 3.5, kRed + 2, text_size);
				drawText(Form("PR #psi(2S) res: %.3f", pr_eff_centBin_for[icent]), pos_x_mass, pos_y - pos_y_diff * 4, text_color, text_size);
				c_decayL_cent_for->Update();
				c_decayL_cent_for->SaveAs(Form("figs_2S_PbPb/decayL/NPMC/decay_state%d_cent%.0f-%.0f_y1.6-2.4_m%.1f-%.1f.pdf",
					state, centBin_for[icent], centBin_for[icent+1], massLow, massHigh));
			}
			delete c_decayL_cent_for;
		}

		// Calculate ctau3D cut for mid centrality bins
		cout << endl << "Mid rapidity (0-1.6) centrality bins:" << endl;
		for (int icent = 0; icent < nCentBins_mid; icent++) {
			double totalPR = h_decay_pr_cent_mid[icent]->GetEntries();
			double totalNP = h_decay_np_cent_mid[icent]->GetEntries();

			TH1D* h_deff_pr_cent_mid = new TH1D(Form("h_deff_pr_cent_mid_%d", icent), ";l_{J/#psi};Efficiency", nbins, xmin, xmax);
			TH1D* h_deff_np_cent_mid = new TH1D(Form("h_deff_np_cent_mid_%d", icent), ";l_{J/#psi};Efficiency", nbins, xmin, xmax);
			TLine *lcutv_mid = nullptr;
			TLine *lcuth_mid = nullptr;
			TLine *lresi_mid = nullptr;
			...
			if (totalPR < 10 || totalNP < 10) {
				cout << Form("cent bin %.0f-%.0f: Not enough entries (PR=%d, NP=%d)",
					centBin_mid[icent], centBin_mid[icent+1], (int)totalPR, (int)totalNP) << endl;
				continue;
			}

			for (int bins = 0; bins < h_decay_pr_cent_mid[icent]->GetNbinsX(); bins++) {
				if (state == 1) {
					h_deff_pr_cent_mid->SetBinContent(bins, h_decay_pr_cent_mid[icent]->Integral(0, bins) / totalPR);
					h_deff_np_cent_mid->SetBinContent(bins, 1 - (h_decay_np_cent_mid[icent]->Integral(0, bins) / totalNP));
					...
					lcutv_mid = new TLine(h_decay_pr_cent_mid[icent]->GetBinCenter(bins), 0, h_decay_pr_cent_mid[icent]->GetBinCenter(bins), 1);
					lcuth_mid = new TLine(xmin, pr_eff_centBin_mid[icent], h_decay_pr_cent_mid[icent]->GetBinCenter(bins), pr_eff_centBin_mid[icent]);
					lresi_mid = new TLine(xmin, np_res_centBin_mid[icent], h_decay_pr_cent_mid[icent]->GetBinCenter(bins), np_res_centBin_mid[icent]);
				} else if (state == 2) {
					h_deff_np_cent_mid->SetBinContent(bins, h_decay_np_cent_mid[icent]->Integral(0, bins) / totalNP);
					h_deff_pr_cent_mid->SetBinContent(bins, 1 - (h_decay_pr_cent_mid[icent]->Integral(0, bins) / totalPR));
					...
					lcutv_mid = new TLine(h_decay_pr_cent_mid[icent]->GetBinCenter(bins), 0, h_decay_pr_cent_mid[icent]->GetBinCenter(bins), 1);
					lcuth_mid = new TLine(xmin, h_decay_np_cent_mid[icent]->Integral(0, bins) / totalNP, h_decay_pr_cent_mid[icent]->GetBinCenter(bins), h_decay_np_cent_mid[icent]->Integral(0, bins) / totalNP);
					lresi_mid = new TLine(xmin, h_decay_pr_cent_mid[icent]->Integral(0, bins) / totalPR, h_decay_pr_cent_mid[icent]->GetBinCenter(bins), h_decay_pr_cent_mid[icent]->Integral(0, bins) / totalPR);
				}
			}

			hcent_ctau_2->SetBinContent(icent+1, ctau3D_cut_centBin_mid[icent]);
			cout << Form("  cent %.0f-%.0f: ctau cut = %.4f, PR eff = %.3f, NP res = %.3f",
				centBin_mid[icent], centBin_mid[icent+1], ctau3D_cut_centBin_mid[icent],
				pr_eff_centBin_mid[icent], np_res_centBin_mid[icent]) << endl;

			// Draw and save plots for this centrality bin
			TCanvas* c_decayL_cent_mid = new TCanvas(Form("c_decayL_cent_mid_%d", icent), "", 600, 600);
			h_deff_pr_cent_mid->GetYaxis()->SetRangeUser(0, 1.02);
			h_deff_np_cent_mid->GetYaxis()->SetRangeUser(0, 1.02);
			h_deff_pr_cent_mid->Draw("l");
			h_deff_np_cent_mid->Draw("same");
			h_deff_np_cent_mid->SetLineColor(kRed+2);
			jumSun(xmin, 1, xmax, 1, 1, 1);
			if (lcutv_mid) lcutv_mid->Draw("same");

			drawText(Form("Centrality %.0f-%.0f%s", centBin_mid[icent]/2, centBin_mid[icent+1]/2, perc.Data()), pos_x_mass, pos_y, text_color, text_size);
			drawText(Form("%.1f < p_{T}^{#mu#mu} < %.1f GeV/c", 6.5, 40.0), pos_x_mass, pos_y - pos_y_diff, text_color, text_size);
			drawText(Form("|y^{#mu#mu}| < %.1f", 1.6), pos_x_mass, pos_y - pos_y_diff * 1.5, text_color, text_size);
			drawText("|#eta^{#mu}| < 2.4", pos_x_mass, pos_y - pos_y_diff * 2.0, text_color, text_size);

			if (state == 1) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_centBin_mid[icent]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				drawText(Form("PR #psi(2S) eff: %.3f", pr_eff_centBin_mid[icent]), pos_x_mass, pos_y - pos_y_diff * 3.5, text_color, text_size);
				drawText(Form("NP #psi(2S) res: %.3f", np_res_centBin_mid[icent]), pos_x_mass, pos_y - pos_y_diff * 4, kRed + 2, text_size);
				c_decayL_cent_mid->Update();
				c_decayL_cent_mid->SaveAs(Form("figs_2S_PbPb/decayL/PRMC/decay_state%d_cent%.0f-%.0f_y0.0-1.6_m%.1f-%.1f.pdf",
					state, centBin_mid[icent], centBin_mid[icent+1], massLow, massHigh));
			} else if (state == 2) {
				drawText(Form("L_{#psi(2S)} cut: %.4f", ctau3D_cut_centBin_mid[icent]), pos_x_mass, pos_y - pos_y_diff * 3, text_color, text_size);
				drawText(Form("NP #psi(2S) eff: %.3f", np_res_centBin_mid[icent]), pos_x_mass, pos_y - pos_y_diff * 3.5, kRed + 2, text_size);
				drawText(Form("PR #psi(2S) res: %.3f", pr_eff_centBin_mid[icent]), pos_x_mass, pos_y - pos_y_diff * 4, text_color, text_size);
				c_decayL_cent_mid->Update();
				c_decayL_cent_mid->SaveAs(Form("figs_2S_PbPb/decayL/NPMC/decay_state%d_cent%.0f-%.0f_y0.0-1.6_m%.1f-%.1f.pdf",
					state, centBin_mid[icent], centBin_mid[icent+1], massLow, massHigh));
			}
			delete c_decayL_cent_mid;
		}

		// Save centrality-binned results
		TString rootFileNameCent;
		if (state == 1) {
			rootFileNameCent = Form("./roots_2S_PbPb/ctau3D_cut_centBin_PRMC_y%.1f-%.1f.root", yLow, yHigh);
		} else {
			rootFileNameCent = Form("./roots_2S_PbPb/ctau3D_cut_centBin_NPMC_y%.1f-%.1f.root", yLow, yHigh);
		}
		TFile *wfCent = new TFile(rootFileNameCent, "RECREATE");
		wfCent->cd();
		...
		for (int icent = 0; icent < nCentBins_for; icent++) {
			h_decay_pr_cent_for[icent]->Write();
			h_decay_np_cent_for[icent]->Write();
		}
		for (int icent = 0; icent < nCentBins_mid; icent++) {
			h_decay_pr_cent_mid[icent]->Write();
			h_decay_np_cent_mid[icent]->Write();
		}

		wfCent->Close();
		cout << endl << "Saved centrality-binned ROOT file: " << rootFileNameCent << endl;
	}
